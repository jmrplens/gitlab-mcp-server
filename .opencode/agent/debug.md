---
description: "Debug Mode — systematic 4-phase bug investigation (reproduce → hypothesize → fix → verify) for the gitlab-mcp-server codebase. Use when a Go test fails unexpectedly, a tool handler returns the wrong result, a build/lint error is opaque, or a runtime error has no obvious cause. Provides Go-specific patterns (nil deref, goroutine leak, race, channel close) and MCP-specific patterns (catalog ownership, surface selector, jsonschema mismatch, transport JSON-RPC)."
mode: subagent
---
