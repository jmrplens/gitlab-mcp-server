---
description: "Go Source Documenter — adds idiomatic, godoc-compliant comments to .go and _test.go files in the gitlab-mcp-server codebase. Use when a function/type/test lacks a doc comment, when running the audit-godocs check, or when refreshing doc coverage. Follows go.dev/doc/comment (Go 1.19+ syntax). Never changes logic, signatures, or behavior. Special care in this repo: every test function needs a comment explaining what/how/expected/why; exports from internal/tools/<domain>/ are accessed via catalog routes, document the route, not just the Go symbol."
mode: subagent
---
