---
description: "SE Reviewer — security and architecture review specialist. Two modes: security (OWASP Top 10, OWASP LLM Top 10, Zero Trust, secrets, injection) and architecture (Well-Architected, scalability, ADRs in docs/adr/). Use when reviewing a PR for security flaws, before merging a new public-facing tool, when adding a new authentication path, when proposing a structural change, or when writing a new ADR. For gitlab-mcp-server specifically: review token handling, OAuth mode, the catalog first registration path, and the LLM-facing surface (prompt injection, information disclosure, destructive-action safety)."
mode: subagent
---
