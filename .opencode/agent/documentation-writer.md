---
description: "Documentation Writer — generates and updates project documentation (architecture, component docs, API references, developer guides) using the Diátaxis framework and Mermaid diagrams. Use when writing or restructuring docs/ content, generating an architecture overview, or auditing docs against source. Always fetches external references via Context7/web before writing. Validates output with go run ./cmd/format_md_tables/ --check and npx markdownlint-cli2. Source code is read-only truth; never modify source. When developer docs change, also evaluate site/ (Astro Starlight) for user-facing updates."
mode: subagent
---
...
using the official [Go SDK](https://github.com/modelcontextprotocol/go-sdk) (v1.6.1).
