...
Runs the Docker GitLab CE evaluation suite end-to-end for one tool surface:
...
if [[ -n "$requested_preset" ]]; then
  case "$requested_preset" in
    docker-read|docker-mutating-safe|docker-destructive-safe|docker-capability-discovery)
...
retry_setup_gitlab() {
  ...
  printf '==> setup-gitlab\n'
  {
    for attempt in 1 2 3; do
      if ./test/e2e/scripts/setup-gitlab.sh "$gitlab_url"; then
        return 0
      fi
      if [[ "$attempt" == "3" ]]; then
...
run_evaluator() {
  local name="$1"
  shift
  run_logged "$name" env GITLAB_ENTERPRISE=false "$go_bin" run ./cmd/eval_mcp_surfaces "$@"
}
...
run_logged docker-down-initial "${compose[@]}" down -v
run_logged docker-up "${compose[@]}" up -d
cleanup_started=1
...
run_evaluator fixtures-prepare \
  ...
  --fixtures-only \
  --fixtures "$fixtures" \
  --out "$run_dir/${timestamp}-${surface}-fixtures.md" \
  --terminal-log "$log_dir/fixtures-prepare.terminal.log"
...
publish_args=(
  --publish-docs
  --publish-label "Docker CE $surface $timestamp"
