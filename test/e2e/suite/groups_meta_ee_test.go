...
//
// The helpers are organized by domain (boards, analytics, security settings,
// SSH certs, credentials, LDAP, SAML, wikis, protected branches, protected
// environments). Each `runEnterpriseMetaGroup*Operations` function builds its
// own [testing.T.Run] subtree so the per-domain E2E flow can be reported
// independently by `go test`.
//
// The top-level `runMetaGroup*Operations` dispatchers skip themselves on
// Community Edition sessions so the same helper functions stay callable from
// shared fixtures without compile-time build-tag gymnastics.
package suite
...
// subtree. On Community Edition sessions it returns without running anything,
// allowing CE and EE suites to share the call site without build-tag checks.
...
// meta-tool subtree (analytics, security settings, SSH certs, credentials,
// LDAP, SAML, wikis, protected branches, protected environments). Each
// subtree is invoked sequentially so per-domain failures do not mask other
// domains in the same test.
//
...
// message contains every fragment. It is used to assert that error responses
// carry the action-specific guidance the meta-tool surface is contracted to
// surface (license tier hints, next-step URLs, etc.).
...
// list-lists → delete. Each subtest scopes its variable so the suite can
// run with or without pre-existing boards on the test group.
...
// lifecycle (create → list → get → update → delete). Update verifies the
// new content via a follow-up get so an edit endpoint that silently
// no-ops is caught.
...
// via the gitlab_group meta-tool. The created rule is wildcarded
// (release-*/ee-*) so it does not collide with real release branches.
...
// get → update → unprotect) via the gitlab_group meta-tool. The update
// step re-declares deploy_access_levels to avoid relying on the
// required_approvals field that older EE versions reject with 422.
