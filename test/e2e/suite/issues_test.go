...
func TestIndividual_Issues(t *testing.T) {
	...
	t.Run("Create", func(t *testing.T) {
		...
		requireTruef(t, out.IID > 0, "issue IID should be positive, got %d", out.IID)
		requireTruef(t, out.State == "opened", "expected state 'opened', got %q", out.State)
		issueIID = out.IID
	...
	t.Run("Get", func(t *testing.T) {
		...
		requireTruef(t, out.IID == issueIID, "expected issue IID %d, got %d", issueIID, out.IID)
		requireTruef(t, out.State == "opened", "expected state 'opened', got %q", out.State)
	...
	t.Run("GetEmbedsCanonicalResource", func(t *testing.T) {
		...
		requireTruef(t, result != nil, "nil result")
		...
		requireTruef(t, found != nil, "expected an EmbeddedResource content block in gitlab_issue_get result")
		requireTruef(t, found.Resource != nil, "EmbeddedResource.Resource is nil")
		wantURI := fmt.Sprintf("gitlab://project/%d/issue/%d", proj.ID, issueIID)
		requireTruef(t, found.Resource.URI == wantURI, "embedded URI = %q, want %q", found.Resource.URI, wantURI)
		requireTruef(t, found.Resource.MIMEType == "application/json", "embedded MIME = %q, want application/json", found.Resource.MIMEType)
		requireTruef(t, found.Resource.Text != "", "embedded Text payload empty")
	...
	t.Run("List", func(t *testing.T) {
		...
		requireTruef(t, len(out.Issues) >= 1, "expected at least 1 issue, got %d", len(out.Issues))
		...
		requireTruef(t, found, "issue #%d not found in list", issueIID)
	...
	t.Run("Update", func(t *testing.T) {
		...
		requireTruef(t, out.IID == issueIID, "expected issue IID %d, got %d", issueIID, out.IID)
		requireTruef(t, out.Title == "E2E issue — updated title", "expected updated title, got %q", out.Title)
	...
	t.Run("NoteCreate", func(t *testing.T) {
		...
		requireTruef(t, out.ID > 0, "issue note ID should be positive, got %d", out.ID)
		noteID = out.ID
	...
	t.Run("NoteList", func(t *testing.T) {
		...
		requireTruef(t, found, "note ID=%d not found in list", noteID)
...
func TestMeta_Issues(t *testing.T) {
	...
	t.Run("List", func(t *testing.T) {
		...
		requireTruef(t, found, "issue #%d not in meta list", issueIID)
	...
	t.Run("NoteCreate", func(t *testing.T) {
		...
		requireTruef(t, out.ID > 0, "note ID should be positive, got %d", out.ID)
		noteID = out.ID
	...
	t.Run("NoteList", func(t *testing.T) {
		...
		requireTruef(t, len(out.Notes) >= 1, "expected at least 1 note")
		...
		requireTruef(t, found, "note %d not found in list", noteID)
