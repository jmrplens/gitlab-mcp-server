...
| `project_mirrors_test.go::TestMeta_ProjectRemoteMirrors` | `project` | Yes | Yes | None | Creates push mirror configuration against a second test-owned GitLab project via `E2E_GITLAB_INTERNAL_URL`. |
| `enterprise_test.go::TestMeta_SecurityClassifications` | `group`, `project`, `enterprise` | Yes | Yes | `CapabilityEnterprise` | Exercises Premium/Ultimate security categories and attributes, and must return before fixture creation on CE. |
| `custom_emoji_test.go::TestIndividual_CustomEmoji` / `TestMeta_CustomEmoji` | `group` | Yes | Yes | None | Creates custom emoji from the Docker fixture PNG (`E2E_FIXTURE_URL`) instead of a public image URL. |
