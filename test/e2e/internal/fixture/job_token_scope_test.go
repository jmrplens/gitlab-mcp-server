...
// pins the two properties the file's comment rests on: the scope is switched
// on before the entry is added, and an allowlist that does not hold the target
// afterwards is reported rather than handed back as a fixture.
...
// retried-attempt ending: GitLab refuses the second add as a conflict, and
// the entry the first one made is what the caller asked for.
func TestAllowJobTokenProject_AlreadyOnTheList_CarriesOnToTheReadBack(t *testing.T) {
	stub, client := newStubGitLab(t)
	scriptAllowlist(stub,
		stubNoContent(),
		stubRefusal(http.StatusConflict, "Target project is already in the allowlist."),
		stubOK([]any{map[string]any{"id": 2, "path_with_namespace": "e2e/target"}}),
	)

	if _, err := allowJobTokenProject(t.Context(), client, 1, 2); err != nil {
		t.Errorf("allowJobTokenProject() error = %v, want the conflict tolerated", err)
	}
}
