...
printing the address it derived. It publishes the Bitbucket port on loopback
when the Bitbucket URL (the derived one, or `E2E_DOCKER_BITBUCKET_URL` when set)
names `localhost`, `127.*` or `[::1]`, and on `0.0.0.0` otherwise, since the
setup script on this machine has to reach a remote container's port; a URL
...
- **Orphans**: what an interrupted run left behind is swept by hand with `make e2e-clean-orphans`, by name prefix (`E2E_SWEEP_PREFIX`, `e2e-` by default), against the instance `.env` names: every project, group and user carrying the prefix is deleted. The test the target runs skips when the variable is empty, which guards only a bare `go test` of it.
