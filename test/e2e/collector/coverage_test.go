...
// behind //go:build e2e, so nothing compiled under collectore2e can import it,
// and a package every one of the four build configurations could import would
// have to carry no constraint at all — which is the one shape the gate over
...
// drifts leaves dist/e2e-cover/collector empty, which `make e2e-go-coverage`
...
func coverEnviron(t *testing.T) []string {
	...
	// under test/e2e/collector and leave the directory the run named empty.
