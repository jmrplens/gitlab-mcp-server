...
key protected by the operating system: the Keychain on macOS, DPAPI under your
Windows login on Windows, and the desktop keyring (such as GNOME Keyring or
KWallet) on Linux. A Linux desktop without a keyring stores them without that
protection. The server keeps the token in process memory only, uses it solely to
