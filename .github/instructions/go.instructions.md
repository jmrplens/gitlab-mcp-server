...
All test functions must follow the pattern `TestFunctionName_Scenario_ExpectedResult`:

```go
// ✅ GOOD: 3-part names with function, scenario, and expected result
func TestCreateBranch_EmptyName_ReturnsError(t *testing.T) { ... }
func TestListProject_WithPagination_Success(t *testing.T) { ... }
func TestApproveGroup_MissingUserID_ReturnsError(t *testing.T) { ... }
...
// ❌ BAD: TestCov prefix
func TestCovListMetricImagesWithPagination(t *testing.T) { ... }
```

- `FunctionName` is the function or method under test (PascalCase)
- `Scenario` describes the input, condition, or variation (PascalCase)
...
- E2E workflow entry points (`TestFullWorkflow`, `TestMetaToolWorkflow`) are exempt
