...
| `GITLAB_MCP_ACTION_TIMEOUT`         | Cancel an action still running after this long, both transports (also `--action-timeout` in HTTP mode; `0` disables, max 24h) | `1h`             |
| `GITLAB_MCP_RATE_LIMIT_RPS`         | Per-server rate limit, in req/s, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `prompts/get`) (also `--rate-limit-rps` flag; `0` = disabled) | `0` (default)    |
