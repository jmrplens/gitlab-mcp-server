...
func TestContextAt_Templates_ClassifiesTheConstructTheHoleSitsIn(t *testing.T) {
	cases := []struct {
		...
		{name: "hash followed by a word is not a heading", template: "#tag %s", want: ctxProse},
		{name: "hash and a tab is a heading", template: "#\t%s", want: ctxHeading},
		...
		{name: "a number followed by a letter is not a list", template: "12x %s", want: ctxProse},
		{name: "an ordered marker without a space is not a list", template: "1.x %s", want: ctxProse},
