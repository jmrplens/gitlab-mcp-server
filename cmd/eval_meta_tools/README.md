...
```bash
timeout 180s go run ./cmd/eval_meta_tools \
  --compare dist/evaluation/meta-tools/snapshots/release-1.6.1/tokens.md \
...
E2E_MODE=docker timeout 900s go run ./cmd/eval_meta_tools \
  --tools-file dist/evaluation/meta-tools/snapshots/release-1.6.1/tools.json \
  --mcp-command dist/evaluation/meta-tools/snapshots/release-1.6.1/gitlab-mcp-server-release-1.6.1 \
  ...
  --task MS-028 \
  --skip-unavailable \
  --out dist/evaluation/meta-tools/snapshots/release-1.6.1/live-ms-028.md
```
