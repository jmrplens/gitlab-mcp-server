...
func TestRun_AgainstAnInstance_WritesBothArtifacts(t *testing.T) {
	...
	for _, want := range []string{"introspecting", "wrote gitlab-schema.graphql.gz", "GitLab 19.4.0", "retrieved 2026-09-06"} {
...
func TestRun_CheckMode_JudgesTheCommittedFilesWithoutNetwork(t *testing.T) {
	...
	if err := writeArtifacts(sound, compress(minimalSDL), sampleSource); err != nil {
		t.Fatalf("prepare the fixture: %v", err)
	}

	t.Run("a pair that parses", func(t *testing.T) {
		status, out, errOut := runCommand(t, genRun{dir: sound, check: true})
	...
	t.Run("a directory with nothing in it", func(t *testing.T) {
		status, _, errOut := runCommand(t, genRun{dir: t.TempDir(), check: true})
