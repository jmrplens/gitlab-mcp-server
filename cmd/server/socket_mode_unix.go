...
// socket_mode_unix.go creates the unix socket with its permission mode already
...
func bindUnixSocket(ctx context.Context, path string, mode os.FileMode) (net.Listener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(
			"listening on unix socket %q (bound as %q first, see socket_mode_unix.go): %w", path, staged, err,
		)
	}
