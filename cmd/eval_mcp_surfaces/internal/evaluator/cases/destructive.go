...
func destructiveEvalCases() []Case {
	return []Case{
		...
		baseDestructiveEvalCase("MT-031", "Delete file `tmp/eval.txt` from branch `feature/eval` in project `my-org/tools/gitlab-mcp-server`, with commit message `Delete evaluation file`. The path is exact and the file exists, so delete it as given rather than checking for it first.", destructiveStep("gitlab_repository", "file_delete", params("project_id", "file_path", "branch", "commit_message"), params("confirm"))),
		...
		baseDestructiveEvalCase(
			"MS-026", "Exercise scoped group CI variable CRUD in group `my-org`: create variable `GROUP_EVAL_CRUD_TOKEN` with value `group-crud-value-1` scoped to environment `review/eval`, get that scoped variable, update it to value `group-crud-value-2`, then delete that same scoped variable.",
...
var baseDestructiveLivePrompts = map[string]livePrompt{
	...
	"MT-017":  {"Merge merge request `{{ .MergeRequest.IID }}` in project `{{ .Project.Path }}` when its pipeline succeeds.", []string{fixtureMergeableMergeRequest}},
	...
	"MT-031":  {"Delete file `{{ .Values.file_path }}` from branch `{{ .Branch.Name }}` in project `{{ .Project.Path }}`, with commit message `Delete evaluation file`. The path is exact and the file exists, so delete it as given rather than checking for it first.", []string{fixtureRepositoryFileDelete}},
	...
	"MS-026":  {"Exercise scoped group CI variable CRUD in group `{{ .Group.Path }}`: create variable `{{ .Values.group_ci_variable_key }}` with value `group-crud-value-1` scoped to environment `review/eval`, get that scoped variable, update it to value `group-crud-value-2`, then delete that same scoped variable.", []string{fixtureBootstrapProject, fixtureAttemptNames}},
