...
func mutatingEvalCases() []Case {
	return []Case{
		...
		baseMutatingEvalCase("MT-012", "Close issue `42` in project `my-org/tools/gitlab-mcp-server`.", readStep("gitlab_issue", "update", params("project_id", "issue_iid", "state_event"), nil)),
		...
		baseMutatingEvalCase("MT-027", "Update CI variable `EVAL_TOKEN` scoped to environment `production` in project `my-org/tools/gitlab-mcp-server` to value `masked-value-456`.", readStep("gitlab_ci_variable", "update", params("project_id", "key", "value", "environment_scope"), nil)),
		baseMutatingEvalCase("MT-030", "Create file `tmp/eval.txt` containing `evaluation file` on branch `feature/eval` in project `my-org/tools/gitlab-mcp-server`, with commit message `Create evaluation file`.", readStep("gitlab_repository", "file_create", params("project_id", "file_path", "branch", "content", "commit_message"), nil)),
		...
		baseMutatingEvalCase("MT-036", "Create a release named `v0.0.0-eval` for tag `v0.0.0-eval` from `main` in project `my-org/tools/gitlab-mcp-server`.", readStep("gitlab_release", "create", params("project_id", "tag_name", "ref"), params("name", "description"))),
		...
		baseMutatingEvalCase("MT-046", "Pause runner ID `99`.", readStep("gitlab_runner", "update", params("runner_id", "paused"), nil)),
		...
		baseMutatingEvalCase("MT-058", "Add a coverage badge to project `my-org/tools/gitlab-mcp-server` that links to `https://example.com/coverage` and shows the image at `https://example.com/badge.svg`.", readStep("gitlab_project", "badge_add", params("project_id", "link_url", "image_url"), nil)),
		...
		baseMutatingEvalCase("MT-061", "Resolve discussion `abc123` on merge request `7` in project `my-org/tools/gitlab-mcp-server`.", readStep("gitlab_mr_review", "discussion_resolve", params("project_id", "merge_request_iid", "discussion_id", "resolved"), nil)),
		...
		baseMutatingEvalCase("MT-064", "Play manual job `999` in project `my-org/tools/gitlab-mcp-server` with variable `DEPLOY_ENV=staging`.", readStep("gitlab_job", "play", params("project_id", "job_id"), params("job_variables_attributes"))),
		...
		baseMutatingEvalCase(
			"MS-008", "Troubleshoot runner ID `99` for project `my-org/tools/gitlab-mcp-server`: list project runners, inspect runner jobs, fetch trace for job `999`, then pause the runner.",
		...
		baseMutatingEvalCase(
			"MS-038", "Publish the local fixture files `__PACKAGE_RELEASE_FILES__` from directory `__PACKAGE_RELEASE_DIR__` to GitLab Generic Packages in project `my-org/tools/gitlab-mcp-server` as package `__PACKAGE_RELEASE_PACKAGE__` version `__PACKAGE_RELEASE_VERSION__`, then create release `__PACKAGE_RELEASE_TAG__` from ref `main` named `Evaluation package release`, and link all of the uploaded package files to that release as package assets in a single batch call, not one call per file. Upload the package files first, then generate the release, then link all the returned package URLs together in one call; do not construct package URLs manually.",
...
var baseMutatingLivePrompts = map[string]livePrompt{
	...
	"MT-012":             {"Close issue `{{ .Issue.IID }}` in project `{{ .Project.Path }}`.", []string{fixtureIssue}},
	...
	taskFileCreateID:     {"Create file `{{ .Values.file_path }}` containing `evaluation file` on branch `{{ .Values.feature_branch }}` in project `{{ .Project.Path }}`, with commit message `Create evaluation file`.", []string{fixtureBranch, fixtureAttemptNames}},
	...
	"MT-036":             {"Create a release named `{{ .Release.Name }}` for tag `{{ .Release.TagName }}` from `{{ .Branch.Default }}` in project `{{ .Project.Path }}`.", []string{fixtureBootstrapProject, fixtureAttemptNames}},
	"MT-046":             {"Pause runner ID `{{ .Runner.ID }}`.", []string{fixtureRunnerRemove}},
	...
	"MT-061":             {"Resolve discussion `{{ .Values.discussion_id }}` on merge request `{{ .MergeRequest.IID }}` in project `{{ .Project.Path }}`.", []string{fixtureMergeRequestDiscussion}},
	...
	"MS-008":             {"Troubleshoot runner ID `{{ .Runner.ID }}` for project `{{ .Project.Path }}`: list project runners, inspect runner jobs, fetch trace for job `{{ .Job.ID }}`, then pause the runner.", []string{fixtureFailedJobArtifact, fixtureRunnerRemove}},
