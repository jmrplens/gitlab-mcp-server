...
func appendReleaseGuidance(task evalTask, steps []evalStep, guidance string) string {
	...
	if len(steps) > 2 && (steps[0].ExpectedAction == "release.list" || steps[0].ExpectedTool == "gitlab_release" && steps[0].ExpectedAction == "list") {
		guidance += ` For release inventory plus notes, follow exactly this order: release.list, repository.compare. repository.compare requires params.from and params.to.`
	}
...
func appendFailedPipelineGuidance(task evalTask, steps []evalStep, guidance string) string {
	...
	if taskHasStep(steps, "gitlab_pipeline", "get") || taskHasAction(steps, actionPipelineGet) {
		guidance += ` For failed pipeline investigation, follow exactly this order when requested: discover_project.resolve, pipeline.get, job.list, job.trace. Inspecting one known pipeline ID means pipeline.get with params.pipeline_id; do not substitute pipeline.list.`
	}
