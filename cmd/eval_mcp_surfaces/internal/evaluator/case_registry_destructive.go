...
func destructiveEvalCases() []EvalCase {
	return []EvalCase{
		...
		baseDestructiveEvalCase("MS-003", "Prepare a batch review for MR `7` in project `my-org/tools/gitlab-mcp-server`: inspect the MR, inspect changes, create a draft note saying `Please add a regression test`, then publish all draft notes.",
		...
		baseDestructiveEvalCase("MS-004", "Clean up release `v0.0.0-eval` in project `my-org/tools/gitlab-mcp-server`: verify the tag, verify the release, list release links, delete the release, then delete the tag.",
		...
		baseDestructiveEvalCase("MS-007", "Clean up an obsolete package in project `my-org/tools/gitlab-mcp-server`: list generic packages, list files for package ID `55`, then delete package ID `55`.",
		...
		baseDestructiveEvalCase("MS-009", "Schedule and then remove an instance maintenance banner: read current instance settings, immediately create broadcast message `Evaluation maintenance`, then delete the broadcast message created in the previous step using the returned ID.",
		...
		baseDestructiveEvalCase("MS-013", "Remove a temporary feature rollout from project `my-org/tools/gitlab-mcp-server`: inspect feature flag `eval_flag`, list feature flag user lists, then delete the flag.",
		...
		baseDestructiveEvalCase("MS-014", "Exercise issue CRUD in project `my-org/tools/gitlab-mcp-server`: create issue `eval-crud-issue`, fetch it with issue get using the returned issue IID, update its title to `eval-crud-issue-updated`, close it, reopen it, then delete it.",
		...
		baseDestructiveEvalCase("MS-015", "Exercise issue note CRUD in project `my-org/tools/gitlab-mcp-server`: create issue `eval-note-issue`, add a note saying `first note`, fetch that note with note get using the returned note ID, update the note to `updated note`, delete the note, then delete the issue.",
			readStep("gitlab_issue", "create", params("project_id", "title"), params("description", "labels")),
		...
		baseDestructiveEvalCase("MS-016", "Exercise issue link CRUD in project `my-org/tools/gitlab-mcp-server`: create source issue `eval-link-source`, create target issue `eval-link-target`, link source to target as `relates_to`, list source issue links, delete the returned issue link, then delete both issues.",
		...
		baseDestructiveEvalCase("MS-017", "Exercise repository file CRUD in project `my-org/tools/gitlab-mcp-server`: create file `tmp/eval-crud.txt` on branch `feature/eval`, read it, update its content, then delete it from the same branch.",
		...
		baseDestructiveEvalCase("MS-019", "Exercise pipeline trigger CRUD in project `my-org/tools/gitlab-mcp-server`: create trigger `eval-crud-trigger`, fetch it with trigger get using the returned trigger ID, update the description, then delete it.",
		...
		baseDestructiveEvalCase("MS-020", "Exercise pipeline schedule CRUD in project `my-org/tools/gitlab-mcp-server`: create inactive schedule `eval-crud-schedule` on `main`, get it, update its cron, create variable `SCHEDULE_CRUD_TOKEN`, update that variable, delete the variable, then delete the schedule.",
		...
		baseDestructiveEvalCase("MS-021", "Exercise project webhook CRUD in project `my-org/tools/gitlab-mcp-server`: add webhook `https://example.com/eval-crud-hook`, fetch it with hook get using the returned hook ID, edit it to disable SSL verification, then delete it.",
		...
		baseDestructiveEvalCase("MS-022", "Exercise project badge CRUD in project `my-org/tools/gitlab-mcp-server`: add badge `eval-crud-badge`, fetch it with badge get using the returned badge ID, edit the badge name to `Evaluation CRUD badge link`, then delete it.",
		...
		baseDestructiveEvalCase("MS-023", "Exercise wiki CRUD in project `my-org/tools/gitlab-mcp-server`: create wiki page titled `Evaluation CRUD wiki` with content containing `eval-crud-wiki`, fetch the created page with the returned slug, update its title to `Evaluation CRUD wiki v2`, then delete it.",
		...
		baseDestructiveEvalCase("MS-024", "Exercise project snippet CRUD in project `my-org/tools/gitlab-mcp-server`: create project snippet `eval-crud-snippet` titled `Evaluation CRUD snippet`, fetch it with project snippet get using the returned snippet ID, update its content with a `files` entry using action `update` and `file_path` set to the returned file path, not `previous_path`, then delete it.",
		...
		baseDestructiveEvalCase("MS-025", "Exercise scoped project CI variable CRUD in project `my-org/tools/gitlab-mcp-server`: create variable `EVAL_CRUD_TOKEN` with value `crud-value-1` and environment scope `review/eval`, list variables, update the scoped variable to value `crud-value-2`, then delete that same scoped variable.",
		...
		baseDestructiveEvalCase("MS-026", "Exercise scoped group CI variable CRUD in group `my-org`: create variable `GROUP_EVAL_CRUD_TOKEN` with value `group-crud-value-1` and environment scope `review/eval`, get it using top-level `environment_scope`, update it to value `group-crud-value-2`, then delete that same scoped variable.",
		...
		baseDestructiveEvalCase("MS-027", "Exercise merge request note CRUD in project `my-org/tools/gitlab-mcp-server`: add note `eval-mr-note` to MR `7`, fetch the created note using the returned note ID, update it to `eval-mr-note-updated`, then delete it.",
		...
		baseDestructiveEvalCase("MS-028", "Exercise branch protection lifecycle in project `my-org/tools/gitlab-mcp-server`: create branch `eval-protect-branch` from `main`, protect it with Maintainer push and merge access, fetch the protected branch, update it to allow force push, unprotect it, then delete the branch.",
		...
		baseDestructiveEvalCase("MS-030", "Exercise project deploy token lifecycle in project `my-org/tools/gitlab-mcp-server`: create deploy token `eval-deploy-token` with scope `read_repository`, fetch it with the returned deploy token ID, list project deploy tokens, then delete that deploy token.",
		...
		baseDestructiveEvalCase("MS-031", "Exercise project deploy key lifecycle in project `my-org/tools/gitlab-mcp-server`: add deploy key `eval-deploy-key` with public key `ssh-rsa AAAAevalcrud`, fetch it with deploy key get using the returned deploy key ID, update the title to `eval-deploy-key-updated`, then delete it.",
		...
		baseDestructiveEvalCase("MS-032", "Exercise issue time tracking in project `my-org/tools/gitlab-mcp-server`: create issue `eval-time-issue`, set estimate `2h`, add spent time `30m` with summary `pairing`, reset spent time, reset the estimate, then delete the issue.",
			readStep("gitlab_issue", "create", params("project_id", "title"), params("description")),
		...
		baseDestructiveEvalCase("MS-033", "Exercise merge request time tracking and emoji in project `my-org/tools/gitlab-mcp-server`: set estimate `1h` on MR `7`, add spent time `15m`, add award emoji `eyes`, list MR awards, delete the returned award emoji, reset spent time, then reset the estimate.",
		...
		baseDestructiveEvalCase("MS-034", "Exercise project member lifecycle in project `my-org/tools/gitlab-mcp-server`: add user ID `55` as Reporter, fetch that project member, edit access level to Developer, then remove the member.",
		...
		baseDestructiveEvalCase("MS-035", "Exercise group label lifecycle in group `my-org`: create label `eval-group-label` with color `#1f75cb`, fetch it by label ID or name, rename it to `eval-group-label-v2`, then delete it.",
		...
		baseDestructiveEvalCase("MS-036", "Exercise group milestone lifecycle in group `my-org`: create milestone `Evaluation Group Milestone` with due date `2026-12-31`, fetch it using the returned milestone IID, update title to `Evaluation Group Milestone v2`, then delete it.",
...
func baseDestructiveEvalCase(id, prompt string, steps ...ExpectedStep) EvalCase {
	evalCase := EvalCase{
		ID:          EvalCaseID(id),
		Prompt:      prompt,
		Steps:       steps,
		Edition:     EvalCaseEdition(editionCE),
		Presets:     []EvalPreset{EvalPreset(presetDockerDestructiveSafe)},
		Partition:   EvalPartition(partitionBaseDestructive),
		Mutating:    true,
		Destructive: true,
...
func destructiveStep(tool, action string, requiredParams, optionalParams []string) ExpectedStep {
...
func baseDestructiveEarlySinglePromptTemplateAndFixtures(id string) (string, []CaseFixtureSpec) {
	...
	case "MT-024":
		return "Delete artifacts for job `{{ .Job.ID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{FailedJobArtifactFixture}
	case "MT-028":
	...
	case "MT-037":
		return "Delete release `{{ .Release.TagName }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{ReleaseDeleteFixture}
	case "MT-042":
		return "Revoke project access token ID `{{ .Token.ID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{ProjectAccessTokenRevokeFixture}
	case "MT-044":
		return "Delete package ID `{{ .Package.ID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{PackageDeleteFixture}
	case "MT-047":
		return "Remove runner ID `{{ .Runner.ID }}`.", []CaseFixtureSpec{RunnerRemoveFixture}
	case "MT-049":
		return "Stop environment ID `{{ .Environment.ID }}` named `{{ .Environment.Name }}` in project `{{ .Project.Path }}`, forcing the stop if needed.", []CaseFixtureSpec{EnvironmentStopFixture}
	case "MT-051":
		return "Delete personal snippet ID `{{ .Values.snippet_id }}`.", []CaseFixtureSpec{SnippetDeleteFixture}
	case "MT-054":
		return "Delete broadcast message ID `{{ .Values.id }}`.", []CaseFixtureSpec{BroadcastMessageDeleteFixture}
	case "MT-055":
		return "Archive project `{{ .Project.Path }}`.", []CaseFixtureSpec{ProjectArchiveFixture}
	case "MT-057":
		return "Delete webhook ID `{{ .Values.hook_id }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{ProjectHookDeleteFixture}
	case "MT-059":
		return "Delete badge ID `{{ .Values.badge_id }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{ProjectBadgeDeleteFixture}
	case "MT-063":
		return "Publish all draft review notes for MR `{{ .MergeRequest.IID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{DraftNotePublishAllFixture}
	case "MT-066":
...
func baseDestructiveLateSinglePromptTemplateAndFixtures(id string) (string, []CaseFixtureSpec) {
	...
	case "MT-100":
		return "Delete tag `{{ .Tag.Name }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{TagDeleteFixture}
	case "MT-101":
		return "Permanently delete pipeline `{{ .Pipeline.ID }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{PipelineDeleteFixture}
	case "MT-102":
		return "Delete pipeline trigger token ID `{{ .Values.pipeline_trigger_id }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{PipelineTriggerDeleteFixture}
	case "MT-103":
		return "Delete pipeline schedule ID `{{ .Values.pipeline_schedule_id }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{PipelineScheduleDeleteFixture}
	case "MT-104":
		return "Block user ID `{{ .Values.user_id }}`.", []CaseFixtureSpec{UserBlockFixture}
	case "MT-106":
		return "Delete feature flag `{{ .Values.feature_flag_name }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{FeatureFlagDeleteFixture}
	case "MT-108":
		return "Delete wiki page `{{ .Values.wiki_slug }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{WikiDeleteFixture}
	case "MT-109":
		return "Remove award emoji ID `{{ .Award.ID }}` from merge request `{{ .MergeRequest.IID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{MergeRequestAwardEmojiFixture}
	case "MT-110":
		return "Remove award emoji ID `{{ .Award.ID }}` from issue `{{ .Issue.IID }}` in project `{{ .Project.Path }}`.", []CaseFixtureSpec{IssueAwardEmojiFixture}
	case "MT-111":
		return "Delete deploy key ID `{{ .DeployKey.ID }}` from project `{{ .Project.Path }}`.", []CaseFixtureSpec{DeployKeyDeleteFixture}
	case "MT-112":
...
func baseDestructiveWorkflowPromptTemplateAndFixtures(id string) (string, []CaseFixtureSpec) {
	...
	case "MS-003":
		return "Prepare a batch review for MR `{{ .MergeRequest.IID }}` in project `{{ .Project.Path }}`: inspect the MR, inspect changes, create a draft note saying `Please add a regression test`, then publish all draft notes.", []CaseFixtureSpec{MergeRequestFixture}
	case "MS-004":
