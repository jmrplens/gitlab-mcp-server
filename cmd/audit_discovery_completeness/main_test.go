...
func TestSeverityFor_OnlyEscalatesInNonCRUDClusters(t *testing.T) {
	...
	for _, flag := range []string{"weak_aliases", "empty_related", "weak_individual_description"} {
		if got := severityFor(flag, true, pureCRUD); got != "warning" {
	...
	for _, flag := range []string{"weak_aliases", "empty_related", "weak_individual_description"} {
		if got := severityFor(flag, true, withBatch); got != "error" {
	...
	for _, flag := range []string{"weak_aliases", "empty_related", "weak_individual_description"} {
		if got := severityFor(flag, false, nil); got != "warning" {
	...
	if got := severityFor("generic_usage", false, nil); got != "error" {
		t.Errorf("severityFor(generic_usage) = %q, want error", got)
	}
	if got := severityFor("missing_disambiguation", false, nil); got != "error" {
