...
func writeLLMSFullResources(b *strings.Builder, catalog llmsCatalog, resourceCount int) {
	b.WriteString("## Resources\n\n")
	fmt.Fprintf(b, "%d resources providing read-only access to GitLab data. 26 resource kinds (pipelines, jobs, issues, merge requests, releases, files, wikis, single-parent lists, and more) are subscribable — subscriptions/listen on protocol 2026-07-28, resources/subscribe on stateful sessions: the server polls them and sends notifications/resources/updated on change.\n\n", resourceCount)
