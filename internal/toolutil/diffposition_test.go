...
func TestParseHunkHeader_Standard(t *testing.T) {
	old, new_ := parseHunkHeader("@@ -10,5 +20,8 @@ func foo()")
	if old != 10 {
		t.Errorf("old = %d, want 10", old)
	}
	if new_ != 20 {
		t.Errorf("new = %d, want 20", new_)
	}
}
...
func TestParseHunkHeader_NoCount(t *testing.T) {
	old, new_ := parseHunkHeader("@@ -1 +1 @@")
	if old != 1 {
		t.Errorf("old = %d, want 1", old)
	}
	if new_ != 1 {
		t.Errorf("new = %d, want 1", new_)
	}
}
...
func TestParseHunkHeader_Invalid(t *testing.T) {
	old, new_ := parseHunkHeader("not a hunk header")
	if old != 0 || new_ != 0 {
		t.Errorf("invalid header should return (0,0), got (%d,%d)", old, new_)
	}
}
