...
// webpFS embeds the lossless WebP fallbacks generated by
// cmd/gen_icon_webp from the SVG constants below. Regenerate with
// `go run ./cmd/gen_icon_webp/` after adding or editing an icon.
//
...
// Each icon uses currentColor for theme compatibility, so its SVG entry
// serves every client theme without declaring mcp.Icon.Theme. Clients that
// reject image/svg+xml (VS Code Copilot's MIME allowlist, for example) fall
// back to the accompanying light/dark WebP entries instead.
const (
...
// [mcp.Icon] slice with base64-encoded data URIs. Base64 encoding is
// required because raw SVG markup contains characters (<, >, ", spaces, #,
// {, }) that are not valid in an unencoded RFC 2397 data URI. The MCP spec
// also documents base64 as the canonical form for embedded image data.
//
// name identifies the WebP pair embedded at
// icons/webp/<name>-light.webp and icons/webp/<name>-dark.webp (generated
// by cmd/gen_icon_webp). The slice has three entries:
//
...
//     background.
//
// Clients that reject image/svg+xml but implement Icon.Theme selection
// (VS Code Copilot, for example) fall back to the matching WebP entry
// instead of losing the icon entirely.
...
// Domain icons — each returns a three-element []mcp.Icon (SVG + light/dark
// WebP fallback) ready for the Icons field.
var (
