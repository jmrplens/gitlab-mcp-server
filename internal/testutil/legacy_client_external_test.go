...
// TestLegacyClientHandshake verifies what this helper exists to provide: a
// session negotiated at 2025-11-25, advertising elicitation, that answers a
// server-initiated elicitation/create synchronously.
//
// The protocol version is the point. From 2026-07-28 SEP-2322 forbids
// server-initiated requests during a tool call, and the SDK client always
// negotiates the newest version it knows, so a test of the synchronous path
// needs a client that pins an older one. If this handshake ever came back at
// another version, every test of that path would be exercising the wrong
// transport while still passing.
