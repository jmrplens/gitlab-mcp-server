...
// collectMetric looks for an instrument in one collection of the reader.
//
// Separate from the polling below so that both waiting policies here ask the
// same question of the same reader: a test that waits for a measurement and a
// test that proves one was never taken must agree on what "recorded" means.
...
//
// Only the tests that expect a measurement poll, and each of them returns on
// the first or second collection. The test that expects none cannot be written
// this way at all: waiting for something that must never happen spends the
// whole deadline on every green run and still only makes the false pass less
// likely. It uses synctest instead, which answers the question exactly.
func awaitMetric(t *testing.T, reader *metric.ManualReader, name string) (metricdata.Metrics, bool) {
	t.Helper()

	deadline := time.Now().Add(3 * time.Second)
	for {
		if m, found := collectMetric(t, reader, name); found {
			return m, true
		}
...
// actually fires.
//
// It collects once, after synctest.Wait, which also makes it the control for
// the skip test below: that test proves an absence from a single collection,
// and an absence is only a proof if a measurement that had been taken would
// already be there. This is the case where one is taken, so it fails if
// synctest.Wait ever stops outlasting the recording goroutine.
func TestSessionDuration_IsRecordedOnStdio(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		recorded, ok := collectMetric(t, reader, "mcp.server.session.duration")
		if !ok {
...
//
// # Why synctest rather than a Collect or a poll
//
// The regression this exists to catch is the one that starts the goroutine, and
// that goroutine records after Wait unblocks, so a single Collect could run
// before the wrong measurement and report success. Polling for the absence is
// no better in kind: it spends its whole deadline on every green run and still
// only makes the false pass less likely, never impossible.
//
// synctest.Wait answers exactly the question being asked. It returns when every
// other goroutine in the bubble has exited or is durably blocked, so a
// recording goroutine that a regression started has already run to completion
// by the time the reader is collected. The in-memory transports are net.Pipe,
// whose blocking is on bubbled channels rather than on real I/O, which is what
// makes them usable here where an httptest server would not be. That the
// ordering really holds is asserted rather than assumed: it is what
// TestSessionDuration_IsRecordedOnStdio above collects once to show.
func TestSessionDuration_IsSkippedForAStatelessPost(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		reader, restore := newMetricRecorder(t)
		defer restore()

		// TransportTCP with an in-memory session, which carries no session id:
		// the same shape a stateless POST presents to the middleware.
		driveSession(t, TransportTCP)

		synctest.Wait()

		if _, found := collectMetric(t, reader, "mcp.server.session.duration"); found {
