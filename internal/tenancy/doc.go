...
// server binary already links, and it declares no package-level variable: every
// table is built by the function that returns it, on each call. So importing it
// adds no initialization work to a binary, a function nothing reachable calls
// is dropped by the linker, and a layer that replaces a literal with one of the
