...
// that is that the capability is missing, not that the cross-link is wrong.
// Every commit action is a route on the gitlab_repository group, so the domain
// is "repository" and not "commit"; the specs here published "commit.get" and
// its thirty-odd siblings, which read as perfectly plausible pairs and resolve
// to nothing.
func TestPublishedActionIDs_NameActionsTheCatalogHolds(t *testing.T) {
	t.Parallel()

	catalog := catalogForTest(t)
	ids := commits.PublishedActionIDs()
	if len(ids) == 0 {
...
// block, is caught rather than silently exempted. That is exactly how this
// package drifted: the formatters read a correct block and the specs carried
// their own literals.
func TestActionSpecs_RelatedActionsNameActionsTheCatalogHolds(t *testing.T) {
	t.Parallel()

	catalog := catalogForTest(t)
	specs := commits.ActionSpecs(nil)
	if len(specs) == 0 {
