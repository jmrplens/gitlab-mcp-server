...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "List all comments (notes) on a GitLab group epic. Supports ordering by created_at or updated_at, sort direction, and pagination.\n\nReturns: JSON with notes array including body, author, timestamps, and system flags. See also: gitlab_epic_get.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get a single comment (note) from a GitLab group epic by its note ID, including author, timestamps, body, and system flag.\n\nReturns: JSON with note details including ID, body, author, and timestamps. See also: gitlab_epic_note_list.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconEpic,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Edit the body text of an existing comment on a GitLab group epic. Only the note author or a group maintainer can update a note.\n\nReturns: JSON with updated note including ID, body, author, and timestamps. See also: gitlab_epic_note_get.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Permanently delete a comment from a GitLab group epic. Only the note author or a group maintainer can delete a note.\n\nReturns: JSON with deletion confirmation. See also: gitlab_epic_note_get.",
		Annotations: toolutil.DeleteAnnotations,
		Icons:       toolutil.IconEpic,
	}, func(ctx context.Context, req *mcp.CallToolRequest, input DeleteInput) (*mcp.CallToolResult, toolutil.DeleteOutput, error) {
		if r := toolutil.ConfirmAction(ctx, req, fmt.Sprintf("Delete note %d from epic &%d in group %q?", input.NoteID, input.EpicIID, input.GroupID)); r != nil {
		...
		return toolutil.DeleteResult(fmt.Sprintf("note %d from epic &%d in group %s", input.NoteID, input.EpicIID, input.GroupID))
	})
}
