// issue_statistics_catalog_test.go judges the canonical action IDs this
// package publishes against the catalog the server really registers.
//
// It is an external test package because the catalog is assembled in
// internal/tools, which imports this one: package issuestatistics could not
// reach the oracle without an import cycle. The test exists because nothing
// here did: six related actions named the owner package as their domain
// ("issuestatistics.statistics_get_group") where the catalog aggregates these
// specs into gitlab_issue and registers "issue.statistics_get_group", so a
// model told to compare the project counts with the group's was sent to an
// action no surface holds.
package issuestatistics_test
...
// the related actions its specs carry, and the ID its statistics card quotes
// in a result hint.
//
// The hint ID is read back out of rendered text rather than compared with the
// constant it was built from, because what a model acts on is the served
// string: a hint built from the right constant and one built from a stale copy
// of it are indistinguishable at the call site.
func publishedActionIDs(t *testing.T) []publishedActionID {
	t.Helper()
	specs := issuestatistics.ActionSpecs(testutil.NewTestClient(t, testutil.ForbiddenHandler(t)))
	published := relatedActionIDs(specs)
	return append(published, hintedActionIDs(issuestatistics.FormatMarkdown(issuestatistics.StatisticsOutput{}))...)
}
