...
// name becomes: the catalog group a package's actions are projected into is
// decided by the aggregation in internal/tools/action_specs.go, and the IDs
// this package cross-links to reach beyond its own group. That is where it
// went wrong: a job carries the commit it ran on, and the get action offered
// "commit.get" for reading it, which is a plausible pair and no action at all
// (commits are routes on gitlab_repository, so the ID is
...
func TestPublishedActionIDs_ResolveInTheActionCatalog(t *testing.T) {
	...
	t.Run("every ID in the canonical block resolves", func(t *testing.T) {
		for _, id := range jobs.PublishedActionIDs() {
	...
	t.Run("every RelatedActions entry resolves", func(t *testing.T) {
		for _, spec := range jobs.ActionSpecs(nil) {
