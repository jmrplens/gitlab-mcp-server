...
func List(ctx context.Context, client *gitlabclient.Client, input ListInput) (ListOutput, error) {
	if input.ControllerID <= 0 {
		return ListOutput{}, errors.New("controller_id is required and must be > 0")
	}
...
func Get(ctx context.Context, client *gitlabclient.Client, input GetInput) (Output, error) {
	if input.ControllerID <= 0 {
		return Output{}, errors.New("controller_id is required and must be > 0")
	}
	if input.TokenID <= 0 {
		return Output{}, errors.New("token_id is required and must be > 0")
	}
	if err := ctx.Err(); err != nil {
...
func Revoke(ctx context.Context, client *gitlabclient.Client, input RevokeInput) error {
	...
	if input.TokenID <= 0 {
		return errors.New("token_id is required and must be > 0")
	}
	if err := ctx.Err(); err != nil {
