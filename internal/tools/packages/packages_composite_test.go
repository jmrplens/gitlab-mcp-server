...
func TestPackagePublishDirectory_WithProgressToken(t *testing.T) {
	...
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "artifact.bin"), []byte("progress-data"), 0600); err != nil {
		t.Fatalf("write file: %v", err)
	}
...
func TestPackagePublishDirectory_NoMatchingFiles(t *testing.T) {
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "readme.md"), []byte("text"), 0600)
...
func TestPackagePublishDirectory_NotADirectory(t *testing.T) {
	...
	os.WriteFile(tmpFile, []byte("file"), 0600)
...
func TestPackagePublishDirectory_InvalidGlobPattern(t *testing.T) {
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "a.txt"), []byte("data"), 0600)
...
func TestPackagePublishDirectory_ContextCancelled(t *testing.T) {
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "a.bin"), []byte("data"), 0600)
...
func TestPackagePublishDirectory_SkipsSubdirectories(t *testing.T) {
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "file.bin"), []byte("content"), 0600)
	os.MkdirAll(filepath.Join(dir, "subdir"), 0750)
	os.WriteFile(filepath.Join(dir, "subdir", "nested.bin"), []byte("nested"), 0600)
...
func TestPackagePublishDirectory_SkipsSymlinks(t *testing.T) {
	...
	os.WriteFile(regular, []byte("real"), 0600)
