...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	...
	packageTool := func(name, description string) *mcp.Tool {
	...
	mcp.AddTool(server, packageTool("gitlab_package_download", "Download a file from the GitLab Generic Package Registry and save it to a local path. Returns the output path, file size, and SHA256 checksum.\n\nSee also: gitlab_package_list, gitlab_package_publish\n\nReturns: JSON with output path, file size, and SHA256 checksum."), func(ctx context.Context, req *mcp.CallToolRequest, input DownloadInput) (*mcp.CallToolResult, DownloadOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, packageTool("gitlab_package_list", "List packages in a GitLab project. Can filter by name, version, type, and supports pagination and sorting.\n\nSee also: gitlab_package_file_list, gitlab_package_publish\n\nReturns: JSON array of packages with pagination."), func(ctx context.Context, req *mcp.CallToolRequest, input ListInput) (*mcp.CallToolResult, ListOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, packageTool("gitlab_package_file_list", "List files within a specific package. Returns file ID, name, size, and SHA256 for each file with pagination.\n\nSee also: gitlab_package_list, gitlab_package_download\n\nReturns: JSON array of package files with pagination."), func(ctx context.Context, req *mcp.CallToolRequest, input FileListInput) (*mcp.CallToolResult, FileListOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, packageTool("gitlab_package_file_delete", "Delete a single file from a package in the GitLab Package Registry. This action cannot be undone.\n\nSee also: gitlab_package_file_list, gitlab_package_delete\n\nReturns: confirmation message."), func(ctx context.Context, req *mcp.CallToolRequest, input FileDeleteInput) (*mcp.CallToolResult, toolutil.DeleteOutput, error) {
