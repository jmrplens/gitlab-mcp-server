...
type ApproveProjectInput struct {
	...
	AccessLevel int                  `json:"access_level,omitempty" jsonschema:"Access level to grant (10=Guest, 20=Reporter, 30=Developer, 40=Maintainer, 25=Security Manager where supported)"`
}
...
func ApproveProject(ctx context.Context, client *gitlabclient.Client, input ApproveProjectInput) (Output, error) {
	...
	if err != nil {
		return Output{}, toolutil.WrapErrWithStatusHint("access_request_approve_project", err, http.StatusNotFound,
			"verify user_id with gitlab_access_request_list_project; access_level must be valid (10=Guest, 20=Reporter, 30=Developer, 40=Maintainer, 25=Security Manager where supported); approving requires Maintainer role")
	}
	return convertAccessRequest(ar), nil
}
