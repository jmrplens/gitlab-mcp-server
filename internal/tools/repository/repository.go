...
func Contributors(ctx context.Context, client *gitlabclient.Client, input ContributorsInput) (ContributorsOutput, error) {
	...
	if err != nil {
		return ContributorsOutput{}, toolutil.WrapErrWithStatusHint("repositoryContributors", err, http.StatusNotFound,
			"verify project_id with project.get; the repository may be empty")
	}
...
func Blob(ctx context.Context, client *gitlabclient.Client, input BlobInput) (BlobOutput, error) {
	...
	if err != nil {
		return BlobOutput{}, toolutil.WrapErrWithStatusHint("repositoryBlob", err, http.StatusNotFound,
			"verify the blob SHA with repository.tree; SHAs must be full 40-character commit/blob hashes")
	}
...
// RawBlobContent retrieves the raw content of a git blob by SHA.
// For text content, returns decoded text. For images, returns ImageContent.
// For other binary formats, returns metadata only.
func RawBlobContent(ctx context.Context, client *gitlabclient.Client, input BlobInput) (RawBlobContentOutput, error) {
	...
	if err != nil {
		return RawBlobContentOutput{}, toolutil.WrapErrWithStatusHint("repositoryRawBlobContent", err, http.StatusNotFound,
			"verify the blob SHA with repository.tree; SHAs must be full 40-character blob hashes")
	}
...
// classifyBlobContent categorizes blob bytes based on sniffed MIME type.
func classifyBlobContent(data []byte, mime string) (category, content string, imageData []byte, imageMIME string) {
	switch {
...
func GenerateChangelogData(ctx context.Context, client *gitlabclient.Client, input GenerateChangelogInput) (ChangelogDataOutput, error) {
	...
	if err != nil {
		return ChangelogDataOutput{}, toolutil.WrapErrWithStatusHint("generateChangelogData", err, http.StatusUnprocessableEntity,
			"verify version follows semver (e.g. 1.0.0) and 'from'/'to' refs exist; this is a read-only preview. Use repository.changelog_add to commit")
	}
