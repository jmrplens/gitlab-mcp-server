...
func ListContributionEvents(ctx context.Context, client *gitlabclient.Client, input ListContributionEventsInput) (ContributionEventsOutput, error) {
	...
	if input.Before != "" {
		if t, err := time.Parse("2006-01-02", input.Before); err == nil {
			d := gl.ISOTime(t)
			opts.Before = &d
		}
	}
	if input.After != "" {
		if t, err := time.Parse("2006-01-02", input.After); err == nil {
			d := gl.ISOTime(t)
			opts.After = &d
		}
	}
...
func toOutput(u *gl.User) Output {
	...
	if u.LastActivityOn != nil {
		out.LastActivityOn = time.Time(*u.LastActivityOn).Format("2006-01-02")
	}
