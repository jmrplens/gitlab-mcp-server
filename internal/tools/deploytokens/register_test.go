// register_test.go contains integration tests for the deploy token tool closures
...
// TestRegisterTools_DeleteErrors verifies that the delete handler closures in register.go
// return error results when the GitLab API fails, covering the if-err branches after
// project and group deploy token deletion.
...
// branches in both gitlab_deploy_token_delete_project and
// gitlab_deploy_token_delete_group when the user declines.
