// keys_catalog_test.go judges the canonical action IDs this package publishes
// against the catalog the server really registers.
//
// It is an external test package because the catalog is assembled in
// internal/tools, which imports this one: package keys could not reach the
// oracle without an import cycle. The test exists because nothing here did:
// the result hint on the key card named "keys.key_get_by_fingerprint", the
// owner package's name used as a domain, where the catalog aggregates these
// specs into gitlab_user and registers "user.key_get_by_fingerprint". A model
// that followed the hint was answered "unknown action" by every surface.
package keys_test
...
// the related actions its specs carry, and the ID its key card quotes in a
// result hint.
//
...
// of it are indistinguishable at the call site, and this package shipped the
// stale copy.
func publishedActionIDs(t *testing.T) []publishedActionID {
	t.Helper()
	specs := keys.ActionSpecs(testutil.NewTestClient(t, testutil.ForbiddenHandler(t)))
	published := relatedActionIDs(specs)
	return append(published, hintedActionIDs(keys.FormatMarkdownString(keys.Output{}))...)
}
