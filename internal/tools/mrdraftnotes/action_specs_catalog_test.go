...
// internal/tools, which imports this one: package mrdraftnotes could not reach
// the oracle without an import cycle. The test exists because nothing here
// did, and this package is the clearest case of what that cost: markdown.go
// kept its own block of IDs spelling the domain "mr_review", which is right,
// while action_specs.go spelled it "mrdraftnotes", which no surface has ever
// registered, and all nineteen related actions were dead ends. Both files now
// read one block, and this test judges what that block says against the
// catalog rather than against itself.
package mrdraftnotes_test
...
// the related actions its specs carry, and the IDs its two formatters quote in
// result hints.
//
...
// served string. The list is rendered with one draft note in it: a list
// formatter answers an empty page with a message and returns before the footer
// its hints are written in.
func publishedActionIDs(t *testing.T) []publishedActionID {
	t.Helper()
	specs := mrdraftnotes.ActionSpecs(testutil.NewTestClient(t, testutil.ForbiddenHandler(t)))
	published := relatedActionIDs(specs)
	rendered := []string{
		mrdraftnotes.FormatOutputMarkdown(mrdraftnotes.Output{}),
		mrdraftnotes.FormatListMarkdown(mrdraftnotes.ListOutput{DraftNotes: []mrdraftnotes.Output{{}}}),
	}
	return append(published, hintedActionIDs(rendered...)...)
}
