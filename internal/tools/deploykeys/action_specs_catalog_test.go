...
//
// What it guards: deploy key actions are aggregated into the access group, and
// the two related actions this package spelled by hand carried that prefix
// while the three on deploy_key_list_all were bare spec names with no domain
// at all, so a model following one was answered "unknown action". The two
// spellings sat in the same file and nothing compared either with the catalog.
func TestActionSpecs_EveryRelatedActionID_ResolvesInTheCatalog(t *testing.T) {
	t.Parallel()

	ids := catalogIDs(t)
	specs := deploykeys.ActionSpecs(nil)
	if len(specs) == 0 {
...
// under some ID, which is what makes the access.* prefix the related lists
...
func TestActionSpecs_EverySpecName_ReachesTheCatalog(t *testing.T) {
	...
	for _, spec := range deploykeys.ActionSpecs(nil) {
