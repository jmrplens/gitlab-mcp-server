...
func toOutput(t *gl.ImpersonationToken) Output {
	...
	if t.ExpiresAt != nil {
		o.ExpiresAt = time.Time(*t.ExpiresAt).Format("2006-01-02")
	}
	if t.LastUsedAt != nil {
...
func Revoke(ctx context.Context, client *gitlabclient.Client, input RevokeInput) (RevokeOutput, error) {
	if input.UserID <= 0 {
		return RevokeOutput{}, errors.New("user_id must be a positive integer")
	}
	if input.TokenID <= 0 {
...
func CreatePAT(ctx context.Context, client *gitlabclient.Client, input CreatePATInput) (PATOutput, error) {
	if input.UserID <= 0 {
		return PATOutput{}, errors.New("user_id must be a positive integer")
	}
	if input.Name == "" {
