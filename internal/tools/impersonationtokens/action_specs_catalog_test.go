...
package impersonationtokens_test
...
//
// What it guards: these actions are aggregated into the user group, so their
// IDs read user.*. The package published its own package name as the domain,
// impersonationtokens.*, which the catalog has no domain for, so every one of
// them answered a model "unknown action".
func TestActionSpecs_EveryRelatedActionID_ResolvesInTheCatalog(t *testing.T) {
	t.Parallel()

	ids := catalogIDs(t)
	specs := impersonationtokens.ActionSpecs(nil)
	if len(specs) == 0 {
...
// under some ID, which is what makes the user.* prefix the related lists carry
// a read of the catalog rather than a guess at it.
func TestActionSpecs_EverySpecName_ReachesTheCatalog(t *testing.T) {
	...
	for _, spec := range impersonationtokens.ActionSpecs(nil) {
...
//
// They are asserted apart from the specs because the hints and the spec
// metadata were two sets of the same strings written in two files, and both
// carried the impersonationtokens.* spelling. The count check at the end is
// what stops this passing by matching nothing, which is how it would read if
// the hint form ever changed.
func TestMarkdown_EveryHintedActionID_ResolvesInTheCatalog(t *testing.T) {
	t.Parallel()

	ids := catalogIDs(t)
	rendered := map[string]string{
		"FormatListMarkdownString": impersonationtokens.FormatListMarkdownString(impersonationtokens.ListOutput{
			Tokens: []impersonationtokens.Output{{ID: 1, Name: "ci"}},
		}),
		"FormatMarkdownString":       impersonationtokens.FormatMarkdownString(impersonationtokens.Output{ID: 1, Name: "ci"}),
		"FormatPATMarkdownString":    impersonationtokens.FormatPATMarkdownString(impersonationtokens.PATOutput{}),
		"FormatRevokeMarkdownString": impersonationtokens.FormatRevokeMarkdownString(impersonationtokens.RevokeOutput{UserID: 1, TokenID: 1}),
	}
