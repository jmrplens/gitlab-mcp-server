...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	...
	customEmojiTool := func(name, description string) *mcp.Tool {
	...
	mcp.AddTool(server, customEmojiTool("gitlab_list_custom_emoji", "List custom emoji for a GitLab group via GraphQL API. Returns: paginated list with ID, name, URL, and created date. See also: gitlab_create_custom_emoji."), func(ctx context.Context, req *mcp.CallToolRequest, input ListInput) (*mcp.CallToolResult, ListOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, customEmojiTool("gitlab_create_custom_emoji", "Create a custom emoji in a GitLab group via GraphQL API. Requires group path, emoji name (without colons), and image URL. Returns: created emoji with ID, name, URL, and created date. See also: gitlab_list_custom_emoji, gitlab_delete_custom_emoji."), func(ctx context.Context, req *mcp.CallToolRequest, input CreateInput) (*mcp.CallToolResult, CreateOutput, error) {
		start := time.Now()
