...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Create a new remote push mirror for a GitLab project. This push-mirror endpoint may use URL-embedded credentials when GitLab requires inline authentication; treat embedded credentials as secrets and redact them from logs, telemetry, and errors. Pull mirroring uses separate auth_user/auth_password fields and must not put credentials in the URL.\n\nReturns: JSON with created mirror details. See also: gitlab_edit_project_mirror.",
