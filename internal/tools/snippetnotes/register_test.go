// register_test.go contains integration tests for the snippet note tool
...
// branch in the gitlab_snippet_note_delete handler when the user declines.
func TestRegisterTools_DeleteConfirmDeclined(t *testing.T) {
	...
	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name:      "gitlab_snippet_note_delete",
		Arguments: map[string]any{"project_id": "p", "snippet_id": float64(1), "note_id": float64(1)},
	})
	if err != nil {
		t.Fatalf("CallTool error: %v", err)
	}
