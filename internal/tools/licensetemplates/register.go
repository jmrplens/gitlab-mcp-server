...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	...
	mcp.AddTool(server, templateTool("gitlab_list_license_templates", "List all available open-source license templates. Optionally filter by popular.\n\nReturns: JSON array of license templates with pagination.\n\nSee also: gitlab_get_license_template, gitlab_list_gitignore_templates"), func(ctx context.Context, req *mcp.CallToolRequest, input ListInput) (*mcp.CallToolResult, ListOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, templateTool("gitlab_get_license_template", "Get a single license template by key. Optionally substitute project name and full name.\n\nReturns: JSON with the license template content.\n\nSee also: gitlab_list_license_templates, gitlab_list_project_templates"), func(ctx context.Context, req *mcp.CallToolRequest, input GetInput) (*mcp.CallToolResult, GetOutput, error) {
		start := time.Now()
