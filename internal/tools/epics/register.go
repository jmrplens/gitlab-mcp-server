...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "List epics for a GitLab group. Supports filtering by state, labels, author, search text, and pagination.\n\nReturns: JSON with epics array and pagination metadata. See also: gitlab_epic_get.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get a single group epic by its IID, including title, description, state, labels, dates, and author.\n\nReturns: JSON with epic details. See also: gitlab_epic_list, gitlab_epic_get_links.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconEpic,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get all child epics of a parent epic. Returns the list of sub-epics linked to the specified epic.\n\nReturns: JSON with child_epics array. See also: gitlab_epic_get.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconEpic,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Create a new epic in a GitLab group. Supports title, description, labels, confidentiality, parent epic, and color.\n\nReturns: JSON with created epic details. See also: gitlab_epic_get.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Update an existing group epic. Can modify title, description, labels, state (close/reopen), confidentiality, parent, and color.\n\nReturns: JSON with updated epic details. See also: gitlab_epic_get.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Permanently delete an epic from a GitLab group.\n\nReturns: JSON with deletion confirmation. See also: gitlab_epic_get.",
		Annotations: toolutil.DeleteAnnotations,
		Icons:       toolutil.IconEpic,
	}, func(ctx context.Context, req *mcp.CallToolRequest, input DeleteInput) (*mcp.CallToolResult, toolutil.DeleteOutput, error) {
		if r := toolutil.ConfirmAction(ctx, req, fmt.Sprintf("Delete epic &%d from group %q?", input.EpicIID, input.GroupID)); r != nil {
		...
		return toolutil.DeleteResult(fmt.Sprintf("epic &%d from group %s", input.EpicIID, input.GroupID))
	})
}
