...
//
// It is an external test package (freezeperiods_test) on purpose. The oracle is
...
const (
	...
	outputPkgPath = "github.com/jmrplens/gitlab-mcp-server/v3/internal/tools/freezeperiods"
)
...
// TestFreezePeriodRelatedActions_EveryPublishedID_ResolvesInTheCatalog walks
// the RelatedActions of every freeze period action as the catalog itself
// materialized them, and checks each names an action a model could go on to
// call.
//
// The deploy-freeze neighbors are the deployment routes, and those are
// actions of the environment group rather than of a deployment group of their
// own, which is the mistake this catches.
func TestFreezePeriodRelatedActions_EveryPublishedID_ResolvesInTheCatalog(t *testing.T) {
...
// TestFreezePeriodHints_EveryPublishedID_ResolvesInTheCatalog renders every
// Markdown formatter this package registers with a populated fixture and checks
// each action ID the guidance invites a model to call.
