...
//
// It is an external test package (milestones_test) on purpose. The oracle is
...
const (
	...
	outputPkgPath = "github.com/jmrplens/gitlab-mcp-server/v3/internal/tools/milestones"
)
...
// TestMilestoneRelatedActions_EveryPublishedID_ResolvesInTheCatalog walks the
// RelatedActions of every milestone action as the catalog itself materialized
// them, and checks each names an action a model could go on to call.
//
// It reads the catalog rather than ActionSpecs so that what gets judged is what
// the projection actually publishes.
func TestMilestoneRelatedActions_EveryPublishedID_ResolvesInTheCatalog(t *testing.T) {
...
// TestMilestoneHints_EveryPublishedID_ResolvesInTheCatalog renders every
...
// the call site is published just like one behind a constant.
func TestMilestoneHints_EveryPublishedID_ResolvesInTheCatalog(t *testing.T) {
	catalog := ultimateCatalog(t)
