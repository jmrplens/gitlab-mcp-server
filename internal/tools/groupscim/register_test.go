// register_test.go contains integration tests for the group SCIM tool closures
...
// TestRegisterTools_ErrorPaths covers the error branches in the update and delete
// handler closures in register.go when the GitLab API returns errors.
func TestRegisterTools_ErrorPaths(t *testing.T) {
	...
	tools := []struct {
		...
		{"gitlab_update_group_scim_identity", map[string]any{"group_id": "42", "uid": "u", "extern_uid": "new"}},
		{"gitlab_delete_group_scim_identity", map[string]any{"group_id": "42", "uid": "u"}},
	}
	for _, tt := range tools {
		t.Run(tt.name, func(t *testing.T) {
			result, err := session.CallTool(ctx, &mcp.CallToolParams{Name: tt.name, Arguments: tt.args})
			...
			if result == nil || !result.IsError {
				t.Fatalf("expected error result from %s", tt.name)
			}
		})
	}
}
...
// branch in the group SCIM delete handler when the user declines.
func TestRegisterTools_DeleteConfirmDeclined(t *testing.T) {
	...
	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		...
		Arguments: map[string]any{"group_id": "42", "uid": "u"},
	})
	if err != nil {
