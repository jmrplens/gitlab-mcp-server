...
// RegisterTools registers the gitlab_mcp_check_update and
// gitlab_mcp_apply_update tools. If updater is nil the tools are not
...
func RegisterTools(server *mcp.Server, updater *autoupdate.Updater) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		toolutil.LogToolCallAll(ctx, req, "gitlab_mcp_check_update", start, err)
		return toolutil.WithHints(FormatCheckMarkdown(out), out, err)
	})

	mcp.AddTool(server, &mcp.Tool{
		Name:        "gitlab_mcp_apply_update",
		Title:       toolutil.TitleFromName("gitlab_mcp_apply_update"),
		Description: "Download and apply the latest MCP server update. On Linux/macOS the binary is replaced atomically. On Windows the update is downloaded to a staging path with an update script (the running binary cannot be replaced). Use gitlab_mcp_check_update first to verify an update is available.\n\nReturns: JSON with update status and instructions.\n\nSee also: gitlab_mcp_check_update",
		...
		toolutil.LogToolCallAll(ctx, req, "gitlab_mcp_apply_update", start, err)
