...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "List project vulnerabilities (requires Ultimate/Premium). Supports filtering by severity, state, scanner, report type. Returns: paginated list with severity, state, scanner, and detected date.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get a single vulnerability by GID (requires Ultimate/Premium). Returns: full vulnerability details including identifiers, scanner, location, solution, and linked issues/MRs.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconSecurity,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Dismiss a vulnerability with optional comment and reason (requires Ultimate/Premium). Valid reasons: ACCEPTABLE_RISK, FALSE_POSITIVE, MITIGATING_CONTROL, USED_IN_TESTS, NOT_APPLICABLE. Returns: updated vulnerability state.",
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Confirm a detected vulnerability (requires Ultimate/Premium). Changes state from DETECTED to CONFIRMED. Returns: updated vulnerability state.",
		Annotations: toolutil.UpdateAnnotations,
		Icons:       toolutil.IconSecurity,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Resolve a vulnerability (requires Ultimate/Premium). Changes state to RESOLVED. Returns: updated vulnerability state.",
		Annotations: toolutil.UpdateAnnotations,
		Icons:       toolutil.IconSecurity,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Revert a vulnerability to detected state (requires Ultimate/Premium). Changes state back to DETECTED. Returns: updated vulnerability state.",
		Annotations: toolutil.UpdateAnnotations,
		Icons:       toolutil.IconSecurity,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get vulnerability severity counts for a project (requires Ultimate/Premium). Returns: counts per severity level (critical, high, medium, low, info, unknown) and total.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconSecurity,
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Get security report summary for a pipeline (requires Ultimate/Premium). Returns: scanner-level breakdown (SAST, DAST, dependency scanning, container scanning, secret detection, coverage fuzzing, API fuzzing, cluster image scanning) with vulnerability counts.",
		Annotations: toolutil.ReadAnnotations,
		Icons:       toolutil.IconSecurity,
