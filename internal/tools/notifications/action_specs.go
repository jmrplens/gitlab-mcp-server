...
var notificationActionMeta = map[string]notificationActionMetaEntry{
	...
	actionGlobalUpdate: {
		usage:       "Update the authenticated user's account-wide (global) notification settings. Set level (disabled, participating, watch, mention, custom), notification_email, and individual event flags when level is custom. Only the fields you pass are changed.",
