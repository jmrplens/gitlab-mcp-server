...
// and read-only mode cannot leave hidden catalog actions behind. The
// bookkeeping returned beside it is what RegisterCatalogFindExecuteTools
// needs through WithWithheldActions, so a withheld action is reported with
// its cause rather than as unknown.
func Build(client *gitlabclient.Client, cfg *config.ServerConfig) (*actioncatalog.Catalog, gitlabtools.WithheldActions, error) {
	...
	filtered, withheld, filterErr := gitlabtools.FilterActionCatalog(catalog, cfg)
	if filterErr != nil {
