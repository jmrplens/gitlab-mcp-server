...
var externalStatusCheckActionMeta = map[string]externalStatusCheckActionMetaEntry{
	...
	"gitlab_list_project_external_status_checks": {
		usage:       "List the external status check services configured for a project, including their HMAC setting and protected-branch scope. Requires the Maintainer role and an Ultimate license. Supports order_by, sort, and keyset pagination.",
	...
	"gitlab_list_project_mr_external_status_checks": {
		usage:       "List the external status checks that apply to a specific merge request along with each check's current status. Use merge_request_iid (project-scoped). Requires at least the Reporter role and an Ultimate license. Supports order_by, sort, and keyset pagination.",
	...
	"gitlab_create_project_external_status_check": {
		usage:       "Create an external status check service for a project, optionally scoping it to protected branches and supplying a shared secret for HMAC verification. Requires the Maintainer role and an Ultimate license.",
	...
	"gitlab_update_project_external_status_check": {
		usage:       "Update a project external status check service: change its name, external_url, shared secret, or protected-branch scope. Requires the Maintainer role and an Ultimate license.",
	...
	"gitlab_delete_project_external_status_check": {
		usage:       "Permanently delete a project external status check service. Destructive and irreversible. Confirm project_id and check_id before calling. Requires the Maintainer role and an Ultimate license. GitLab answers a caller without the role with success and deletes nothing, so confirm the deletion with external_status_check.list_project.",
	...
	"gitlab_retry_failed_external_status_check_for_project_mr": {
		usage:       "Retry a failed external status check for a merge request so the external service is re-invoked. The check must currently be in the failed state. Requires at least the Developer role and an Ultimate license.",
	...
	"gitlab_set_project_mr_external_status_check_status": {
		usage:       "Set the status (passed or failed) of an external status check for a merge request, identified by sha and external_status_check_id. Typically called by the HMAC-authenticated external service that owns the check. Requires permission to approve the merge request and an Ultimate license.",
