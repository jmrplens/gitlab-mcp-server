...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	description := "Resolve a full git remote URL to a GitLab project and return its project_id and metadata. " +
	...
	mcp.AddTool(server, toolutil.MustIndividualToolFromSpecs(ActionSpecs(client), "gitlab_discover_project", toolutil.IndividualToolProjectionOptions{Description: description, Icons: toolutil.IconProject}), func(ctx context.Context, req *mcp.CallToolRequest, input ResolveInput) (*mcp.CallToolResult, ResolveOutput, error) {
		start := time.Now()
