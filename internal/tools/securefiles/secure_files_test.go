...
// TestSecureFiles_UnreadableFileExtension verifies that every secure file
// handler returns an error rather than a half-filled file when GitLab sends
// file_extension as something that is not a string. client-go models
// file_extension on its own SecureFile as of v3.12.0, so the SDK's decoder is
// what refuses it now that the captured read is retired.
func TestSecureFiles_UnreadableFileExtension(t *testing.T) {
