...
// expanded from the call's parameters and the output as payload. The
// individual surface is this file's module; the other two go through the
// same dispatcher-side embedding and are driven here so one test holds the
// three surfaces to one answer.
