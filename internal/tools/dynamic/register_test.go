...
// TestSearch_ExplainIsOptIn verifies that a search carries a scoring
// explanation and a Why column only when the caller asks for one: the default
// answer has neither, and the explained one has a total that equals the
// result's score, matched terms and a fully populated first reason.
...
// TestCompactParameterGuidance_PrioritizesRequiredParamsAndShowsTruncation
// verifies that the guidance cell names a required parameter before one that
// is merely confusing, drops what the limit leaves no room for, and says how
// many it dropped.
...
// TestCompactParameterGuidanceItem_FormatsAvailableHints verifies the exact
// text one guidance item renders to for each hint it may carry, and the
// fallback sentence for an item carrying none.
...
// TestSearch_ReturnsNextStep verifies that a high-confidence search publishes
// a schema-aware next step, that the same sentence is lifted out of the
// Markdown as a hint, and that the card no longer demands extra discovery.
...
// TestSearch_NoMatchSuggestsNearbyTokens verifies that a query matching
// nothing in the real catalog answers zero results with between one and six
// suggestions, and that the Markdown offers them under "Try:".
...
// TestSearch_RequiresQuery verifies that an empty query is answered with a
// tool error rather than a Go error or an empty result set.
...
// TestSearch_RanksAliasMatches verifies that a query which names an action
// only through its alias and tag vocabulary ("webhook create") ranks that
// action, project.hook_add, first.
...
// TestSearch_UsesIntentSynonymsAndTags verifies the top result for four
// queries that reach their action only through a synonym or a tag: an
// abbreviation, a lifecycle verb, a secret and a metadata request.
...
// TestSearch_ExactCanonicalIDBeatsBroadText verifies that a query that is a
// canonical ID ranks that action first, ahead of every action whose text
// merely contains its words.
...
// TestSearch_CurrentHighConfidenceQueriesRemainStable pins the top result of
// twelve queries against the real catalog that a scoring change must not
// move: the ones a model sends most, each with the action it has to reach.
...
// TestAddStandaloneRoutes_AddsDynamicActions verifies that the standalone
// routes added to an empty route map resolve as the five canonical IDs the
// dynamic surface executes them under: project discovery and the four
// interactive flows.
...
// that a nil catalog is treated as an empty one, and that excluding the four
// interactive tools by name removes their actions while project discovery
// stays.
func TestAddStandaloneCatalog_NilCatalogWithExcludedInteractiveActions(t *testing.T) {
	...
	for _, excluded := range []string{"interactive.issue_create", "interactive.mr_create", "interactive.project_create", "interactive.release_create"} {
		t.Run(excluded, func(t *testing.T) {
			if _, ok := registry.resolveAction(excluded); ok {
				t.Fatalf("%s present, want excluded", excluded)
			}
		})
	}
}
...
func assertCustomCatalogFind(t *testing.T, registry *Registry) {
	...
	// The structured result carries the same metadata as the card, field by
	// field from the description; a model that reads the JSON never sees the
	// table.
	foundResult := found.Results[0]
	if foundResult.Usage != "Use for custom catalog metadata." || !slices.Equal(foundResult.RelatedActions, []string{"custom.audit"}) {
		t.Errorf("Find() usage = %q, related = %v; want the route's usage and related actions", foundResult.Usage, foundResult.RelatedActions)
	}
	if !slices.Equal(foundResult.RequiredParams, []string{"target"}) || foundResult.ParamGuidance["target"].SemanticRole != "custom_target" {
		t.Errorf("Find() required = %v, guidance = %+v; want the schema's required param and its guidance", foundResult.RequiredParams, foundResult.ParamGuidance)
	}
	if foundResult.Score <= 0 {
		t.Errorf("Find() score = %d, want the match's positive score", foundResult.Score)
	}
}

// TestFind_PublishesConfidenceAndAmbiguityOnTheStructuredResult verifies that
// the low-confidence flag and the ambiguity set a match carries reach the
// structured find result, not only the search result and the card. The
// finder's hints tell a model to execute the top row at once, so a result
// that dropped either flag would have the model act on a guess the registry
// knew was one.
func TestFind_PublishesConfidenceAndAmbiguityOnTheStructuredResult(t *testing.T) {
	registry := newRegistry(testRoutes(t), []actionAlias{
		{Alias: "danger.delete", Canonical: "project.delete"},
		{Alias: "danger.delete", Canonical: "package.delete"},
	})

	_, ambiguous, err := registry.Find(t.Context(), nil, FindInput{Query: "danger.delete", Limit: 5})
	if err != nil || ambiguous.Count == 0 {
		t.Fatalf("Find(danger.delete) = %+v, %v; want matches", ambiguous, err)
	}
	if !slices.ContainsFunc(ambiguous.Results, func(result FindResult) bool {
		return slices.Equal(result.AmbiguousWith, []string{"package.delete", "project.delete"})
	}) {
		t.Errorf("Find(danger.delete) results = %+v, want a result carrying both ambiguous targets", ambiguous.Results)
	}

	_, vague, err := registry.Find(t.Context(), nil, FindInput{Query: "project", Limit: 5})
	if err != nil || vague.Count < 2 {
		t.Fatalf("Find(project) = %+v, %v; want several close matches", vague, err)
	}
	if !vague.Results[0].LowConfidence {
		t.Errorf("Find(project) top result = %+v, want it flagged low confidence", vague.Results[0])
	}
}

// TestNewRegistryFromCatalog_NilCatalog verifies that a registry built from a
// nil catalog is usable: a search answers an empty, non-error result rather
// than dereferencing what it was not given.
...
// TestDescribe_CanonicalizesStandaloneAlias verifies that describing a
// standalone tool by its registered name answers the canonical action it
// resolves to, interactive.issue_create, rather than an unknown-action error.
...
// TestDescribe_ReturnsSchemaAndExample verifies what a described destructive
// action carries: the x_destructive and x_confirmation markers pointing at the
// execute tool's own confirm argument, no confirm property or requirement
// inside the params schema, confirm=true in the example, and the tool detail
// URI as its schema URI.
...
// TestDescribe_IncludesOutputSchema verifies that a description carries the
// route's output schema, by finding the property the test route declares in
// it.
...
// TestDescribe_MetaCatalogSchemas verifies, against the real catalog, that a
// multi-action describe publishes each action's input and output schema and
// required params in the structured output, renders the input schema as a
// fenced block in the Markdown, and writes neither raw JSON key into the card.
...
// TestSearch_WhyThisActionOnlyAppearsForCloseAlternatives verifies that the
// why_this_action note is absent from a clear-cut top result and present on
// at least one row of a search whose top result is low confidence.
...
// TestDiscovery_ResultsNameTheActionTheSameWayOnEverySurface verifies that a
// search result, a find result and a description carry one identity for an
// action: the canonical ID, the backing meta-tool, the domain, the action
// name and the schema URI. Each converter copies these field by field from
// the entry and a model reads the domain and action back out of them, while
// nothing else in the package reads the copies, so a converter that swapped
// two of them would still rank, describe and execute correctly.
func TestDiscovery_ResultsNameTheActionTheSameWayOnEverySurface(t *testing.T) {
	registry := NewRegistry(testRoutes(t))
	const id = "project.delete"
	type identity struct{ ID, Tool, Domain, Action, SchemaURI string }
	want := identity{ID: id, Tool: "gitlab_project", Domain: "project", Action: "delete", SchemaURI: "gitlab://tools/" + id}

	_, search, err := registry.Search(t.Context(), nil, SearchInput{Query: id, Limit: 1})
	if err != nil || search.Count != 1 {
		t.Fatalf("Search() = %+v, %v; want one match", search, err)
	}
	_, find, err := registry.Find(t.Context(), nil, FindInput{Query: id, Limit: 1})
	if err != nil || find.Count != 1 {
		t.Fatalf("Find() = %+v, %v; want one match", find, err)
	}
	_, describe, err := registry.Describe(t.Context(), nil, DescribeInput{Action: id})
	if err != nil || describe.Count != 1 {
		t.Fatalf("Describe() = %+v, %v; want one action", describe, err)
	}

	searched, found, described := search.Results[0], find.Results[0], describe.Actions[0]
	cases := []struct {
		name string
		got  identity
	}{
		{name: "search", got: identity{searched.ID, searched.Tool, searched.Domain, searched.Action, searched.SchemaURI}},
		{name: "find", got: identity{found.ID, found.Tool, found.Domain, found.Action, found.SchemaURI}},
		{name: "describe", got: identity{described.ID, described.Tool, described.Domain, described.Action, described.SchemaURI}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.got != want {
				t.Errorf("%s identity = %+v, want %+v", tc.name, tc.got, want)
			}
		})
	}
}
...
// TestFind_ExplainIsOptIn verifies that the finder carries a scoring
// explanation and a Why column only when the caller asks for one: the default
// answer has neither, and the explained one has a total equal to the result's
...
// TestFind_RequiresQuery verifies that an empty query is answered with a tool
// error and an empty structured output rather than a Go error.
...
// TestFind_NoMatchOffersTheNearbyTokens verifies that a find matching nothing
// closes its card with the tokens the registry found near the query.
// FindOutput publishes no suggestions field, so the Markdown hint is the only
// place a caller with nothing else to go on is told what to try; a finder that
// computed the list for the wrong branch would answer that caller with the
// generic broader-terms sentence instead of the token it nearly spelled.
func TestFind_NoMatchOffersTheNearbyTokens(t *testing.T) {
	registry := &Registry{SearchIndex: searchIndex{byToken: map[string][]int{"widget": {0}}}}

	result, output, err := registry.Find(t.Context(), nil, FindInput{Query: "widgat"})
	if err != nil {
		t.Fatalf("Find() error = %v", err)
	}
	if result == nil || result.IsError || output.Count != 0 {
		t.Fatalf("Find() result/output = %+v %+v, want a non-error empty result", result, output)
	}
	hints := toolutil.ExtractHints(textContent(result))
	if !slices.ContainsFunc(hints, func(hint string) bool { return strings.HasPrefix(hint, "Try: `widget`") }) {
		t.Errorf("Find() hints = %q, want the nearby token offered first", hints)
	}
	if slices.Contains(hints, wantBroaderTermsHint) {
		t.Errorf("Find() hints = %q, want the nearby tokens rather than the broader-terms fallback", hints)
	}
}
...
// TestDescribe_UnknownActionReturnsToolError verifies that describing an ID
// the registry does not hold is answered with a tool error and no Go error.
...
// TestDescribe_CanonicalizesAlias verifies that a compatibility alias given
// to describe is answered under the canonical ID it maps to.
...
// TestDescribe_CanonicalizesProviderSpecificAliases pins, against the real
// catalog, the canonical action each provider-observed alias spelling
// describes: the spellings models have been seen to send, from tool-prefixed
// names to dotted note and link forms.
...
// TestDescribe_IncludesDisambiguationUsage verifies that the usage note a
// description carries for each commonly confused action contains the phrase
// that tells it from its neighbor, and that the compare action's card also
// lists its related actions.
...
// TestDescribe_IncludesConsolidatedRegisterMetaReplacementActions verifies
// that four actions which replaced standalone meta registrations are
// describable under their own canonical IDs in the real catalog.
...
// TestDescribe_JobSingleArtifactRequiresArtifactPath verifies that the single
// artifact download names artifact_path, job_id and project_id as required
// and that its example call carries an artifact_path placeholder, since the
// path is what tells it from the whole-archive download.
...
// TestExecute_UsesCatalogFormatter verifies that a dispatch renders its text
// through the formatter the catalog group declares, and still returns the
// route's own output as the structured result.
...
// TestExecute_CanonicalizesAlias verifies that executing a compatibility
// alias dispatches the canonical action it maps to, read back from the action
// the test route echoes.
...
// TestNormalizeActionScopedParamsWithExplanation verifies that an
// action-scoped alias is rewritten to the schema's name and removed under its
// own, and that the one explanation returned names the alias, the canonical
// parameter and the action-scoped source.
...
// TestExecute_ReportsUnknownAndMissingParamsBeforeDispatch verifies that a
// misspelled parameter is refused before the route runs, with a tool error
// naming the unknown key, the nearest valid one, the required key it left
// unset and the full valid list, and no structured output.
...
// TestExecute_RejectsIssueLifecycleAliasStateConflict verifies that
// issue.close sent with state_event=reopen is refused with a tool error that
// names the implied state and points at issue.update, and dispatches nothing.
...
// refusalLog is one "tool call refused" record: the call it names and why.
type refusalLog struct {
	Tool   string `json:"tool"`
	Reason string `json:"reason"`
}

// refusalLogs returns every refusal record written to buf so far, in order.
func refusalLogs(t *testing.T, buf *bytes.Buffer) []refusalLog {
	t.Helper()
	refusals := make([]refusalLog, 0)
	for line := range strings.SplitSeq(strings.TrimSpace(buf.String()), "\n") {
		if strings.TrimSpace(line) == "" {
			continue
		}
		...
		if record.Msg == "tool call refused" {
			refusals = append(refusals, record.refusalLog)
		}
	}
	return refusals
}

// TestDynamicRefusals_LogTheCallAndItsReason verifies, for every way the two
// dynamic tools refuse a call, the record an operator reads: the call as
// executeCallName spells it, and the reason as the shared refusal vocabulary
// spells it. The reason each branch passes is a choice nothing else here
...
func TestDynamicRefusals_LogTheCallAndItsReason(t *testing.T) {
	...
	execute := func(input ExecuteInput) func(context.Context) (*mcp.CallToolResult, error) {
		return func(ctx context.Context) (*mcp.CallToolResult, error) {
			result, _, err := registry.Execute(ctx, nil, input)
			return result, err
		}
	}
	find := func(input FindInput) func(context.Context) (*mcp.CallToolResult, error) {
		return func(ctx context.Context) (*mcp.CallToolResult, error) {
			result, _, err := registry.Find(ctx, nil, input)
			return result, err
		}
	}
	cases := []struct {
		...
		{
			name: "execute with no action",
			call: execute(ExecuteInput{}),
			want: refusalLog{Tool: ExecuteActionToolName, Reason: toolutil.RefusalInvalidParams},
		},
		{
			name: "execute of an unknown action",
			call: execute(ExecuteInput{Action: " Project.Vanish "}),
			want: refusalLog{Tool: ExecuteActionToolName + "/project.vanish", Reason: toolutil.RefusalUnknownAction},
		},
		{
			name: "execute with a misspelled param",
			call: execute(ExecuteInput{Action: "repository.file_get", Params: map[string]any{"project_id": 123, "file_path": "README.md", "reff": "main"}}),
			want: refusalLog{Tool: ExecuteActionToolName + "/repository.file_get", Reason: toolutil.RefusalInvalidParams},
		},
		{
			name: "execute of a lifecycle alias that contradicts its state_event",
			call: execute(ExecuteInput{Action: "issue.close", Params: map[string]any{"project_id": 123, "issue_iid": 1, "state_event": "reopen"}}),
			want: refusalLog{Tool: ExecuteActionToolName + "/issue.update", Reason: toolutil.RefusalInvalidParams},
		},
		{
			name: "execute of a destructive action without confirm",
			call: execute(ExecuteInput{Action: "project.delete", Params: map[string]any{"project_id": 123}}),
			want: refusalLog{Tool: ExecuteActionToolName + "/project.delete", Reason: toolutil.RefusalNeedsConfirmation},
		},
		{
			name: "find with no query",
			call: find(FindInput{Query: "   "}),
			want: refusalLog{Tool: FindActionToolName, Reason: toolutil.RefusalInvalidParams},
		},
		{
			name: "find with an over-long query",
			call: find(FindInput{Query: strings.Repeat("a", MaxSearchQueryLength+1)}),
			want: refusalLog{Tool: FindActionToolName, Reason: toolutil.RefusalInvalidParams},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := refusalLogs(t, logs); !slices.Equal(got, []refusalLog{tc.want}) {
				t.Errorf("refusal records = %+v, want exactly %+v", got, tc.want)
			}
		})
	}
}

// dispatchRecorder is the call identifier the telemetry middleware consults,
// keeping the last tool and route a handler reported through RecordDispatch.
// It resolves nothing, since what is under test is what was reported.
type dispatchRecorder struct {
	tool, action string
	calls        int
}

func (r *dispatchRecorder) Identify(string, any) (mcpotel.Identity, bool) {
	return mcpotel.Identity{}, false
}

func (r *dispatchRecorder) IdentifyDispatch(tool, action string) (mcpotel.Identity, bool) {
	r.tool, r.action = tool, action
	r.calls++
	return mcpotel.Identity{}, false
}

// TestExecute_ReportsTheResolvedRouteToTheSpan verifies what the telemetry
// middleware is told a dynamic call dispatched: the meta-tool and route the
// action resolved to, reported as soon as the action is known so that a call
// refused before the meta handler runs is still named, and nothing at all for
// an action that resolved to nothing. The alias case is the one the report
// exists for: the middleware's own prediction reads the argument, which is
...
func TestExecute_ReportsTheResolvedRouteToTheSpan(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "a destructive action refused for want of confirm",
			input:      ExecuteInput{Action: "project.delete", Params: map[string]any{"project_id": 123}},
			wantCalls:  1,
			wantTool:   "gitlab_project",
			wantAction: "delete",
		},
		{
			name:       "an alias refused for a misspelled param",
			input:      ExecuteInput{Action: "repository_file.get", Params: map[string]any{"project_id": 123, "file_path": "README.md", "reff": "main"}},
			wantCalls:  1,
			wantTool:   "gitlab_repository",
			wantAction: "file_get",
		},
		{
			name:       "a dispatched action, as the meta handler names it",
			input:      ExecuteInput{Action: "project.get", Params: map[string]any{"project_id": 123}},
			wantCalls:  1,
			wantTool:   "gitlab_project",
			wantAction: "get",
		},
		{
			name:  "an unknown action reports nothing",
			input: ExecuteInput{Action: "project.vanish"},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			recorder := &dispatchRecorder{}
			handler := mcpotel.Middleware(mcpotel.Options{Identifier: recorder, Surface: "dynamic"})(
				func(ctx context.Context, _ string, req mcp.Request) (mcp.Result, error) {
					result, _, err := registry.Execute(ctx, req.(*mcp.CallToolRequest), tc.input)
					return result, err
				},
			)
			request := &mcp.CallToolRequest{Params: &mcp.CallToolParamsRaw{Name: executeActionToolName}}
			if _, err := handler(t.Context(), "tools/call", request); err != nil {
				t.Fatalf("handler error = %v", err)
			}
			if recorder.calls != tc.wantCalls {
				t.Fatalf("dispatch reported %d times, want %d", recorder.calls, tc.wantCalls)
			}
			if recorder.tool != tc.wantTool || recorder.action != tc.wantAction {
				t.Errorf("dispatch reported = %s/%s, want %s/%s", recorder.tool, recorder.action, tc.wantTool, tc.wantAction)
			}
		})
	}
}
...
// the registry does not hold is refused with a tool error, no output, and a
// "did you mean" suggestion naming the nearest canonical ID.
...
// TestExecute_RejectsAmbiguousAlias verifies that an alias resolving to two
// canonical actions is refused even with confirm=true, with a tool error that
// names both targets and no output, rather than picking one of them.
...
// TestDescribe_RejectsAmbiguousAlias verifies that describing an alias which
// resolves to two canonical actions answers a tool error and an empty output
...
// TestExecute_DestructiveActionRequiresConfirm verifies that a destructive
...
// TestExecute_CurrentDestructiveSafetyRemainsStable pins both halves of the
// destructive contract on one registry: the same call is refused with the
...
// TestRegisterCatalogFindExecuteTools_ExposesDynamicTools verifies what a
// client sees in tools/list over a real session: exactly the two dynamic
// tools, their descriptions as declared with the guidance phrases a model
// relies on, the execute schema requiring params and carrying action and
// confirm with their linkage text, and an open output schema that declares
// next_steps and pagination.
...
// TestSearch_PartialMatchLongQuery verifies that a query carrying a word no
// action name holds ("open") still returns at least one merge_request action
// instead of letting the unmatched word suppress every result.
...
// TestSearch_NaturalLLMQueriesReturnActions verifies that two phrasings a
// model has been observed to send, with stop words and incidental words in
// them, still return the action each one is asking for within five results.
...
// TestSearch_QueryShapeMatrix_ReturnsExpectedActions verifies, for queries of
// each shape a model sends (canonical, synonym, alias, typo, polite, long,
// standalone and mixed two-intent), that the results contain the action or
// actions the query names.
...
// TestSearch_FuzzyRecoveryQueriesReturnExpectedCandidates verifies, against
// the real catalog, that a query with one or two misspelled words recovers
// the action it meant: a merge_request action on top for the two typo
// spellings, and the retry and protect actions among the results.
...
// TestSearch_ProviderConfusionQueries_ReturnExpectedActions verifies, against
// the real catalog, that each query models were observed to get wrong returns
// every action it needs within its limit: the queries that name a sibling
// action's words, a multi-step workflow, or a path where a name was expected.
...
// TestSearch_TypoQueryReturnsRelevantActions verifies that a query with two
// misspelled words still returns at least one merge_request action from the
// test fixture.
...
// TestRegisterCatalogFindExecuteTools_ListsReadAnnotationsAndIcons verifies
// the listing metadata the two tools carry beyond the execute hints the test
// above derives: find is served with the shared read annotations, execute is
// open-world, and each shows its own icon. A client decides whether to ask
// before calling a tool from these hints, so a find that lost its read-only
// hint would prompt for the one tool that touches nothing.
func TestRegisterCatalogFindExecuteTools_ListsReadAnnotationsAndIcons(t *testing.T) {
	server := mcp.NewServer(&mcp.Implementation{Name: "dynamic-test", Version: "0"}, nil)
	RegisterCatalogFindExecuteTools(server, actioncatalog.FromActionMaps(testRoutes(t)))
	tools := listDynamicTools(t, server)

	findTool := listedTool(t, tools, findToolName)
	if got := findTool.Annotations; got == nil || !got.ReadOnlyHint || !got.IdempotentHint ||
		got.DestructiveHint == nil || *got.DestructiveHint || got.OpenWorldHint == nil || !*got.OpenWorldHint {
		t.Errorf("gitlab_find_action annotations = %+v, want the shared read annotations", got)
	}
	if len(findTool.Icons) != len(toolutil.IconSearch) || findTool.Icons[0].Source != toolutil.IconSearch[0].Source {
		t.Errorf("gitlab_find_action icons = %+v, want the search icon set", findTool.Icons)
	}

	executeTool := listedTool(t, tools, executeActionToolName)
	if got := executeTool.Annotations; got == nil || got.OpenWorldHint == nil || !*got.OpenWorldHint {
		t.Errorf("gitlab_execute_action annotations = %+v, want an open-world hint", got)
	}
	if len(executeTool.Icons) != len(toolutil.IconServer) || executeTool.Icons[0].Source != toolutil.IconServer[0].Source {
		t.Errorf("gitlab_execute_action icons = %+v, want the server icon set", executeTool.Icons)
	}
}
...
// TestRegistry_HelperCoverage verifies the small helpers the registry leans
// on: a nil annotation base yields an empty annotation value, dedupeStrings
// trims, lowercases and drops repeats, normalizedLimit clamps both ends, and
// the suggestion, scoring and segmented-search entry points answer nothing for
// an empty or too-short query rather than failing.
...
// TestScoreSearchAlternative_ReturnsReason verifies that a match made through
// a synonym scores, and that its reason names the field, the original query
// term and the synonym it matched under, rather than only the score.
...
// score must be at least 80 and the top-result margin must be at least 15. The
// margin the explanation publishes is asserted too, since it is what a reader
// of a low-confidence verdict checks it against: the gap to the runner-up, or
// the whole score when there is no runner-up.
func TestComputeConfidence_Thresholds(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantMargin: minimumHighConfidenceMargin,
		},
		{
			name: "low score",
			matches: []scoredActionEntry{
			...
			wantLow:    true,
			wantMargin: minimumHighConfidenceScore - 1,
		},
		{
			name: "close margin",
			matches: []scoredActionEntry{
			...
			wantLow:    true,
			wantMargin: minimumHighConfidenceMargin - 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got[0].explanation.MarginToNext != tt.wantMargin {
				t.Errorf("explanation.MarginToNext = %d, want %d", got[0].explanation.MarginToNext, tt.wantMargin)
			}
		})
	}
}
...
// TestSearchRuntimeMetrics_RecordQualitySignals verifies that three searches
// through the registry, one matching nothing, one misspelled and one on an
// ambiguous alias, leave a snapshot counting three searches with the
// zero-result, fuzzy, ambiguous and low-confidence signals each raised, and
// that a negative suppression count moves nothing.
...
// TestCatalogActionAliases_CarriesEachFieldOfTheSpec verifies that an alias a
// catalog action declares reaches the registry with every field it carries:
// the canonical target is the action's own ID, the source is the spec's
// unless the alias is deprecated, and searchability and the reason travel as
// declared. Nothing else reads this copy back field by field: a searchable
// alias copied as unsearchable still resolves in execute, and a reason copied
// from the wrong field still audits as present.
func TestCatalogActionAliases_CarriesEachFieldOfTheSpec(t *testing.T) {
	catalog := actioncatalog.NewCatalog()
	group := actioncatalog.NewGroup(actioncatalog.GroupOptions{ToolName: "gitlab_project"})
	group.SetAction(actioncatalog.Action{
		Name:  "get",
		...
		Compatibility: toolutil.CompatibilityPolicy{ActionAliases: []toolutil.ActionAliasSpec{
			{Alias: "project.lookup", Target: "project.get", Source: "provider_observed", Searchable: true, Reason: "seen from a model"},
			{Alias: "project.show", Target: "project.get", Source: "catalog", Deprecated: true, Reason: "renamed in 3.0"},
		}},
	})
	if err := catalog.AddGroup(group); err != nil {
		t.Fatalf("AddGroup() error = %v", err)
	...
	want := []actionAlias{
		{Alias: "project.lookup", Canonical: "project.get", Source: aliasSourceProviderObserved, Searchable: true, Notes: "seen from a model"},
		{Alias: "project.show", Canonical: "project.get", Source: aliasSourceDeprecated, Searchable: false, Notes: "renamed in 3.0"},
	}
	if got := catalogActionAliases(catalog); !slices.Equal(got, want) {
		t.Errorf("catalogActionAliases() = %+v, want %+v", got, want)
	}
}
...
// TestCompactExampleJSON_OmitsOnlyAnEmptyExample verifies that the example
// fence is left out for an example carrying nothing, and written for one
// carrying either half. Both halves are always filled by exampleFor, so a
// half-empty example is a defect upstream of the formatter, and the fence is
// where a reader would see it; a guard that also hid those would hide the
// defect with them.
func TestCompactExampleJSON_OmitsOnlyAnEmptyExample(t *testing.T) {
	cases := []struct {
		name    string
		example ActionExample
		want    string
	}{
		{name: "nothing at all", example: ActionExample{}},
		{name: "a tool and no arguments", example: ActionExample{Tool: executeActionToolName}, want: `{"tool":"gitlab_execute_action","arguments":null}`},
		{name: "arguments and no tool", example: ActionExample{Arguments: map[string]any{"action": "project.get"}}, want: `{"tool":"","arguments":{"action":"project.get"}}`},
		{name: "an unmarshalable argument", example: ActionExample{Tool: executeActionToolName, Arguments: map[string]any{"bad": make(chan int)}}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := compactExampleJSON(tc.example); got != tc.want {
				t.Errorf("compactExampleJSON(%+v) = %q, want %q", tc.example, got, tc.want)
			}
		})
	}
}
...
// TestNormalization_FormattingBranches covers compact helpers that shape
// user-facing dynamic tool output. It verifies deduplication of described
// actions, placeholder selection, confirmation parsing, the empty-result card
// each discovery tool writes, and the hints a one-row find card closes with.
...
// TestAddServiceAccountPATActionTags_TagShapes verifies that a service
// account token action is tagged with the scoped resource name and, when its
// name carries a verb suffix, with the scoped "pat <verb>" phrase; an action
// with no verb suffix still carries the resource tag. That nothing more is
// added without a verb is held by the test below, which compares the whole
// list.
...
//
// The access token branch reads three ID patterns, one per token scope, and
// each is claimed on its own: a project case alone cannot tell "any of the
// three" from "the project one, or both of the others", and under the second
// reading every group and personal token action loses the access token
// phrases while the project ones keep them.
func TestAddIDPatternTags_ClaimsOnlyTheIDShapesItRecognizes(t *testing.T) {
	runActionTaggerCases(t, addIDPatternTags, []actionTaggerCase{
		...
		{name: "project access token id", id: "project.access_token_project_create", domain: "project", action: "access_token_project_create", matched: true, want: []string{"access token", "project access token"}},
		{name: "group access token id", id: "access.token_group_list", domain: "access", action: "token_group_list", matched: true, want: []string{"access token", "personal access token"}},
		{name: "personal access token id", id: "access.token_personal_revoke", domain: "access", action: "token_personal_revoke", matched: true, want: []string{"access token", "personal access token"}},
...
// TestScoreReleaseListIntent_PositiveCase verifies that "list releases"
// against release.list earns the release-list boost with a reason naming the
// intent field and the "list releases" query term.
...
// likely to get wrong and the list is truncated from the end. The confusion
// order runs against the alphabetical one on purpose: a fixture where the two
// agreed let a comparator that skipped the count and fell through to the name
// produce this same list, and whether any other test caught it depended on map
// iteration order, since that comparator is inconsistent wherever three
// parameters meet.
func TestCompactParameterGuidance_OrdersByConfusionCount(t *testing.T) {
	guidance := map[string]toolutil.ParameterGuidance{
		"alpha": {SemanticRole: "no recorded confusion"},
		"bravo": {CommonConfusions: []string{"first"}},
		"delta": {CommonConfusions: []string{"first", "second"}},
		"echo":  {CommonConfusions: []string{"first", "second", "third"}},
	}

	got := compactParameterGuidance(guidance, 4)
	previous := -1
	for _, name := range []string{"`echo`", "`delta`", "`bravo`", "`alpha`"} {
		t.Run(name, func(t *testing.T) {
			index := strings.Index(got, name)
			if index <= previous {
				t.Fatalf("compactParameterGuidance() = %q, want echo, delta, bravo, alpha in that order", got)
			}
			previous = index
		})
	}
...
// "Required params" line reads as an action that takes none. The same holds
// of the card as a whole: one describing no action closes with no hint, since
// the hint tells a model to call the action ID the card names and an empty
// card names none.
func TestFormatDescribeOutput_OptionalSections(t *testing.T) {
	...
	t.Run("a card describing nothing carries no execute hint", func(t *testing.T) {
		want := "## GitLab Catalog: 0 actions described\n\n"
		if got := formatDescribeOutput(DescribeOutput{}); got != want {
			t.Fatalf("formatDescribeOutput(empty) = %q, want %q", got, want)
		}
	})
...
// TestHasSearchGuidance_EachSignalAlone is the search twin of the test above:
// either of the two signals a search result can carry opens its Guidance
// column on its own. A destructive action with no usage note is the common
// case, since most of the catalog carries no note, and a column that needed
// both would drop the confirm reminder from every row that needs it most.
func TestHasSearchGuidance_EachSignalAlone(t *testing.T) {
	cases := []struct {
		name   string
		result SearchResult
		want   bool
	}{
		{name: "destructive alone", result: SearchResult{ID: "project.delete", Destructive: true}, want: true},
		{name: "usage alone", result: SearchResult{ID: "project.get", Usage: "reads one project"}, want: true},
		{name: "blank usage is not a signal", result: SearchResult{ID: "project.get", Usage: "   "}, want: false},
		{name: "nothing to say", result: SearchResult{ID: "project.get"}, want: false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := hasSearchGuidance([]SearchResult{tc.result}); got != tc.want {
				t.Errorf("hasSearchGuidance() = %v, want %v", got, tc.want)
			}
		})
	}
}

// TestAmbiguousTargetsFromFindResults_UsesTheFirstPopulatedSet is the find
// twin of the search test of the same name: the ambiguity set comes from the
// first result that carries one, so a top result without a set does not hide
// the set a lower row carries. The finder's hints tell a model to execute the
// top row at once, and this set is what tells it to choose instead.
func TestAmbiguousTargetsFromFindResults_UsesTheFirstPopulatedSet(t *testing.T) {
	t.Run("skips results without an ambiguity set", func(t *testing.T) {
		...
		if got := ambiguousTargetsFromFindResults(results); !slices.Equal(got, []string{"group.list", "project.list"}) {
			t.Errorf("ambiguousTargetsFromFindResults() = %v, want the deduplicated first set", got)
		}
	})

	t.Run("no result carries one", func(t *testing.T) {
		if got := ambiguousTargetsFromFindResults([]FindResult{{ID: "project.get"}, {ID: "project.list"}}); got != nil {
