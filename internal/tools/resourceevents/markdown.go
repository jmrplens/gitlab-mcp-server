...
func FormatLabelEventsMarkdown(out ListLabelEventsOutput) string {
	...
	for _, e := range out.Events {
		fmt.Fprintf(&sb, "| %d | %s | %s | %s | %s |\n", e.ID, e.Action, e.Label.Name, e.Username, toolutil.FormatTime(e.CreatedAt))
	}
...
func FormatMilestoneEventsMarkdown(out ListMilestoneEventsOutput) string {
	...
	for _, e := range out.Events {
		fmt.Fprintf(&sb, "| %d | %s | %s | %s | %s |\n", e.ID, e.Action, e.MilestoneTitle, e.Username, toolutil.FormatTime(e.CreatedAt))
	}
...
func FormatIterationEventsMarkdown(out ListIterationEventsOutput) string {
	...
	for _, e := range out.Events {
		fmt.Fprintf(&sb, "| %d | %s | %s | %s | %s |\n", e.ID, e.Action, e.Iteration.Title, e.Username, toolutil.FormatTime(e.CreatedAt))
	}
