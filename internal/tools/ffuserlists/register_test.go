// register_test.go contains integration tests for the feature flag user list
...
func TestRegisterTools_DeleteError(t *testing.T) {
	...
	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		...
		Arguments: map[string]any{"project_id": "my-project", "iid": float64(10)},
	})
	if err != nil {
...
// branch in the gitlab_ff_user_list_delete handler when the user declines.
func TestRegisterTools_DeleteConfirmDeclined(t *testing.T) {
	...
	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name:      "gitlab_ff_user_list_delete",
		Arguments: map[string]any{"project_id": "p", "iid": float64(1)},
	})
	if err != nil {
		t.Fatalf("CallTool error: %v", err)
	}
