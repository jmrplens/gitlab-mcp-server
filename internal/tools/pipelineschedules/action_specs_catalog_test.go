// action_specs_catalog_test.go holds the pipeline schedule action IDs this
// package publishes against the catalog the server really builds. It is an
// external test package because internal/tools imports this one, so only a
// package outside it may import the catalog back.
package pipelineschedules_test
...
// TestPipelineScheduleActionIDs_EveryPublishedID_NamesACatalogAction asserts
...
// string was copied twice. These specs are aggregated into the gitlab_pipeline
// group, so their domain is "pipeline"; the "pipeline_schedule." spelling the
// metadata carried before named the package instead.
func TestPipelineScheduleActionIDs_EveryPublishedID_NamesACatalogAction(t *testing.T) {
	ids := catalogActionIDs(t)
	for _, id := range pipelineschedules.PublishedActionIDs() {
...
// TestPipelineScheduleSpecs_EveryRelatedAction_NamesACatalogAction asserts the
...
func TestPipelineScheduleSpecs_EveryRelatedAction_NamesACatalogAction(t *testing.T) {
	ids := catalogActionIDs(t)
	client := testutil.NewTestClient(t, http.NotFoundHandler())
	for _, spec := range pipelineschedules.ActionSpecs(client) {
