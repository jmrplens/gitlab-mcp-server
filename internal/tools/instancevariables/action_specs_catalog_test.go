...
// internal/tools, which imports this one: package instancevariables could not
// reach the oracle without an import cycle. The test exists because nothing
// here did: all fifteen related actions named the owner package as their
// domain ("instancevariables.instance_get") where the catalog aggregates these
// specs into gitlab_ci_variable and registers "ci_variable.instance_get", so
// every one of them was a dead end for a model that followed it.
package instancevariables_test
...
// the related actions its specs carry, and the IDs its Markdown formatters
// quote in result hints.
//
...
// served string. These formatters quote bare action words rather than
// canonical IDs today, so they contribute nothing; the reading is here so the
// first canonical ID a hint names is judged on the same terms as the rest.
func publishedActionIDs(t *testing.T) []publishedActionID {
	t.Helper()
	specs := instancevariables.ActionSpecs(testutil.NewTestClient(t, testutil.ForbiddenHandler(t)))
	published := relatedActionIDs(specs)
	rendered := []string{
		instancevariables.FormatOutputMarkdown(instancevariables.Output{}),
		instancevariables.FormatListMarkdown(instancevariables.ListOutput{Variables: []instancevariables.Output{{}}}),
	}
...
// sorted so the subtests run in a stable order.
