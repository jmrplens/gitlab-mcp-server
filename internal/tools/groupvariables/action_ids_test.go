...
// group-scoped variable actions are aggregated into the ci_variable group
// beside the project and instance ones, so the package name is not the domain;
// the specs here published "group_variable.group_get" and its siblings, which
// read as perfectly plausible pairs and resolve to nothing.
func TestPublishedActionIDs_NameActionsTheCatalogHolds(t *testing.T) {
	t.Parallel()

	catalog := catalogForTest(t)
	ids := groupvariables.PublishedActionIDs()
	if len(ids) == 0 {
...
func TestActionSpecs_RelatedActionsNameActionsTheCatalogHolds(t *testing.T) {
	...
	catalog := catalogForTest(t)
	specs := groupvariables.ActionSpecs(nil)
	if len(specs) == 0 {
