...
func RegisterTools(server *mcp.Server, client *gitlabclient.Client) {
	...
	mcp.AddTool(server, templateTool("gitlab_list_project_templates", "List project templates of a given type (dockerfiles, gitignores, gitlab_ci_ymls, licenses).\n\nReturns: JSON array of templates with pagination.\n\nSee also: gitlab_get_project_template, gitlab_list_ci_yml_templates"), func(ctx context.Context, req *mcp.CallToolRequest, input ListInput) (*mcp.CallToolResult, ListOutput, error) {
		start := time.Now()
	...
	mcp.AddTool(server, templateTool("gitlab_get_project_template", "Get a single project template by type and key.\n\nReturns: JSON with the template details.\n\nSee also: gitlab_list_project_templates, gitlab_list_dockerfile_templates"), func(ctx context.Context, req *mcp.CallToolRequest, input GetInput) (*mcp.CallToolResult, GetOutput, error) {
		start := time.Now()
