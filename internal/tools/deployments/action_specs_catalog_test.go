// action_specs_catalog_test.go holds the deployment action IDs this package
...
// TestDeploymentActionIDs_EveryPublishedID_NamesACatalogAction asserts that
...
// gitlab_environment group, so their IDs carry the environment domain, and
// the "deployment." spelling they carried before named no group at all.
func TestDeploymentActionIDs_EveryPublishedID_NamesACatalogAction(t *testing.T) {
	ids := catalogActionIDs(t)
	for _, id := range deployments.PublishedActionIDs() {
...
// TestDeploymentActionSpecs_EveryRelatedAction_NamesACatalogAction asserts the
...
func TestDeploymentActionSpecs_EveryRelatedAction_NamesACatalogAction(t *testing.T) {
	ids := catalogActionIDs(t)
	client := testutil.NewTestClient(t, http.NotFoundHandler())
	for _, spec := range deployments.ActionSpecs(client) {
