...
Every release publishes one static binary per platform: Linux, macOS and Windows, on amd64 and arm64, built with `CGO_ENABLED=0` as position-independent executables with the version and commit stamped in. No runtime, no shared libraries, no container. This page covers the two install scripts, downloading by hand, checking what you downloaded, where to keep the file, and how to replace it later.

## Install script

Both scripts do the same four things: detect the platform, download the matching asset from the latest GitHub Release, verify its SHA-256 against the `checksums.txt` of that same release, and copy it to a per-user directory. Verification fails closed: a mismatch, a missing entry, or a `checksums.txt` that cannot be fetched aborts the install rather than skipping the check.
...
The same overrides exist as environment variables (`VERSION`, `INSTALL_DIR`, `REPO`, `ALLOW_UNVERIFIED`) and, when you save the script and run it as a file, as parameters (`-Version`, `-InstallDir`, `-Repo`). When piping into `iex`, set the variables first:
...
The install scripts already compare the download with `checksums.txt`. Done by hand, that check plus the signature on the checksums file takes two commands. `checksums.txt` lists the SHA-256 of the seven binaries, and `checksums.txt.sigstore.json` is a keyless [Cosign](https://docs.sigstore.dev/cosign/installation/) signature of that file, made by the release workflow under GitHub's OIDC identity, so there is no key to distribute.
...
With Claude Code the whole step is one command, `claude mcp add gitlab --env GITLAB_TOKEN=glpat-xxxx -- gitlab-mcp-server` (add `--env GITLAB_URL=https://gitlab.example.com` for self-managed GitLab). The per-client tabs, including VS Code's `servers` map with a masked token prompt, are on the [Quick Start](/gitlab-mcp-server/getting-started/#manual-configuration).
