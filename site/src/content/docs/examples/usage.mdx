...
The server first calls `gitlab_project` with `action: label_list` to show existing labels, then `action: label_create` to add the new one.

## Track milestones

**Prompt**: "Show me the progress on the Sprint 14 milestone in my-project"

The server calls `gitlab_project` with `action: milestone_get`, returning completion percentage, open/closed issue counts, and due date.
...
The server calls `gitlab_project` with `action: members`, returning team members with their roles and permissions.
