...
:::tip
These examples show meta-tool mode with `TOOL_SURFACE=meta`. Each prompt maps to a single meta-tool call with the appropriate `action` parameter. The AI assistant handles the mapping automatically — just describe what you need in natural language.
:::
