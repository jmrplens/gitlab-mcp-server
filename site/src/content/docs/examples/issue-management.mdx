...
faq:
  - q: "How do I triage GitLab issues with an AI assistant?"
    a: "Ask your assistant to list the open issues you care about, then to assign, label, and link them. A typical triage loop is four calls: `issue.list` (filtered by `state: opened` and a label) to see the backlog, `issue.update` to set `assignee_ids`, `labels`, and `milestone_id`, `issue.link_create` to record dependencies, and `issue.update` again with `state_event: close` once resolved. On the default dynamic surface these are `gitlab_execute_action` actions, discoverable through `gitlab_find_action`; with `TOOL_SURFACE=meta` they are the `gitlab_issue` meta-tool's `list`, `update`, and `link_create` actions."
...
With `TOOL_SURFACE=meta` the same calls go to the `gitlab_issue` meta-tool with `action: list`, `action: update` and `action: link_create`, each with the parameters nested under `params`.
