...
faq:
  ...
  - q: "Can an AI assistant close a GitLab issue for me?"
    a: "Yes, but closing is a state change, so it follows the same safety rules as any other mutation. The call is `issue.update` with `state_event: close` (`gitlab_issue`, `action: update`, on the meta surface). If the deployment runs with `GITLAB_MCP_READ_ONLY=true` the action is not registered at all, and under `GITLAB_MCP_SAFE_MODE=true` the server returns a JSON preview of the change instead of applying it."
