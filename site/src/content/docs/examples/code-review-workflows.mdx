...
faq:
  ...
  - q: "How do I find unresolved review threads on a merge request?"
    a: "Run `mr_review.discussion_list` for the MR (the `discussion_list` action of `gitlab_mr_review` when `TOOL_SURFACE=meta`), then filter the returned threads for `resolved: false`. Notes created with `mr_review.note_create` are not resolvable and never appear as outstanding items, so only discussions show up here. This is the check to run before merging when the project requires all threads resolved."
...
With `TOOL_SURFACE=meta` the same calls go to `gitlab_merge_request` with `action: list` and to `gitlab_mr_review` with `action: changes_get` and `action: discussion_create`, each with the parameters nested under `params`.
