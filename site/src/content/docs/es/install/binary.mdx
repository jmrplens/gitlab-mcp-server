...
Cada release publica un binario estático por plataforma: Linux, macOS y Windows, en amd64 y arm64, compilado con `CGO_ENABLED=0` como ejecutable independiente de la posición y con la versión y el commit grabados dentro. Sin runtime, sin bibliotecas compartidas, sin contenedor. Esta página cubre los dos scripts de instalación, la descarga manual, cómo comprobar lo que has descargado, dónde guardar el archivo y cómo reemplazarlo más adelante.
...
imprime `gitlab-mcp-server 2.7.5 (commit: <commit>)` para la release actual. Arrancar el binario sin argumentos en una terminal, o hacerle doble clic en Windows, sin `GITLAB_TOKEN` definido imprime una breve pantalla de primera ejecución que nombra los dos valores que necesita y espera a que pulses Enter; un cliente MCP nunca la ve, porque habla con el servidor por tuberías. No hay asistente de configuración.
...
Con Claude Code el paso entero es un comando, `claude mcp add gitlab --env GITLAB_TOKEN=glpat-xxxx -- gitlab-mcp-server` (añade `--env GITLAB_URL=https://gitlab.example.com` para GitLab autoalojado). Las pestañas por cliente, incluido el mapa `servers` de VS Code con la petición enmascarada del token, están en el [Inicio rápido](/gitlab-mcp-server/es/getting-started/#configuración-manual).
