...
El servidor primero llama a `gitlab_project` con `action: label_list` para mostrar las etiquetas existentes, y luego `action: label_create` para añadir la nueva.

## Seguir milestones

**Prompt**: "Muéstrame el progreso del milestone Sprint 14 en my-project"

El servidor llama a `gitlab_project` con `action: milestone_get`, devolviendo el porcentaje de completado, conteos de issues abiertos/cerrados y fecha límite.
...
El servidor llama a `gitlab_project` con `action: members`, devolviendo los miembros del equipo con sus roles y permisos.
