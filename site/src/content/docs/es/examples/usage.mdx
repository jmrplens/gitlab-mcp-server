...
faq:
  ...
  - q: "¿Necesito conocer los nombres de las herramientas para usar GitLab MCP Server?"
    a: "No. En la superficie dinámica predeterminada describes lo que quieres y el asistente llama a `gitlab_find_action` para localizar la acción y su esquema exacto, y después a `gitlab_execute_action` para ejecutarla. Los nombres como `gitlab_issue` solo se hacen visibles si cambias a `TOOL_SURFACE=meta` o `TOOL_SURFACE=individual`, que intercambian contexto de arranque por una lista navegable de herramientas."
...
El asistente ejecuta `project.list` (`gitlab_project` con `action: list` cuando `TOOL_SURFACE=meta`), devolviendo nombres de proyectos, descripciones y URLs.
