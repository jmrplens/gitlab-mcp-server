...
La superficie dinámica predeterminada descubre acciones CI/CD exactas antes de ejecutar:

```text
gitlab_find_action → query: "estado del último pipeline del proyecto"
gitlab_execute_tool → action: "pipeline.latest", params: { project_id: "42" }
```
...
gitlab_execute_tool → action: "job.trace", params: { project_id: "42", job_id: 9876 }
```
