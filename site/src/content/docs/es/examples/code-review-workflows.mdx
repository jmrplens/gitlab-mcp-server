...
Con `TOOL_SURFACE=meta` las mismas llamadas van a `gitlab_merge_request` con `action: list` y a `gitlab_mr_review` con `action: changes_get` y `action: discussion_create`, cada una con los parámetros anidados en `params`.
