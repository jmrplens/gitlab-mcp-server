...
| `GITLAB_MCP_RATE_LIMIT_RPS`            | `0`            | Límite por credencial en req/s sobre toda llamada que llega a GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get` (`0` lo desactiva). Ambos transportes: el valor por defecto es `0` en stdio y `10` en modo HTTP, donde `--rate-limit-rps` lo sobrescribe                                                                                                                                                     |
...
| `--tool-search`               | _(vacío)_      | Busca en el catálogo de acciones por nombre, alias, etiqueta o descripción y sale; imprime cada ID canónico de acción junto al nombre que le da la superficie configurada. Lee `--tool-surface` y `--tier`, y en su defecto `GITLAB_MCP_TOOL_SURFACE` y `GITLAB_MCP_TIER` |
