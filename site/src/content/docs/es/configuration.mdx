    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "¿Cuál es la configuración mínima de GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "En modo stdio basta con un único GITLAB_TOKEN con el scope api para arrancar, y cualquier otra variable es opcional y tiene un valor predeterminado seguro. GITLAB_URL usa https://gitlab.com por defecto, así que solo la estableces al conectar a una instancia autogestionada. Para una configuración de solo lectura, usa un token read_api y añade GITLAB_READ_ONLY=true. En modo HTTP no se configura ningún token en el servidor; cada cliente envía el suyo en cada petición."
            }
          },
          {
            "@type": "Question",
            "name": "¿Debo usar el modo stdio o el modo HTTP en GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Usa el modo stdio, el predeterminado, para configuraciones locales de un solo usuario como integraciones con IDE (VS Code, Cursor, Claude Desktop); se configura mediante variables de entorno o un archivo .env. Usa el modo HTTP, activado con --http, para despliegues compartidos o remotos como Docker, Fly.io o Kubernetes, donde la configuración usa flags de CLI y cada cliente se autentica con su propio token de GitLab en cada petición."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cómo apunto GitLab MCP Server a una instancia de GitLab autogestionada?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Establece GITLAB_URL con la URL base de tu instancia, por ejemplo GITLAB_URL=https://gitlab.example.com; en modo HTTP usa el flag --gitlab-url. Si la instancia usa un certificado autofirmado o de CA interna, establece también GITLAB_SKIP_TLS_VERIFY=true, o --skip-tls-verify en modo HTTP, pero solo en una red de confianza porque desactiva la verificación de certificados para todas las conexiones a GitLab."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cómo elijo una superficie de herramientas en GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Establece la variable TOOL_SURFACE, o el flag --tool-surface en modo HTTP. La superficie dynamic es la predeterminada y la de menor consumo de tokens: expone gitlab_find_action y gitlab_execute_action mientras mantiene cada acción de GitLab accesible a través del catálogo canónico de acciones. Elige meta cuando un cliente prefiere despachadores consolidados por dominio con un parámetro action, e individual para registrar una herramienta MCP por cada operación de GitLab."
            }
          }
        ]
      }
