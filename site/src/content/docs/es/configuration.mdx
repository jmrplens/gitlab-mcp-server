...
| `CAPABILITY_SURFACE`     | `full`               | Selector del catálogo de recursos y prompts: `full` conserva el catálogo completo; `minimal` mantiene solo `gitlab://workspace/roots` y desactiva recursos opcionales, meta-schema, guías y prompts                |
...
Para despliegues dinámicos donde los recursos y prompts dominan el contexto inicial, usa `CAPABILITY_SURFACE=minimal`. Mantiene `gitlab://workspace/roots` para descubrimiento de proyectos y omite recursos opcionales, recursos de meta-schema, guías de flujo y prompts. Con `CAPABILITY_SURFACE=minimal`, usa `gitlab_describe_tools` para recuperar los schemas de acciones dinámicas en línea. El valor por defecto sigue siendo `full`; hoy no se expone un modo schemas-only.
:::
