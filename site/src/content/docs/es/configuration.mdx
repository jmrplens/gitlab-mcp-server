...
| `META_TOOLS`             | `true`               | Selector del catálogo de herramientas: `true` para meta-herramientas, `false` para herramientas individuales, `dynamic` para la superficie de bajo consumo search/describe/execute, `dynamic-3` para la superficie dinámica actual explícita, o `dynamic-2` experimental |
...
Usa `TOOL_SURFACE=dynamic` para el catálogo visible estable más pequeño: solo se exponen `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo catálogo canónico de acciones de GitLab. Las meta-herramientas siguen siendo el modo predeterminado hoy, pero el modo dinámico es el candidato actual para un futuro predeterminado de bajo consumo. Usa `TOOL_SURFACE=dynamic-2` solo para el candidato A/B find/execute aparcado.
...
Para despliegues dinámicos donde los recursos y prompts dominan el contexto inicial, usa `CAPABILITY_SURFACE=minimal`. Mantiene `gitlab://workspace/roots` para descubrimiento de proyectos y omite recursos opcionales, recursos de meta-schema, guías de flujo y prompts. El valor por defecto sigue siendo `full`.
:::
