...
| `CAPABILITY_SURFACE`     | `full`               | Selector del catálogo de recursos y prompts: `full` conserva el catálogo completo; `minimal` mantiene `gitlab://workspace/roots`, desactiva recursos opcionales, prompts y guías de flujo, y conserva recursos de meta-schema solo para `TOOL_SURFACE=meta` |
...
Para despliegues dinámicos donde los recursos y prompts dominan el contexto inicial, usa `CAPABILITY_SURFACE=minimal`. Mantiene `gitlab://workspace/roots` para descubrimiento de proyectos y omite recursos opcionales, prompts, guías de flujo y recursos de meta-schema dinámicos. Esto no elimina los schemas del descubrimiento dinámico: usa `gitlab_find_action` para recuperar schemas exactos antes de llamar a `gitlab_execute_tool`. En `TOOL_SURFACE=meta`, minimal conserva recursos de meta-schema para que los clientes mantengan `META_PARAM_SCHEMA=opaque` sin perder schemas exactos de parámetros. El valor por defecto sigue siendo `full`; hoy no se expone un modo schemas-only.
:::
...
| `--capability-surface`   | `full`                       | Selector del catálogo de recursos y prompts: `full` o `minimal`; minimal omite recursos opcionales, guías y prompts, pero conserva meta-schema para `--tool-surface=meta` |
