...
Usa `TOOL_SURFACE=dynamic` para el catálogo visible estable más pequeño: solo se exponen `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo registro oculto de acciones de GitLab. Las meta-herramientas siguen siendo el modo predeterminado hoy, pero el modo dinámico es el candidato actual para un futuro predeterminado de bajo consumo. Usa `TOOL_SURFACE=dynamic-2` solo para el candidato A/B find/execute aparcado.
