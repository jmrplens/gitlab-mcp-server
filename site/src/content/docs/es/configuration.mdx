...
| `META_TOOLS`             | `true`               | Selector del catálogo de herramientas: `true` para meta-herramientas, `false` para herramientas individuales, `dynamic` para la superficie de bajo consumo search/describe/execute, o superficies experimentales `dynamic-2`/`dynamic-3` |
...
Usa `TOOL_SURFACE=dynamic` para el catálogo visible estable más pequeño: solo se exponen `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo registro oculto de acciones de GitLab. Usa `TOOL_SURFACE=dynamic-2` solo para el candidato experimental A/B find/execute.
