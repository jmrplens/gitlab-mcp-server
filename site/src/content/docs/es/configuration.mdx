...
| `TOOL_SURFACE`           | `meta`               | Selector canónico del catálogo: `meta`, `individual`, `dynamic`, `dynamic-2` o `dynamic-3`                                                                                                                         |
...
| `CAPABILITY_SURFACE`     | `full`               | Selector del catálogo de recursos y prompts: `full` conserva el catálogo completo; `minimal` mantiene solo `gitlab://workspace/roots` y desactiva recursos opcionales, prompts, guías de flujo y meta-schema       |
...
:::tip
**El modo meta-herramientas** (`TOOL_SURFACE=meta`) es altamente recomendado. Agrupa las herramientas por dominio (p. ej., `gitlab_issue` gestiona list, get, create, update, delete) lo que reduce el uso de tokens y mejora la precisión de selección de herramientas por la IA.

Usa `TOOL_SURFACE=dynamic` para el catálogo visible estable más pequeño: solo se exponen `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo catálogo canónico de acciones de GitLab. `TOOL_SURFACE=dynamic-3` selecciona explícitamente esa misma superficie actual de tres herramientas para configuraciones fijadas. Usa `TOOL_SURFACE=dynamic-2` solo cuando quieras intencionadamente la variante experimental find/execute.

Consulta [Conjunto de herramientas dinámico](/gitlab-mcp-server/es/tools/dynamic-tools/) para el flujo buscar, describir y ejecutar.

Para despliegues dinámicos donde los recursos y prompts dominan el contexto inicial, usa `CAPABILITY_SURFACE=minimal`. Mantiene `gitlab://workspace/roots` para descubrimiento de proyectos y omite recursos opcionales, prompts, guías de flujo y recursos de meta-schema. Esto no elimina los schemas del descubrimiento dinámico: usa `gitlab_describe_tools` para recuperar schemas exactos antes de llamar a `gitlab_execute_tool`. El valor por defecto sigue siendo `full`; hoy no se expone un modo schemas-only.
:::
...
| `--tool-surface`         | `meta`                       | Selector canónico del catálogo: `meta`, `individual`, `dynamic`, `dynamic-2` o `dynamic-3`                              |
| `--meta-tools`           | `true`                       | Flag de compatibilidad deprecado. Usa `--tool-surface=individual` en lugar de `--meta-tools=false`                      |
| `--capability-surface`   | `full`                       | Selector del catálogo de recursos y prompts: `full` o `minimal`; minimal omite recursos opcionales, meta-schema y prompts |
