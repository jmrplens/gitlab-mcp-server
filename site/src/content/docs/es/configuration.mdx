...
| `META_TOOLS`             | `true`               | Selector del catálogo de herramientas: `true` para meta-herramientas, `false` para herramientas individuales, o `dynamic` para la superficie de bajo consumo search/describe/execute                               |
| `TOOL_SURFACE`           | —                    | Selector explícito del catálogo: `meta`, `individual` o `dynamic`. Cuando se establece, sobrescribe `META_TOOLS`                                                                                                   |
...
Usa `TOOL_SURFACE=dynamic` para el catálogo visible más pequeño: solo se exponen `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo registro oculto de acciones de GitLab.
...
| `--tool-surface`         | —                            | Selector explícito del catálogo: `meta`, `individual` o `dynamic`. Sobrescribe `--meta-tools`                          |
