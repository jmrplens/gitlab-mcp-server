...
faq:
  ...
  - q: "¿Cuánto cuesta vigilar contra mi límite de peticiones de GitLab?"
    a: "Los vigilantes usan el token del propio suscriptor, con un tope de **10 vigilantes por servidor**. El peor caso — los diez al suelo de 5 segundos — son 120 peticiones por minuto, el 6% del límite por usuario de GitLab.com. Diez vigilantes degradados cuestan una petición por minuto. Un 429 de GitLab pausa todos los vigilantes con retroceso exponencial."
---
...
Las suscripciones se anuncian con `CAPABILITY_SURFACE=full` (el valor por defecto). La superficie `minimal` no registra los recursos de datos de GitLab a los que aplica una suscripción, así que tampoco anuncia la capacidad.

## ¿Qué se puede vigilar?

26 tipos de recurso de objeto único — objetos con un ciclo de vida que merece seguirse:
...
Los vigilantes usan el token de GitLab del propio suscriptor, con un tope de **10 vigilantes por servidor** (un servidor por token+URL en modo HTTP). El peor caso — diez vigilantes al suelo de 5s — son 120 peticiones/minuto; diez degradados cuestan una. Un 429 de GitLab pausa todos los vigilantes con retroceso exponencial (30s doblando hasta 5 minutos).
...
| Límite de peticiones, cupo de watchers lleno, o servidor apagándose | `-32000` (servidor ocupado — transitorio, reintente más tarde)                        |
