...
faq:
  ...
  - q: "¿Qué herramientas de asistente interactivo están disponibles?"
    a: "GitLab MCP Server proporciona cuatro asistentes de elicitación: `gitlab_interactive_issue_create` (título, descripción, etiquetas, confidencialidad), `gitlab_interactive_mr_create` (rama origen y destino, título, descripción, etiquetas, opciones de eliminar la rama origen y de squash), `gitlab_interactive_release_create` (nombre del tag, nombre de la release, descripción) y `gitlab_interactive_project_create` (nombre, descripción, visibilidad, inicialización del README, rama predeterminada). Los asistentes de issue, MR y release reciben el `project_id` de destino como parámetro de la herramienta, y cada uno confirma con el usuario antes de realizar la llamada final a la API de GitLab."
  - q: "¿Qué ocurre cuando un cliente no admite la elicitación?"
    a: "GitLab MCP Server degrada de forma elegante a las herramientas parametrizadas estándar. El asistente proporciona todos los parámetros directamente en una sola llamada en lugar de usar el flujo del asistente. La funcionalidad se preserva por completo — solo se pierde la experiencia paso a paso. Por ejemplo, ejecutar `issue.create` mediante `gitlab_execute_action` en la superficie predeterminada (o `gitlab_issue` con `action: create` cuando `GITLAB_MCP_TOOL_SURFACE=meta`) y pasar todos los campos a la vez produce el mismo resultado sin soporte de elicitación."
...
Como los formularios multi-ronda viajan dentro del resultado de la herramienta, la elicitación sigue funcionando sobre el [transporte HTTP sin estado](/gitlab-mcp-server/es/operations/http-server/#modo-sin-estado-stateless) predeterminado para clientes con protocolo `2026-07-28`. Los clientes con protocolo heredado necesitan un canal iniciado por el servidor, así que sobre HTTP sin estado reciben las alternativas no interactivas (avisos de error de los asistentes y `confirm` para acciones destructivas).
...
:::tip
Incluso sin soporte de elicitación, puedes usar las acciones de creación estándar (`issue.create` mediante `gitlab_execute_action` en la superficie predeterminada, o `gitlab_issue` con `action: create` cuando `GITLAB_MCP_TOOL_SURFACE=meta`) proporcionando todos los parámetros en una única llamada.
:::
