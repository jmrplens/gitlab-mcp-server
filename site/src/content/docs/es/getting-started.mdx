...
Para el catálogo visible estable más pequeño, establece `TOOL_SURFACE=dynamic` o `TOOL_SURFACE=dynamic-3`. El servidor expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo catálogo canónico de acciones de GitLab. `TOOL_SURFACE=dynamic-2` está disponible como candidato experimental A/B find/execute.
