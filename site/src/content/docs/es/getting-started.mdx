...
| `META_TOOLS`             | No          | Selector del catálogo: `true`, `false` o `dynamic` (por defecto `true`)                                       |
| `TOOL_SURFACE`           | No          | Selector explícito del catálogo: `meta`, `individual` o `dynamic`                                             |
...
Para el catálogo visible más pequeño, establece `TOOL_SURFACE=dynamic`. El servidor expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo registro oculto de acciones de GitLab.
