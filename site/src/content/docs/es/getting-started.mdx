...
```bash
# Sin instalar nada — los clientes lo lanzan directamente
...
Publicado como [`jmrplens-gitlab-mcp-server`](https://pypi.org/project/jmrplens-gitlab-mcp-server/) con el mismo modelo que `uv` y `ruff` (el nombre sin prefijo en PyPI es un registro vacío de una cuenta ajena, en proceso de reclamación PEP 541): cada wheel de plataforma lleva el binario nativo, el instalador lo coloca en la ruta de scripts como el propio comando, y no se ejecuta Python cuando corre el servidor.

<Tabs syncKey="pypkg">
<TabItem label="uvx">

```bash
# Sin instalación — los clientes pueden lanzarlo directamente
...
La configuración incluida reenvía estas variables al contenedor. **Tu host tiene que ponerlas antes en el entorno del plugin**: la §9.1 de Agent Plugins permite al cliente «heredar, omitir o sanear» las variables del entorno ambiente, y la spec no define ninguna forma portable de que un plugin referencie un secreto, así que el token no puede viajar dentro del propio `mcp.json`. Si el servidor devuelve un fallo de autorización, define `GITLAB_TOKEN` como documente tu host, o añádelo al bloque `env` del `mcp.json` local del plugin instalado.
...
:::caution[Cómo llega el token al servidor]
Agent Plugins solo expande `${PLUGIN_ROOT}` y `${PLUGIN_DATA}`; cualquier otro
`${...}` se queda literal, así que escribir `"GITLAB_TOKEN": "${GITLAB_TOKEN}"`
le entrega al servidor esa cadena exacta en vez de tu token. La spec no define
ninguna forma portable de referenciar un secreto, y permite que el cliente
herede, omita o sanee el entorno ambiente, de modo que cómo llega `GITLAB_TOKEN`
lo decide tu host: o hace visible la variable al subproceso del plugin, o pones
tú el valor en el bloque `env` de este fichero local. Ese fichero es tuyo, no
forma parte del plugin publicado — mantenlo fuera del control de versiones.
:::
