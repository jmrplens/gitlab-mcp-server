...
| `TOOL_SURFACE`           | No          | Selector canónico del catálogo: `meta`, `individual`, `dynamic`, `dynamic-2` o `dynamic-3` (por defecto `meta`)                             |
...
Por defecto, el servidor funciona en **modo meta-herramientas** — en lugar de registrar cientos de herramientas individuales (hasta 1019 en GitLab.com Enterprise/Premium), las consolida en **33 meta-herramientas por dominio** (49 en Enterprise/Premium autoalojado, 50 en GitLab.com Enterprise/Premium con Orbit) que cubren el 100% de la funcionalidad.
...
Para el catálogo visible estable más pequeño, establece `TOOL_SURFACE=dynamic`. El servidor expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, respaldadas por el mismo catálogo canónico de acciones de GitLab. `TOOL_SURFACE=dynamic-3` selecciona explícitamente esa misma superficie de tres herramientas, y `TOOL_SURFACE=dynamic-2` está disponible como variante experimental find/execute.

Combina el modo dinámico con `CAPABILITY_SURFACE=minimal` para mantener solo workspace roots y omitir recursos opcionales, recursos de meta-schema, guías y prompts cuando necesites el contexto MCP inicial más pequeño. Dynamic conserva schemas inline mediante `gitlab_describe_tools`.
:::
