...
- **Herramientas dinámicas** buscan, describen y ejecutan las mismas entradas del catálogo con una lista visible mucho más pequeña.
...
Con `TOOL_SURFACE=meta` (predeterminado), el servidor expone una base de **33 meta-herramientas por dominio** en lugar del catálogo individual. Los entornos Enterprise/Premium autoalojados añaden 14 herramientas Enterprise/Premium para un total de 47, y GitLab.com Enterprise/Premium con Orbit (la funcionalidad Knowledge Graph de GitLab.com) añade una herramienta más para un total de 48. Cada meta-herramienta agrupa operaciones relacionadas:
...
Con `TOOL_SURFACE=individual`, se exponen todas las herramientas individuales (p. ej., `gitlab_list_issues`, `gitlab_create_issue`): 1006 en Enterprise/Premium autoalojado, o 1011 en GitLab.com Enterprise/Premium con Orbit. Esto puede ser útil para pruebas pero no se recomienda para producción.

### Conjunto de herramientas dinámico

Con `TOOL_SURFACE=dynamic`, el servidor expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`. El mismo catálogo de acciones canónico sigue disponible y se comparte con las meta-herramientas, así que el modo dinámico cambia el descubrimiento, no el comportamiento de GitLab.

```mermaid
flowchart TD
    specs[ActionSpecs de dominio] --> catalog[Catálogo de acciones canónico]
    catalog --> search[gitlab_search_tools]
    catalog --> describe[gitlab_describe_tools]
    catalog --> execute[gitlab_execute_tool]
    execute --> route[ActionRoute compartido]
    route --> handler[Handler tipado existente]
    handler --> gitlab[API de GitLab]
```

Las meta-herramientas siguen siendo el modo predeterminado hoy. El modo dinámico es la alternativa de bajo consumo de tokens y está documentado en [Conjunto de herramientas dinámico](/gitlab-mcp-server/es/tools/dynamic-tools/).
...
46 recursos MCP de solo lectura que proporcionan datos contextuales:
