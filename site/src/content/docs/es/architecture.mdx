...
Con `META_TOOLS=true` (predeterminado), el servidor expone **32 meta-herramientas por dominio** (47 con enterprise) en lugar de más de 1006 herramientas individuales. Cada meta-herramienta agrupa operaciones relacionadas:
...
Con `META_TOOLS=false`, se exponen todas las más de 1006 herramientas individuales (p. ej., `gitlab_list_issues`, `gitlab_create_issue`). Esto puede ser útil para pruebas pero no se recomienda para producción.
