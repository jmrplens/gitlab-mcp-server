...
Con `META_TOOLS=true` (predeterminado), el servidor expone una base de **32 meta-herramientas por dominio** en lugar del catálogo individual. Los entornos Enterprise/Premium autoalojados añaden 15 herramientas Enterprise/Premium para un total de 47, y GitLab.com Enterprise/Premium con Orbit (la funcionalidad Knowledge Graph de GitLab.com) añade una herramienta más para un total de 48. Cada meta-herramienta agrupa operaciones relacionadas:
...
Con `META_TOOLS=false`, se exponen todas las herramientas individuales (p. ej., `gitlab_list_issues`, `gitlab_create_issue`): 1006 en Enterprise/Premium autoalojado, o 1011 en GitLab.com Enterprise/Premium con Orbit. Esto puede ser útil para pruebas pero no se recomienda para producción.

### Conjunto de herramientas dinámico

Con `TOOL_SURFACE=dynamic`, el servidor expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`. El catálogo completo de acciones sigue disponible mediante un registro oculto construido a partir de los mismos mapas de rutas de meta-herramientas, así que el modo dinámico cambia el descubrimiento, no el comportamiento de GitLab.

```mermaid
flowchart TD
    A[Mapas de rutas de meta-herramientas] --> B[Registro dinámico oculto]
...
Las meta-herramientas siguen siendo el modo predeterminado hoy. El modo dinámico es el candidato de bajo consumo de tokens para un futuro predeterminado y está documentado en [Conjunto de herramientas dinámico](/gitlab-mcp-server/es/tools/dynamic-tools/).
