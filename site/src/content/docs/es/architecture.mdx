...
2. **Traduce** las llamadas en peticiones a la API REST v4 de GitLab con la autenticación adecuada
...
sequenceDiagram
    ...
    U1->>S: Petición MCP + Token A
    S->>P: Obtener/crear sesión para Token A
    P->>GL: Llamada API con Token A
    GL-->>P: Respuesta
    P-->>S: Resultado
    S-->>U1: Respuesta MCP

    U2->>S: Petición MCP + Token B
    S->>P: Obtener/crear sesión para Token B
...
- Aislamiento de sesión por token mediante pool LRU
...
Con `META_TOOLS=true` (predeterminado), el servidor expone **40 meta-herramientas a nivel de dominio** en lugar de más de 1000 herramientas individuales. Cada meta-herramienta agrupa operaciones relacionadas:
...
24 recursos MCP de solo lectura que proporcionan datos contextuales:
...
```json
{
  "structuredContent": {
    ...
    "next_steps": ["Ver detalles del issue", "Añadir etiquetas", "Asignar a usuario"]
  },
  "content": [
    {
      "type": "text",
