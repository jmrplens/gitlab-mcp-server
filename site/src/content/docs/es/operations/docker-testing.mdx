...
Ese objetivo ejecuta solo los tests seleccionados desde `test/e2e/suite/*_ee_test.go`, asi la cobertura CE/comun queda en `make test-e2e-docker` y las fixtures Enterprise pueden ajustarse de forma independiente. Define `E2E_DOCKER_ENTERPRISE_RUN` para sustituir el selector `go test -run` generado en un retry focalizado.
