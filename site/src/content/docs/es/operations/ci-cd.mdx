...
faq:
  ...
  - q: "¿Cómo bloqueo un script CI hasta que un pipeline de GitLab termine?"
    a: "Llama a la meta-herramienta `gitlab_pipeline` con la acción `wait`. Consulta el pipeline hasta que alcanza un estado terminal (success, failed o canceled), lo que permite que un script CI se bloquee hasta que un pipeline disparado termine. La meta-herramienta `gitlab_job` ofrece una acción `wait` equivalente para jobs individuales."
