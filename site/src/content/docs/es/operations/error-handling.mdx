...
<Tabs>
  <TabItem label="WrapErr">
    ...
    ```text
    list_issues: authentication failed: GitLab rejected the token (GITLAB_TOKEN) itself as invalid, expired or revoked, so renew or replace it
    ```
...
**HTTP Status**: 401 (unauthorized: either the token (GITLAB_TOKEN) is invalid or expired, or it is valid and lacks a permission this action needs, since some GitLab endpoints answer a missing permission with 401 rather than 403. If the token works for other calls, treat this as a permission refusal)
...
| **HTTP Status** | El código de estado de GitLab y su clasificación, cuando lo hubo    |
...
```text
❌ list_projects: authentication failed: GitLab rejected the token (GITLAB_TOKEN) itself as invalid, expired or revoked, so renew or replace it
💡 Generate a new token with api scope at GitLab → Preferences → Access Tokens
```

GitLab lo indica de forma explícita para un token expirado, revocado o de suplantación con la suplantación desactivada, y para cualquier 401 de su endpoint GraphQL, que es cuando el mensaje nombra solo el token.
