...
- El scope exigido es el menor privilegio con el que el despliegue puede funcionar: `read_api` con `--read-only` o `--safe-mode`, `api` en el resto de casos, anunciado en el `scopes_supported` de los metadatos. Un token `api` satisface ambos
