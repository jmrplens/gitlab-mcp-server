...
- Configura `--trusted-proxy-header` con la cabecera que tu proxy establece (ej. `Fly-Client-IP`, `X-Forwarded-For`) para que el rate limiter integrado vea las IPs reales de los clientes
