...
:::tip
Si solo necesitas operaciones de lectura, usa el scope `read_api`: el servidor lo detecta al arrancar y sirve solo las acciones de lectura, tanto en stdio como en modo HTTP. `GITLAB_MCP_READ_ONLY=true` mantiene en solo lectura también un token que podría escribir, como defensa en profundidad.
:::
...
:::caution[Para la imagen de contenedor usa Cosign 3.x]
Las firmas las produce Cosign 3.x, que las adjunta como referrers OCI 1.1. Cosign 2.x busca en su lugar la etiqueta antigua `sha256-<digest>.sig` y responde **`no signatures found`** sobre una imagen correctamente firmada, sin nada que indique que está mirando donde no es. Ese mensaje significa que tu cliente es demasiado antiguo, no que la imagen no esté firmada. Compruébalo con `cosign version` y actualiza si informa de una 2.x. La verificación del blob `checksums.txt` (paso 3) no se ve afectada.
:::
...
La imagen lleva además una atestación de SBOM, una por manifiesto de plataforma y no una sobre el índice, porque un índice no tiene contenido propio. `gh attestation verify` resuelve una etiqueta al índice y allí no encuentra ningún SBOM, así que verifícalo por el digest del manifiesto que ejecutas:

```bash
# SBOM: qué hay dentro de la imagen, un documento por plataforma
...
:::note
Las atestaciones de procedencia y de SBOM de la imagen empiezan en el primer release posterior al 3.0.0. Sobre el propio 3.0.0 los comandos `gh attestation verify` de arriba responden que no encuentran atestación: ese release está firmado pero no atestado. El comando de firma sí funciona sobre 3.0.0.
:::
