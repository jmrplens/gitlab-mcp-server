...
[Modo servidor HTTP](/gitlab-mcp-server/es/operations/http-server/) documenta el
...
[Docker](/gitlab-mcp-server/es/install/docker/). Lo que sigue es la forma de
despliegue.
...
El `HEALTHCHECK` de la propia imagen, desde la 2.8.0, es `gitlab-mcp-server
--probe`: el binario localiza el proceso del servidor dentro del contenedor,
lee `--http-addr`,
...
| Requisito                           | Por qué                                                                                                                                                                                                                   |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Declarar el host con `--public-url` | El proxy reenvía el `Host` del cliente, y un host que nadie declaró se rechaza con `403` por considerarse un intento de DNS rebinding. `--public-url` es lo que lo declara                                                |
...
raíz; los metadatos OAuth no, y el desafío `401` apunta a la forma derivada. La
forma escueta tampoco se sirve: es el documento del recurso que _es_ el origen,
y un servidor que vive bajo una ruta no lo es. Un proxy que solo enruta
`/gitlab` responde `404` al documento que todo cliente OAuth pide primero, y el
descubrimiento falla antes de llegar siquiera al endpoint MCP.
...
**Anuncia el drenaje antes de cerrar el listener.** Con `SIGTERM` el proceso se
...
- [Modo servidor HTTP](/gitlab-mcp-server/es/operations/http-server/) para cada opción, el pool de servidores y las reglas de derivación de OAuth
- [Seguridad](/gitlab-mcp-server/es/operations/security/) para el modelo de amenazas, CORS y la lista de comprobación de fortificación
- [OpenTelemetry](/gitlab-mcp-server/es/operations/telemetry/) para lo que una instancia puede contar sobre sí misma
- [Solución de problemas](/gitlab-mcp-server/es/operations/troubleshooting/) para síntomas y sus causas habituales
