    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "¿Cómo desactivo las actualizaciones automáticas en GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Establece AUTO_UPDATE=false en modo stdio o pasa --auto-update=false en modo HTTP para omitir todas las verificaciones de actualización. Para recibir aviso de nuevas versiones sin instalarlas, usa AUTO_UPDATE=check (o --auto-update=check), que detecta actualizaciones y registra su disponibilidad pero nunca reemplaza el binario. Los alias aceptados son 1/yes para true y 0/no para false, y el valor no distingue mayúsculas de minúsculas."
            }
          },
          {
            "@type": "Question",
            "name": "¿La actualización interrumpe el servidor en ejecución?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "No. GitLab MCP Server aplica las actualizaciones con un truco de renombrado: el binario en ejecución se renombra a .old y el nuevo binario se escribe en la ruta original mientras el proceso actual sigue sirviendo peticiones MCP. La nueva versión solo se usa tras reiniciar el servidor. La verificación al inicio es no bloqueante y no fatal, así que un timeout de red o un release ausente se registra como advertencia y nunca impide que el servidor arranque."
            }
          },
          {
            "@type": "Question",
            "name": "¿La actualización automática usa mi token o instancia de GitLab?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "No. La actualización automática solo se comunica con la API de GitHub Releases y es completamente independiente de tu configuración de GitLab. Los ajustes GITLAB_URL, GITLAB_TOKEN y GITLAB_SKIP_TLS_VERIFY no afectan a las verificaciones ni a las descargas. Por defecto obtiene releases de jmrplens/gitlab-mcp-server, que puedes sobrescribir con AUTO_UPDATE_REPO=propietario/repo."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cómo revierto GitLab MCP Server tras una actualización defectuosa?",
            "acceptedAnswer": {
              "@type": "Answer",
...
Establece `AUTO_UPDATE=false` en modo stdio o pasa `--auto-update=false` en modo HTTP para omitir todas las verificaciones de actualización. Para recibir aviso de nuevas versiones sin instalarlas, usa `AUTO_UPDATE=check` (o `--auto-update=check`), que detecta actualizaciones y registra su disponibilidad pero nunca reemplaza el binario. Los alias aceptados son `1`/`yes` para `true` y `0`/`no` para `false`, y el valor no distingue mayúsculas de minúsculas.

### ¿La actualización interrumpe el servidor en ejecución?

No. GitLab MCP Server aplica las actualizaciones con un **truco de renombrado**: el binario en ejecución se renombra a `.old` y el nuevo binario se escribe en la ruta original mientras el proceso actual sigue sirviendo peticiones MCP. La nueva versión solo se usa tras reiniciar el servidor. La verificación al inicio es no bloqueante y no fatal, así que un timeout de red o un release ausente se registra como advertencia y nunca impide que el servidor arranque ni acepte peticiones MCP.

### ¿La actualización automática usa mi token o instancia de GitLab?

No. La actualización automática solo se comunica con la **API de GitHub Releases** y es completamente independiente de tu configuración de GitLab. Los ajustes `GITLAB_URL`, `GITLAB_TOKEN` y `GITLAB_SKIP_TLS_VERIFY` no afectan a las verificaciones ni a las descargas. Por defecto obtiene releases de `jmrplens/gitlab-mcp-server`, que puedes sobrescribir con `AUTO_UPDATE_REPO=propietario/repo` para seguir un fork personalizado.

### ¿Cómo revierto tras una actualización defectuosa?
