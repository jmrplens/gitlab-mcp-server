...
Ambas fichas, y el documento RFC 9728 en `/.well-known/oauth-protected-resource`, se generan una vez al arrancar y se sirven sin cambios hasta que el proceso se reinicia. Las tres lo dicen dos veces: `Cache-Control: public, max-age=3600` es cuánto tiempo puede reutilizar una copia el cliente sin preguntar, y el `ETag` es lo que envía después para preguntar si esa copia sigue vigente.
