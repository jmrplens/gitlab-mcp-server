...
Cuando el servidor arranca con `--allow-any-gitlab-url` y no publica ninguna instancia, el cliente elige a qué instancia de GitLab dirigirse con la cabecera `GITLAB-URL`. Ahí es opcional: una petición sin ella usa `https://gitlab.com`. Cuando el servidor publica varias instancias la cabecera pasa a ser obligatoria, y una petición sin ella se rechaza.
