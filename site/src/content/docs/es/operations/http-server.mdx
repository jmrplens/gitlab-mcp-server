...
| HTTP, ocho credenciales activas       |        445–719 MiB | Picos de 599 MiB a 1,16 GiB mientras las ocho llamaban             |
...
**`--max-http-clients` es un ajuste de memoria.** Su valor por defecto de 100 entradas describe un pool mucho mayor de lo que cabe en una instancia pequeña, y `--session-timeout` (30m por defecto) junto con `--pool-idle-timeout` (1h por defecto) deciden cuánto tiempo conserva su memoria una entrada sin usar. Bájalos si vas justo de memoria y súbelos solo junto con más RAM.
