...
**La memoria sigue a las configuraciones y al trabajo concurrente, no al número de tokens.** El pool mantiene una entrada por cada par token+URL distinto, pero una entrada es un cliente de GitLab y su contabilidad, no un catálogo: el servidor MCP y su superficie registrada se construyen una vez por configuración y los comparten todas las credenciales cuya configuración coincide. Medido en reposo, pasar de una credencial a veinte hace crecer el heap vivo en 0,14 MiB en `dynamic`, 0,15 MiB en `meta` y 0,16 MiB en `individual`, frente a 8, 15 y 27 MiB antes de compartir. Las cifras de la tabla de arriba son anteriores a ese cambio y son una cota superior; los números con los que dimensionar son el propio proceso más lo que reserven las peticiones concurrentes mientras se atienden.
