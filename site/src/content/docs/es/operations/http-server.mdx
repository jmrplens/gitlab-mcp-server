...
| `--stateless`            | `false`                      | HTTP streamable sin sesiones (SEP-2567 / protocolo 2026-07-28): sin seguimiento de `Mcp-Session-Id`, cada POST es autónomo, GET/DELETE devuelven `405`                                                                         |
...
`--stateless` cambia el transporte HTTP streamable al modelo sin sesiones introducido por [SEP-2567](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2567) (protocolo MCP `2026-07-28`):
...
- Las peticiones iniciadas por el servidor quedan deshabilitadas — los flujos de elicitación MCP recurren a sus alternativas no interactivas (p. ej. el parámetro `confirm` para acciones destructivas).
