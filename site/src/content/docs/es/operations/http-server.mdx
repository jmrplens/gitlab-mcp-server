...
4. **Desalojo del pool**: Cuando se alcanza la capacidad, la entrada desalojable más antigua se elimina completamente, y se avisa a su cliente en lugar de dejarlo mudo: sus vigilantes se detienen, sus peticiones `subscriptions/listen` abiertas se completan con un resultado que nombra el final (`credential_evicted` por capacidad, `credential_revoked` por un token que GitLab rechazó, y [cinco más](/gitlab-mcp-server/es/capabilities/subscriptions/#por-qué-terminó-una-suscripción)) y, con `--stateless=false`, las sesiones que ningún stream terminó se cierran, de modo que su siguiente petición vuelve a inicializar

#### La palanca y lo que cuesta
...
Una entrada está ocupada mientras su credencial mantiene un stream `subscriptions/listen` abierto o al menos un vigilante. En el transporte sin estado por defecto tiene que mantener un stream, porque un `resources/subscribe` de la era de sesiones se rechaza allí salvo que llegue por un listen, y los streams abiertos están limitados a 512 por proceso. Un pool de 513 entradas o más no puede, por tanto, ponerse entero en estado ocupado, así que el recurso que se lleva la entrada de un suscriptor nunca se dispara. Eso cuesta unos 25 MiB de ocupación con los 50 KiB por entrada medidos, frente a un tope de 10000 para el flag.

Con `--stateless=false` todavía no hay tamaño de pool que lo haga inalcanzable: un subscribe de la era de sesiones crea un vigilante sin conexión retenida, y los vigilantes están limitados a diez por credencial y por nada a nivel de proceso. Un tope de vigilantes por proceso se sigue en la [issue 561](https://github.com/jmrplens/gitlab-mcp-server/issues/561).
