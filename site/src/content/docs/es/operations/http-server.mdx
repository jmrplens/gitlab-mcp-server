...
description: Despliega GitLab MCP Server como servicio HTTP multiusuario con autenticación por token en cada petición, pool de credenciales aisladas, OAuth y rate limiting.
...
- **Cada petición lleva su propio token de GitLab.** Cada credencial obtiene su entrada en un pool LRU acotado, indexado por un hash de token + URL de GitLab, y cada petición se ejecuta con el cliente de GitLab que lleva su entrada, así que los clientes nunca comparten estado.
- **Dimensiona por la carga concurrente, no por el número de tokens.** El proceso reposa cerca de 40 MiB sin ningún catálogo, y construye uno por cada configuración que se le pide atender, compartido por todas las credenciales de esa configuración. Una credencial propia añade un cliente de GitLab y algo de contabilidad; lo que crece con la gente que lo usa a la vez son las peticiones que tienen en vuelo.
...
| `--skip-tls-verify`             | `false`           | Omite la verificación del certificado TLS al llamar a GitLab (saliente; sin relación con `--tls-cert`). El modo OAuth lo rechaza para una instancia que no sea loopback, porque los tokens bearer se reenvían allí en cada llamada                                                                                                          |
...
| `--read-only`                   | `false`           | Modo solo lectura: elimina las operaciones que modifican, acción por acción; las lecturas siguen funcionando                                                                                                                                                                                                                                |
...
| `--exclude-tools`               | _(vacío)_         | Nombres de herramientas, separados por comas, que se excluyen del registro                                                                                                                                                                                                                                                                  |
| `--ignore-scopes`               | `false`           | Omite la detección de scopes del PAT y registra todas las herramientas que permita el catálogo configurado                                                                                                                                                                                                                                  |
| `--max-http-clients`            | `100`             | Máximo de entradas únicas token+URL en el pool del servidor; acota las entradas del pool, no las sesiones ni las peticiones concurrentes (límite: 10000)                                                                                                                                                                                    |
| `--session-timeout`             | `30m`             | Timeout de sesión MCP inactiva; solo se aplica con `--stateless=false`, porque con el transporte sin estado por defecto la sesión de cada POST termina con su respuesta (límite: 24h)                                                                                                                                                       |
...
| `--public-url`                  | _(vacío)_         | Origen https accesible desde fuera de este despliegue. **Obligatoria con `--auth-mode=oauth`**: es el identificador de recurso protegido de RFC 9728, y la URL de metadatos se deriva de él; consulta [Dónde viven los metadatos](#dónde-viven-los-metadatos-raíz-del-host-o-sub-ruta)                                                      |
...
| `--resource-policy-uri`         | _(vacío)_         | URL https publicada como `resource_policy_uri` de RFC 9728; vacío omite el campo                                                                                                                                                                                                                                                            |
| `--resource-tos-uri`            | _(vacío)_         | URL https publicada como `resource_tos_uri` de RFC 9728; vacío omite el campo                                                                                                                                                                                                                                                               |
...
| `--trusted-origins`             | _(vacío)_         | Orígenes absolutos (`esquema://host[:puerto]`), separados por comas, autorizados a hacer peticiones cross-origin desde un navegador; `*` acepta cualquier origen y desactiva la protección; vacío no añade ninguno. El origen de `--public-url` es de confianza automáticamente                                                             |
...
| `--revalidate-interval`         | `15m`             | Intervalo de revalidación del token; `0` detiene la comprobación periódica, pero una entrada cuya credencial tenga más de 1h se reconstruye igualmente (límite: 24h)                                                                                                                                                                        |
...
| `--trusted-proxies`             | _(vacío)_         | Direcciones o rangos CIDR de los proxies inversos cuya `--trusted-proxy-header` se cree (ej. `127.0.0.1,10.0.0.0/8`); desde cualquier otro origen la cabecera se ignora. Obligatoria junto a `--trusted-proxy-header`                                                                                                                       |
| `--trusted-proxy-header`        | _(vacío)_         | Cabecera HTTP con la IP real del cliente (ej. `CF-Connecting-IP`, `X-Forwarded-For`), para que el limitador de fallos de autenticación cargue a quien llama y no al proxy; solo se cree desde `--trusted-proxies`, obligatoria junto a esta                                                                                                 |
...
| `--log-level`                   | `info`            | Verbosidad del registro: `debug`, `info`, `warn` o `error`. Fija `GITLAB_MCP_LOG_LEVEL`                                                                                                                                                                                                                                                     |
| `--client-compat`               | `auto`            | Compatibilidad de respuesta por cliente, `auto` u `off`. Fija `GITLAB_MCP_CLIENT_COMPAT`; consulta [Compatibilidad de clientes](/gitlab-mcp-server/es/compatibility/)                                                                                                                                                                       |
| `--upload-max-file-size`        | `2GB`             | Tamaño máximo para las herramientas de subida y de lectura de ficheros. Fija `GITLAB_MCP_UPLOAD_MAX_FILE_SIZE`                                                                                                                                                                                                                              |
| `--yolo-mode`                   | `false`           | Omite la confirmación en las acciones destructivas. Fija `GITLAB_MCP_YOLO_MODE`                                                                                                                                                                                                                                                             |
| `--description-substitutions`   | _(vacío)_         | Reescribe las descripciones y títulos listados para validadores de gateway estrictos: pares `viejo=nuevo` separados por comas. Fija `GITLAB_MCP_DESCRIPTION_SUBSTITUTIONS`                                                                                                                                                                  |
| `--pprof-addr`                  | _(vacío)_         | Sirve los manejadores de perfilado de Go (`net/http/pprof`) en esta dirección de loopback, en un listener propio; un host que no sea loopback se rechaza al arrancar. Fija `GITLAB_MCP_PPROF_ADDR`                                                                                                                                          |
| `--telemetry`                   | `false`           | Exporta trazas, métricas y logs de OpenTelemetry por OTLP al colector que nombren las variables estándar `OTEL_EXPORTER_OTLP_*`; consulta [OpenTelemetry](/gitlab-mcp-server/es/operations/telemetry/)                                                                                                                                      |
| `--telemetry-identity`          | `none`            | Qué registra la telemetría sobre quien llama: `none`, `pseudonymous` o `full`                                                                                                                                                                                                                                                               |
| `--telemetry-identity-rotation` | _(vacío)_         | Cuánto vive una clave de seudonimización generada, p. ej. `24h`; vacío la mantiene lo que dure el proceso. Se ignora cuando `GITLAB_MCP_TELEMETRY_IDENTITY_KEY` está definida                                                                                                                                                               |
| `--telemetry-tool-name`         | `auto`            | Si `gen_ai.tool.name` es una dimensión de métrica: `auto` (activa en `dynamic` y `meta`, inactiva en `individual`), `on` u `off`                                                                                                                                                                                                            |

Las opciones compartidas con el modo stdio (`--transport`, `--env-file`, `--probe`, `--shutdown`, `--tool-search`, `--version`) están documentadas en la [referencia de la CLI](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/reference/cli.md) y no se repiten aquí.
...
:::note
El ciclo de vida de sesión MCP (el intercambio `Mcp-Session-Id`, la expiración por inactividad con `--session-timeout` y la terminación con DELETE) aplica solo al **modo stateful heredado** (`--stateless=false`). En el modo sin estado por defecto no hay sesiones MCP — el pool de credenciales descrito abajo sigue aplicando, porque sus entradas se indexan por token y URL de GitLab, no por sesión.
:::

### Arquitectura del pool de credenciales
...
graph TD
    subgraph "Arquitectura del Modo HTTP"
        ...
        HANDLER --> POOL[Pool de Credenciales]

        POOL --> ENTRY1["hash(glpat-aaa + gitlab.com)<br/>Cliente GitLab + estado de credencial"]
        POOL --> ENTRY2["hash(glpat-bbb + gitlab.com)<br/>Cliente GitLab + estado de credencial"]
        POOL --> ENTRY3["hash(glpat-aaa + self-hosted)<br/>Cliente GitLab + estado de credencial"]

        ENTRY1 --> SHARED["Servidor MCP<br/>uno por configuración"]
        ENTRY2 --> SHARED
        ENTRY3 --> SHARED
    end
...
4. **Desalojo del pool**: Cuando se alcanza la capacidad, la entrada desalojable más antigua se elimina completamente, y se avisa a su cliente: las suscripciones abiertas se terminan en lugar de quedarse mudas
...
<TabItem label="Varias instancias">

```bash
# Publica varias instancias; los clientes eligen una por petición con la cabecera GITLAB-URL, que pasa a ser obligatoria
docker run -d \
  --name gitlab-mcp \
  --read-only \
  --tmpfs /tmp:rw,size=64m \
...
Dimensiona el host por el conjunto residente del servidor HTTP, no por el tamaño del binario, y hazlo **por el trabajo en vuelo**. Estas cifras vienen del [consumo de recursos](/gitlab-mcp-server/es/operations/resource-benchmark/), que mide el binario real en ambos transportes e indica la máquina en la que se ejecutó. La fila por credencial es anterior al servidor compartido y es una cota superior; lo que cuesta una credencial en reposo está tres líneas más abajo:

| Modo                                  | Conjunto residente | Notas                                                              |
| ------------------------------------- | -----------------: | ------------------------------------------------------------------ |
| HTTP, en reposo, sin credenciales     |            ~40 MiB | El proceso no tiene ningún catálogo hasta que una petición lo pide |
| HTTP, por credencial activa adicional |          35–71 MiB | Medido cuando cada credencial construía su propio catálogo         |
...
**La memoria sigue a las configuraciones y al trabajo concurrente, no al número de tokens.** El pool mantiene una entrada por cada par token+URL distinto, pero una entrada es un cliente de GitLab y su contabilidad, no un catálogo: el servidor MCP y su superficie registrada se construyen una vez por configuración y los comparten todas las credenciales cuya configuración coincide. Medido en reposo y sin ninguna conexión abierta, pasar de una credencial a veinte hace crecer el heap vivo en 0,14 MiB en `dynamic`, 0,15 MiB en `meta` y 0,16 MiB en `individual`, frente a 8, 15 y 27 MiB antes de compartir. Eso es lo que cuesta la credencial por sí sola; una credencial que mantiene una conexión cuesta además los búferes que hay detrás, y eso lo mide el [consumo de recursos](/gitlab-mcp-server/es/operations/resource-benchmark/), que publica una pendiente por credencial mayor. Las cifras de la tabla de arriba son anteriores a ese cambio y son una cota superior; los números con los que dimensionar son los del benchmark, más lo que reserven las peticiones concurrentes mientras se atienden.

**`--max-http-clients` acota las credenciales del pool; `--pool-idle-timeout` las recupera.** El primero vale 100 entradas por defecto y el segundo (`1h` por defecto) decide cuánto tiempo se conserva una entrada sin usar, salvo que esté atendiendo una suscripción, en cuyo caso no se considera inactiva. Ninguno es ya el mando de memoria que era, porque una entrada no lleva un catálogo, pero ambos siguen acotando cuántos clientes de GitLab y vigilantes vivos mantiene el proceso. `--session-timeout` es otra cosa distinta, aunque lo parezca: acota una sesión MCP inactiva, solo se aplica con `--stateless=false` (con el transporte sin estado por defecto, una sesión termina con la respuesta a su propio POST) y terminar una sesión no libera la entrada del pool que hay detrás.

**La superficie de herramientas cambia las respuestas más que la memoria.** Las tres construyen el mismo catálogo canónico de acciones, y como el servidor se comparte por configuración, la superficie no dice nada sobre lo que cuesta una credencial más en reposo y sin conexión abierta: 7,7 KiB en `dynamic`, 8,3 KiB en `meta` y 8,5 KiB en `individual`, que es la misma cifra tres veces, porque lo que queda por credencial es su cliente de GitLab y su contabilidad, no sus herramientas. Una credencial que mantiene una conexión cuesta más que eso, por los búferes que hay detrás, y eso es lo que mide el [consumo de recursos](/gitlab-mcp-server/es/operations/resource-benchmark/). La vieja regla del «factor de dos, con `dynamic` la que más» describía un servidor por credencial y ya no describe nada. Bajo carga el orden lo encabeza `individual` con diferencia, y eso son las peticiones, no las credenciales. Lo que la superficie decide de verdad es el tamaño de un `tools/list`: 12 KB, 584 KB y 3,1 MB para `dynamic`, `meta` e `individual`. Elige la superficie por coste de tokens y tiempo de respuesta, y el tamaño de instancia por concurrencia.
...
Para la referencia técnica completa, consulta [`docs/guides/http-server-mode.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/guides/http-server-mode.md) en el repositorio.
:::
