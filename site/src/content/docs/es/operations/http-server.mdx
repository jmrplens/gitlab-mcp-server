...
- **Dimensiona por la carga concurrente, no por el número de tokens.** El proceso reposa entre 35 y 38 MiB sin ningún catálogo, y construye uno por cada configuración que se le pide atender, compartido por todas las credenciales de esa configuración. Una credencial del pool añade un cliente de GitLab y algo de contabilidad, medido en 30 a 53 KiB; lo que crece con la gente que lo usa a la vez son las peticiones que tienen en vuelo, a 0,81 a 4,27 MiB cada credencial. `--max-http-clients` acota lo primero, así que no es un ajuste de memoria.
...
| ----------------------------------------- | -----------------: | ---------------------------------------------------------------------- |
| HTTP, en reposo, sin credenciales         |        35 a 38 MiB | El proceso no tiene ningún catálogo hasta que una credencial lo pide   |
| HTTP, veinte credenciales, todas llamando |     257 a 1073 MiB | Medido por la serie de concurrencia, un proceso por superficie         |
| HTTP, cien credenciales, todas llamando   |      0,4 a 1,7 GiB | La misma serie, en su paso de cien credenciales                        |
| HTTP, por credencial adicional en el pool |        30 a 53 KiB | Heap vivo en reposo, la credencial mantenida y no llamando             |
| HTTP, por credencial adicional que llama  |    0,81 a 4,27 MiB | Pico de conjunto residente, con entre dos y cuatro peticiones en vuelo |
| stdio, un proceso por cliente             |      106 a 277 MiB | Empieza a construir su catálogo enseguida, así que no tiene reposo     |
