...
- **Dimensiona por la carga concurrente, no por el número de tokens.** El proceso reposa entre 36 y 40 MiB sin ningún catálogo, y construye uno por cada configuración que se le pide atender, compartido por todas las credenciales de esa configuración. Una credencial del pool añade un cliente de GitLab y algo de contabilidad, medido en 29 a 52 KiB; lo que crece con la gente que lo usa a la vez son las peticiones que tienen en vuelo, a 0,84 a 4,05 MiB cada credencial. `--max-http-clients` acota lo primero, así que no es un ajuste de memoria.
...
Dimensiona el host por el conjunto residente del servidor HTTP, no por el tamaño del binario, y hazlo **por el trabajo en vuelo**. Estas cifras vienen del [consumo de recursos](/gitlab-mcp-server/es/operations/resource-benchmark/), que mide el binario real en ambos transportes e indica la máquina en la que se ejecutó: un Intel i5-14400 con 16 CPU lógicas y 62 GiB de RAM, kernel 6.12, Go 1.27.1.

| Modo                                      | Conjunto residente | Notas                                                                  |
| ----------------------------------------- | -----------------: | ---------------------------------------------------------------------- |
| HTTP, en reposo, sin credenciales         |        36 a 40 MiB | El proceso no tiene ningún catálogo hasta que una credencial lo pide   |
| HTTP, ocho credenciales activas           |      113 a 267 MiB | Picos de 208 a 647 MiB mientras las ocho llamaban                      |
| HTTP, cien credenciales, todas llamando   |      0,5 a 2,0 GiB | Medido por la serie de concurrencia, un proceso por superficie         |
| HTTP, por credencial adicional en el pool |        29 a 52 KiB | Heap vivo en reposo, la credencial mantenida y no llamando             |
| HTTP, por credencial adicional que llama  |    0,84 a 4,05 MiB | Pico de conjunto residente, con entre dos y cuatro peticiones en vuelo |
| stdio, un proceso por cliente             |      108 a 254 MiB | Empieza a construir su catálogo enseguida, así que no tiene reposo     |
| Binario en disco                          |             ~55 MB | Binario estático único, sin dependencias de runtime                    |

De aquí se derivan tres cosas.

**La memoria sigue al trabajo concurrente, no al número de tokens.** El pool mantiene una entrada por cada par token+URL distinto, pero una entrada es un cliente de GitLab y su contabilidad, no un catálogo: el servidor MCP y su superficie registrada se construyen una vez por configuración y los comparten todas las credenciales cuya configuración coincide. La serie de concurrencia del benchmark mide lo que cuesta mantenerlas en cada paso hasta mil credenciales y lo sitúa en 50,9 KiB cada una en `dynamic`, 52,1 en `meta` y 29,1 en `individual`, de modo que mil credenciales admitidas mantienen menos de 120 MiB de heap vivo en cualquier superficie. Los escenarios puntuales de ocho credenciales dicen lo mismo desde el otro extremo: 0, 1 y -3 MiB por credencial adicional, es decir, nada por encima del ruido de una lectura de conjunto residente. Lo que crece son las peticiones en vuelo.
...
**La superficie de herramientas cambia las respuestas más que la memoria.** Las tres construyen el mismo catálogo canónico de acciones, y como el servidor se comparte por configuración, la superficie casi no dice nada sobre lo que cuesta una credencial más: 51, 52 y 29 KiB en `dynamic`, `meta` e `individual`, en superficies cuyo número de herramientas registradas difiere en un factor de quinientos. La vieja regla del «factor de dos, con `dynamic` la que más» describía un servidor por credencial y ya no describe nada. Bajo carga las superficies sí se separan, y tampoco por número de herramientas: `meta` es la más barata por credencial que llama, 0,84 MiB frente a 4,05 y 4,00, porque lo que queda una vez compartido el catálogo es lo que cada superficie reserva mientras responde a una llamada. Lo que la superficie decide de verdad es el tamaño de un `tools/list`: 12 KB, 599 KB y 3,2 MB para `dynamic`, `meta` e `individual`. Elige la superficie por coste de tokens y tiempo de respuesta, y el tamaño de instancia por concurrencia.
