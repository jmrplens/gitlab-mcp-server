...
- **Conexión HTTP inactiva** (`--http-idle-timeout`, por defecto `0` = desactivado): cuánto tiempo mantiene el `http.Server` una conexión keep-alive inactiva antes de cerrarla.

El servidor habla el transporte moderno **Streamable HTTP**, que usa Server-Sent Events (`text/event-stream`) para las respuestas en streaming y el stream independiente que transporta las notificaciones iniciadas por el servidor y los pings de keep-alive. Esas son respuestas HTTP de larga duración, así que el `IdleTimeout` y `WriteTimeout` HTTP — no `--session-timeout` — deciden si sobreviven.

Como `--http-idle-timeout` vale `0` (desactivado) por defecto, la capa HTTP no cierra las conexiones inactivas de fábrica, y `--session-timeout` es la vida efectiva de inactividad. El timeout de escritura de respuesta se acopla automáticamente a al menos el timeout de inactividad, de modo que los streams SSE nunca se cortan antes de la fecha límite de inactividad. Pon un `--http-idle-timeout` bajo solo si quieres reciclar antes las conexiones inactivas.
