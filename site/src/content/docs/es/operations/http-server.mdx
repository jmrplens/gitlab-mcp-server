...
- `dynamic`: la superficie actual de bajo consumo con dos herramientas: `gitlab_find_action` y `gitlab_execute_tool`.

`--capability-surface` controla recursos y prompts de forma independiente a las herramientas: `full` registra todos los recursos, recursos de meta-schema, guías de flujo y prompts, mientras que `minimal` conserva el recurso de roots del workspace, omite prompts y recursos opcionales, y mantiene recursos de meta-schema solo para `--tool-surface=meta`. El descubrimiento de schemas dinámicos sigue funcionando con `minimal` porque find devuelve schemas inline.

`--meta-param-schema` solo afecta los schemas visibles de meta-herramientas de dominio. Mantén `opaque` salvo que un cliente no pueda leer recursos de schema y necesite `compact` o `full` en `tools/list`.
