...
Cada tarjeta nombra los dominios del catálogo que usa. En la superficie dinámica predeterminada el asistente localiza y ejecuta sus acciones con `gitlab_find_action` y `gitlab_execute_action`; con `GITLAB_MCP_TOOL_SURFACE=meta` cada dominio es una meta-herramienta `gitlab_<dominio>`.
