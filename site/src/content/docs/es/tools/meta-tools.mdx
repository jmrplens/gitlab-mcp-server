...
La respuesta del schema por acción describe solo `params`. La llamada final a la herramienta sigue usando el sobre de meta-herramienta con `action` y `params`. Estos recursos se omiten cuando `CAPABILITY_SURFACE=minimal` está habilitado. Los despliegues dinámicos pueden seguir usando `gitlab_describe_tools` o `gitlab_find_action` para schemas inline; los despliegues de meta-herramientas pueden mantener `CAPABILITY_SURFACE=full` o configurar `META_PARAM_SCHEMA=compact`/`full` para incluir schemas en `tools/list`. Las métricas actuales muestran que `compact` es 6.5x más grande que `opaque`, y `full` es 11.9x más grande.
...
| `CAPABILITY_SURFACE` | `full`         | Selector del catálogo de recursos y prompts: `full` o `minimal`. Minimal mantiene solo `gitlab://workspace/roots` y omite recursos opcionales, meta-schema, guías y prompts. |
| `META_PARAM_SCHEMA`  | `opaque`       | Controla cuánto schema de `params` por acción se incluye en `tools/list`: `opaque`, `compact` o `full`. Los recursos de schema requieren `CAPABILITY_SURFACE=full`.          |
