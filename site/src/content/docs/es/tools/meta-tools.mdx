...
Para el modo de descubrimiento progresivo con menor consumo de tokens, consulta [Conjunto de herramientas dinámico](/gitlab-mcp-server/es/tools/dynamic-tools/). El modo dinámico reutiliza el mismo catálogo de rutas de meta-herramientas detrás de `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`.
...
Algunos clientes de IA imponen límites en el número de herramientas (por ejemplo, JetBrains AI Assistant limita los servidores MCP a 100 herramientas). El modo meta-herramientas (32 herramientas base, 47 herramientas enterprise autoalojadas o 48 herramientas GitLab.com Enterprise con Orbit) funciona dentro de estas restricciones. Si desactivas las meta-herramientas (`META_TOOLS=false`), los clientes con dichos límites solo verán un subconjunto del conjunto completo de herramientas individuales.
:::
...
Consulta [Conjunto de herramientas dinámico](/gitlab-mcp-server/es/tools/dynamic-tools/) para la superficie de tres herramientas: buscar, describir y ejecutar.

`dynamic` es la superficie dinámica recomendada de bajo consumo para uso normal. `dynamic-3` selecciona explícitamente esa misma superficie actual de tres herramientas para informes A/B o configuraciones fijadas. `dynamic-2` es una comparación experimental find/execute y puede cambiar. `TOOL_SURFACE` puede seleccionar cualquiera de estos valores explícitamente y sobrescribe `META_TOOLS` cuando ambos están definidos.

Las meta-herramientas y el modo dinámico consumen el mismo catálogo de acciones canónico. Las meta-herramientas lo exponen como dispatchers por dominio con un parámetro `action`; el modo dinámico lo expone mediante herramientas de búsqueda, descripción y ejecución. Por eso ambos modos comparten los mismos schemas, protecciones de acciones destructivas, filtrado de solo lectura, previsualizaciones de safe mode, filtrado por scopes del token y formato de resultados.
