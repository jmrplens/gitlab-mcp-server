...
| `GITLAB_MCP_META_TOOLS`         | heredado          | Selector de compatibilidad deprecado, leído solo cuando `GITLAB_MCP_TOOL_SURFACE` no está definido: `true` mapea a `meta`, `false` a `individual`, y `dynamic` también se acepta.                                  |
...
| `--tier`                        | _(autodetectado)_ | Flag de edición en modo HTTP: `free`/`ce`, `premium` o `ultimate`. Cuando no se pasa, se usa en su lugar `GITLAB_TIER` (o el deprecado `GITLAB_ENTERPRISE`) del entorno.                                           |
