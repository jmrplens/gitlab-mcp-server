    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "¿Qué son las meta-herramientas de GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Las meta-herramientas son un modo de operación explícito de GitLab MCP Server, habilitado con TOOL_SURFACE=meta. En lugar de exponer cada operación de la API de GitLab como una herramienta MCP separada, una meta-herramienta agrupa operaciones relacionadas bajo una única herramienta a nivel de dominio con un parámetro action que despacha al handler correcto. Por ejemplo, gitlab_issue gestiona list, get, create, update y delete a través de una sola herramienta. Cada operación de herramienta individual sigue disponible como una acción dentro de una de las meta-herramientas de dominio, así que la funcionalidad no cambia."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cuánto reducen las meta-herramientas el número de herramientas?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Las meta-herramientas reducen el recuento de herramientas registradas en más del 95% manteniendo el 100% de la funcionalidad. El modo individual puede registrar cientos de herramientas —hasta el total de GitLab.com Enterprise/Premium con Orbit—, mientras que el modo meta-herramientas expone un pequeño conjunto de herramientas de dominio base, más otras cuando se habilita el catálogo Enterprise. Cada operación individual sigue siendo accesible como una acción dentro de una meta-herramienta de dominio, así que no se pierde ninguna capacidad; solo cae drásticamente el overhead de tokens de la lista de herramientas."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cómo descubro los parámetros exactos de una acción de meta-herramienta?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Por defecto, META_PARAM_SCHEMA=opaque mantiene pequeño el schema de cada meta-herramienta: los clientes ven el enum válido de action, mientras params queda como un objeto específico de la acción. Para descubrir la forma exacta de una acción, lee el manifiesto de herramientas. gitlab://tools lista las herramientas visibles y entradas ejecutables de la superficie activa, y gitlab://tools/{id} devuelve la forma de llamada aceptada y el JSON Schema de una sola acción, como gitlab://tools/gitlab_merge_request.create. Estos recursos siguen disponibles incluso cuando CAPABILITY_SURFACE=minimal."
            }
          },
          {
            "@type": "Question",
            "name": "¿Cómo habilito las meta-herramientas Enterprise?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "El catálogo Enterprise/Premium habilita 16 meta-herramientas adicionales para funciones de GitLab Premium y Ultimate. En modo stdio, establece GITLAB_TIER=premium o GITLAB_TIER=ultimate; en modo HTTP, usa --tier=premium o --tier=ultimate para forzar el catálogo, u omítelo para permitir autodetección CE/EE por entrada token+URL. Los flags heredados GITLAB_ENTERPRISE=true y --enterprise siguen respetándose como alternativa cuando el tier no está definido, pero están deprecados. Habilitar el tier también añade rutas de acción solo enterprise a meta-herramientas base existentes como gitlab_issue, gitlab_project y gitlab_group."
            }
          },
          {
            "@type": "Question",
            "name": "¿Por qué algunos clientes de IA necesitan el modo meta-herramientas?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Algunos clientes de IA imponen límites en el número de herramientas; por ejemplo, JetBrains AI Assistant limita los servidores MCP a 100 herramientas. El modo meta-herramientas mantiene la lista visible de herramientas dentro de esas restricciones porque consolida cientos de operaciones en un número reducido de herramientas a nivel de dominio. Si en su lugar seleccionas herramientas individuales con TOOL_SURFACE=individual, los clientes con dichos límites solo verán un subconjunto del conjunto completo de herramientas individuales, ocultando algunas operaciones."
            }
          }
        ]
      }
...
Las meta-herramientas reducen el recuento de herramientas registradas en **más del 95%** manteniendo el 100% de la funcionalidad. El modo individual puede registrar cientos de herramientas —{stats.tools.free} en CE, hasta {stats.tools.gitlab_com} en GitLab.com Enterprise/Premium con Orbit—, mientras que el modo meta-herramientas expone {stats.meta.base} herramientas de dominio base (más con el catálogo Enterprise). Cada operación individual sigue siendo accesible como una acción dentro de una meta-herramienta de dominio, así que no se pierde ninguna capacidad; solo cae drásticamente el overhead de tokens de la lista de herramientas.
...
Algunos clientes de IA imponen límites en el número de herramientas; por ejemplo, JetBrains AI Assistant limita los servidores MCP a 100 herramientas. El modo meta-herramientas mantiene la lista visible de herramientas dentro de esas restricciones porque consolida cientos de operaciones en {stats.meta.base} herramientas de dominio base (o {stats.meta.self_managed_enterprise}/{stats.meta.gitlab_com} con el catálogo Enterprise). Si en su lugar seleccionas herramientas individuales con `TOOL_SURFACE=individual`, los clientes con dichos límites solo verán un subconjunto del conjunto completo de herramientas individuales, ocultando algunas operaciones.
