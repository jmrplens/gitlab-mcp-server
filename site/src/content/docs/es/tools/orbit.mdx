...
faq:
  ...
  - q: "¿Qué licencia y configuración requiere Orbit?"
    a: "Orbit requiere GitLab Ultimate en una conexión a GitLab.com. Actívalo con `GITLAB_TIER=ultimate` en modo stdio o `--tier=ultimate` en modo HTTP. Los flags heredados `GITLAB_ENTERPRISE=true` y `--enterprise` siguen funcionando como alternativa pero están deprecados. Cuando se cumplen estas condiciones, Orbit aparece como la meta-herramienta `gitlab_orbit` (seis acciones) o seis herramientas individuales `gitlab_orbit_*`."
