...
Orbit expone la API experimental Knowledge Graph de GitLab.com mediante **seis herramientas MCP de solo lectura** —`status`, `schema`, `tools`, `dsl`, `query` y `graph_status`—. Solo está disponible cuando el servidor se conecta a `https://gitlab.com` y el nivel resuelto es Premium o Ultimate (`GITLAB_TIER=premium` o `GITLAB_TIER=ultimate`, `--tier=premium` o `--tier=ultimate` en modo HTTP, o el nivel detectado desde la licencia de la instancia). La variable de entorno heredada `GITLAB_ENTERPRISE=true` sigue funcionando como alternativa cuando `GITLAB_TIER` no está definido, pero está deprecada; el modo HTTP lee ambas variables de su entorno cuando no se pasa `--tier`. Orbit nunca escribe: cada herramienta inspecciona salud, esquema o datos indexados, o ejecuta una consulta de solo lectura sobre el grafo.
