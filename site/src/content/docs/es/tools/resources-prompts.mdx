...
Por ejemplo, `gitlab://schema/meta/gitlab_merge_request/create` devuelve el schema de parámetros para la acción `create` de `gitlab_merge_request`. Estos recursos de schema están disponibles con `CAPABILITY_SURFACE=full` sin importar el modo de `META_PARAM_SCHEMA`. Se omiten con `CAPABILITY_SURFACE=minimal`; los clientes dinámicos deben usar `gitlab_describe_tools` o `gitlab_find_action` para schemas inline en ese modo.
...
| -------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------- |
| `CAPABILITY_SURFACE` | `full`         | `full` registra recursos, recursos de meta-schema, guías y prompts; `minimal` mantiene solo las raíces del workspace |
