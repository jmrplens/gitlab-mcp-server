...
:::note
MCP lista por separado los recursos fijos y las plantillas de URI. En modo full, los clientes que solo inspeccionan `resources/list` ven 9 URIs fijas: usuario actual, grupos, raíces del workspace, índice de meta-schema y 5 guías de flujo. Las 37 entradas restantes son plantillas de URI devueltas por `resources/templates/list` y se leen sustituyendo IDs de proyecto, grupo, issue, MR u otros valores en tiempo de ejecución.
:::
...
| `gitlab://schema/meta/`    | Catálogo de meta-herramientas registradas y sus acciones                                                                  |

### Plantilla de recurso de meta-esquema (1)
...
Por ejemplo, `gitlab://schema/meta/gitlab_merge_request/create` devuelve el schema de parámetros para la acción `create` de `gitlab_merge_request`. Estos recursos de schema están disponibles para meta-herramientas con `CAPABILITY_SURFACE=full` o `minimal`, sin importar el modo de `META_PARAM_SCHEMA`. Los clientes dinámicos que usan `CAPABILITY_SURFACE=minimal` deben usar `gitlab_find_action` para schemas inline.
...
| -------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CAPABILITY_SURFACE` | `full`         | `full` registra recursos, recursos de meta-schema, guías y prompts; `minimal` mantiene raíces del workspace y, para meta-herramientas, recursos de meta-schema |
