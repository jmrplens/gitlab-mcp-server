...
## Cómo llamar acciones de análisis

El catálogo de análisis está disponible desde cualquier superficie de herramientas:

| Superficie                | Cómo llamar análisis                                                                                                                                      |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dinámica predeterminada   | Usa `gitlab_find_action` para encontrar la acción canónica `analyze.*`, y luego llama a `gitlab_execute_action` con ese ID de acción y parámetros validados |
| Meta-herramientas         | Usa la meta-herramienta `gitlab_analyze` con acciones como `pipeline_failure`, `mr_security` o `release_notes`                                            |
| Herramientas individuales | Llama directamente a la herramienta especializada, como `gitlab_analyze_pipeline_failure`                                                                 |
...
En la superficie dinámica predeterminada, el descubrimiento devuelve schemas exactos antes de ejecutar. En modo meta-herramientas, `gitlab_analyze` agrupa las acciones de análisis detrás de un despachador de dominio. En modo individual, cada operación de análisis aparece como su propia herramienta MCP.
