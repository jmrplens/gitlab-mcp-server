...
| ------------------- | -------- | --------------------------------------------------------------------: | -------------------------------------------------------------------------: | -----------------------------------------------------------: |
| `dynamic` (predet.) | Cualquie |                                     {footprint.dynamic.visible_tools} |             {footprint.dynamic.tool_schema_tokens.toLocaleString("es-ES")} |                                                   referencia |
...
**Metodología.** Los recuentos usan el tokenizador <code>{footprint.tokenizer}</code> (la codificación de GPT-4 / GPT-3.5) mediante [tiktoken-go](https://github.com/tiktoken-go/tokenizer), medidos sobre el árbol de código de la v{stats.version} con el catálogo construido en memoria y sin llamadas de red. «Tokens de schema» cubre solo las definiciones de herramienta visibles —schemas de entrada, anotaciones y descripciones—, que es lo que el cliente paga en cada `tools/list`. Los recursos y prompts MCP añaden otros {footprint.shared.full.toLocaleString("es-ES")} tokens con `CAPABILITY_SURFACE=full`, o {footprint.shared.minimal.toLocaleString("es-ES")} con `minimal`, por igual en todas las superficies. Estas cifras se regeneran con `make gen-footprint`; la matriz completa de tier × superficie × modo de schema está en [la referencia de huella de tokens](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/token-footprint.md).
...
Esta página es la guía de usuario. Para el contrato autoritativo a nivel de campos y las notas de implementación, consulta [`docs/dynamic-tools.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/concepts/dynamic-tools.md) en el repositorio.
:::
