...
Los servidores MCP grandes pueden gastar mucho contexto solo en descubrimiento de herramientas antes de que el usuario pida nada. GitLab MCP Server puede exponer hasta 1019 operaciones individuales, e incluso el catálogo predeterminado de meta-herramientas anuncia entre 33 y 50 herramientas de dominio.
...
`CAPABILITY_SURFACE=minimal` mantiene `gitlab://workspace/roots` y omite recursos opcionales, prompts, guías de flujo y recursos de meta-schema. Dynamic conserva el descubrimiento de schemas porque `gitlab_describe_tools` devuelve los schemas exactos inline. `META_PARAM_SCHEMA` solo afecta los schemas de las meta-herramientas, así que deja el valor predeterminado `opaque` en despliegues dinámicos.

:::caution[Variantes dynamic]
`TOOL_SURFACE=dynamic` y `TOOL_SURFACE=dynamic-3` seleccionan la superficie estable de tres herramientas. Usa `dynamic` para configuración normal y `dynamic-3` solo cuando necesites fijar explícitamente la variante de tres herramientas.

`TOOL_SURFACE=dynamic-2` es una variante experimental de dos herramientas con `gitlab_find_action` más `gitlab_execute_tool`. No la uses en despliegues nuevos salvo que estés probando intencionadamente ese experimento.
:::
...
| --------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------- |
| Qué aparece en `tools/list` | 33 a 50 herramientas de dominio                             | 3 herramientas públicas de descubrimiento y ejecución                 |
