...
El modo dinámico mueve ese catálogo detrás de un registro oculto. El modelo descubre solo lo que necesita para la tarea actual.

```mermaid
flowchart LR
    A[tools/list] --> B[3 herramientas dinámicas públicas]
    B --> C[Buscar en registro oculto]
...
| Cómo elige el modelo        | Escoge una herramienta de dominio y una acción              | Busca en el registro oculto y después describe la acción              |
