...
<Facts>
	...
	<Fact kind="requires">`TOOL_SURFACE=individual` (o el legado `META_TOOLS=false`), y un cliente con presupuesto de contexto para sostener la lista completa.</Fact>
	<Fact kind="tier">
...
<Facts>
	...
	<Fact kind="requires">`TOOL_SURFACE=meta`. Las formas de llamada por acción siguen siendo descubribles vía `gitlab://tools` y `gitlab://tools/{id}` sea cual sea el valor de `META_PARAM_SCHEMA`.</Fact>
	<Fact kind="tier">
...
<Facts>
	...
	<Fact kind="requires">nada: es el modo por defecto cuando `TOOL_SURFACE` no está definido. `CAPABILITY_SURFACE=minimal` reduce aún más la huella compartida de recursos y prompts.</Fact>
	<Fact kind="tier">
...
:::note
La autoactualización del servidor se registra aparte cuando está habilitada y no cuenta en la cifra de {stats.meta.base} anterior. El modo meta añade una meta-herramienta `gitlab_server` cuyas acciones incluyen `status`, `health_check`, `check_update` y `apply_update`; el modo individual registra `gitlab_server_status`, `gitlab_server_check_update` y `gitlab_server_apply_update`; el modo dinámico alcanza esas mismas acciones a través del catálogo canónico de acciones.
:::
