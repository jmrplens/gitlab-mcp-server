...
El modo dinámico (`TOOL_SURFACE=dynamic`) es el candidato actual de bajo consumo de tokens para convertirse en predeterminado en el futuro. Expone solo `gitlab_search_tools`, `gitlab_describe_tools` y `gitlab_execute_tool`, mientras conserva las mismas acciones de GitLab detrás de un registro oculto. El asistente busca una acción, describe el schema exacto y luego ejecuta el ID canónico `domain.action`.
