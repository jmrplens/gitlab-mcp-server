...
faq:
  - q: "Does this server send telemetry to its maintainer?"
    a: "No, and there is no address to opt out of. Telemetry is off by default, and when you enable it the data goes to a collector **you** configure through the standard `OTEL_EXPORTER_OTLP_ENDPOINT` variable. If you set nothing, the exporters fall back to their own default, `https://localhost:4318`, your machine, where the export fails quietly unless you run a collector there. No default in any code path points anywhere else."
...
their percent-decoded spellings, out of every SDK diagnostic it emits.
...
The enumerating server card reports it, at the `.well-known` path. The SEP-2127 card at `/server-card` states identity and connection details only, and carries no capability blocks.
