...
<Tabs>
  ...
  <TabItem label="WrapErrWithHint">
    **Known corrective actions** — Used when the corrective action for a specific error is known. Appends an actionable suggestion:
    ...
    A hint names the action by its canonical ID (`branch.get_protected`), which every tool surface resolves, rather than by a tool name, which only one of them registers.
...
This pattern is applied through a shared formatter in each of 20 domains, so every get handler of a domain answers the same way. It logs at INFO level (expected outcome) and provides the AI assistant with actionable next steps.
