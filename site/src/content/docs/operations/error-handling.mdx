...
GitLab says so explicitly for an expired, revoked or impersonation-disabled token, and for any 401 from its GraphQL endpoint, which is when the message names the token alone.
