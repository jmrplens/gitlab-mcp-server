...
configure. It has no telemetry, no analytics, no crash reporting, and no
backend of its own.
...
The GitLab instance you configure is the only host the server contacts. There
is no update check, no licence key to validate, and no telemetry, so there are
no other network destinations at all.
