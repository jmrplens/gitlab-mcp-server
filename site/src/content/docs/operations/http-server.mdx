...
When the server starts with `--allow-any-gitlab-url` and publishes no instance, a client selects the GitLab instance to target with the `GITLAB-URL` header. It is optional there: a request without it falls back to `https://gitlab.com`. When the server publishes several instances the header is required instead, and a request without it is refused.
