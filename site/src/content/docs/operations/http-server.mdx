...
| `--allow-any-gitlab-url`        | `false`        | Start with no instance published, letting `GITLAB-URL` name any host. For a single-user local deployment; it is refused unless `--http-addr` binds a loopback address or a unix socket, and warns at startup even there                                                                                                                                                                                             |
...
GitLab restricts dynamically-registered OAuth clients to the `mcp` scope, which grants no REST or GraphQL access — every action here would fail. Create a GitLab OAuth Application with the `api` scope (`read_api` for a deployment that runs `--read-only` or `--safe-mode`, or for a client that should only read) and configure its Application ID in each client.

The deployment advertises that one scope in `scopes_supported`, and a client such as Claude Code asks GitLab for it. GitLab refuses an authorization request that names a scope the application does not have with `invalid_scope` ("The requested scope is invalid, unknown, or malformed"), and an existing application can be edited without changing its Application ID. A client that wants a read-only credential from a deployment that can write pins `read_api` (Claude Code's `oauth.scopes`, VS Code's `scopes`) against an application that has it; the token is admitted and served the read-only surface.
:::
...
4. **Pool eviction**: When capacity is reached, the oldest evictable entry is removed entirely, and its client is told rather than left silent: its watchers stop, its open `subscriptions/listen` requests are completed with a result naming the ending (`credential_evicted` for capacity, `credential_revoked` for a token GitLab refused, and [five more](/gitlab-mcp-server/capabilities/subscriptions/#why-a-subscription-ended)), and under `--stateless=false` the sessions that no stream ended are terminated, so its next request re-initializes
5. **A call GitLab refuses with `401`**: a `401` that names the token (the `invalid_token` code, or any `401` from GraphQL) removes the entry at once, ending its streams with `credential_revoked`. A plain `401` is not enough on its own, because GitLab also answers some missing permissions that way, such as approving a merge request you opened: the server first asks `GET /api/v4/user`, at most once every 30 seconds per credential, and keeps the entry, its subscriptions and its sessions when GitLab still accepts the token
...
<Aside type="tip" title="Replicas agree on the tag">
The tag is a hash of the document's own bytes and carries nothing about the process, so replicas serving the same configuration publish the same one. A client that revalidates against a different replica than the one that answered it first is still told 304. Nothing needs configuring for that, and nothing in front should replace the tag with a per-instance value.
</Aside>
