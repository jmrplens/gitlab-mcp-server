...
| HTTP, eight live credentials         |  445–719 MiB | Peaks of 599 MiB to 1.16 GiB while all eight were calling       |
...
**`--max-http-clients` is a memory setting.** Its default of 100 entries describes a pool far larger than a small instance can hold, and `--session-timeout` (default 30m) with `--pool-idle-timeout` (default 1h) decide how long an unused entry keeps its memory. Lower them if you are memory-constrained; raise them only alongside more RAM.
