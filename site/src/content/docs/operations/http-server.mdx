...
- **HTTP idle connection** (`--http-idle-timeout`, default `0` = disabled): how long the underlying `http.Server` keeps an idle keep-alive connection before closing it.

The server speaks the modern **Streamable HTTP** transport, which uses Server-Sent Events (`text/event-stream`) for streamed responses and the standalone stream that carries server-initiated notifications and keep-alive pings. Those are long-lived HTTP responses, so the HTTP `IdleTimeout` and `WriteTimeout` — not `--session-timeout` — decide whether they survive.

Because `--http-idle-timeout` defaults to `0` (disabled), the HTTP layer does not close idle connections out of the box, and `--session-timeout` is the effective idle lifetime. The response write timeout is automatically coupled to at least the idle timeout, so SSE streams are never cut before the idle deadline. Set a low `--http-idle-timeout` only if you want idle connections recycled sooner.
