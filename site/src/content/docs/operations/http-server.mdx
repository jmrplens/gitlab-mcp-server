...
- **`PRIVATE-TOKEN`** (required) — your GitLab Personal Access Token. It travels with each request and is never stored server-side.
- **`GITLAB-URL`** (optional) — target another GitLab instance; defaults to `https://gitlab.com`. Self-managed instances must be reachable from the public internet.
