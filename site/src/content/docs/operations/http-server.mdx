...
Both cards, and the RFC 9728 document at `/.well-known/oauth-protected-resource`, are rendered once at startup and served unchanged until the process restarts. All three say so twice: `Cache-Control: public, max-age=3600` is how long a client may reuse a copy without asking, and the `ETag` is what it sends back afterwards to ask whether that copy is still current.
