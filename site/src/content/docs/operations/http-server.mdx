...
**Memory tracks configurations and concurrent work, not the number of tokens.** The pool holds one entry per distinct token and GitLab URL, but an entry is a GitLab client and its bookkeeping, not a catalog: the MCP server and its registered surface are built once per configuration and shared by every credential whose configuration matches. Measured at rest, over twenty credentials, the twentieth adds 0.3 MiB on `dynamic`, 1.4 MiB on `meta` and 0.2 MiB on `individual` over the first, against 8, 15 and 27 MiB before the sharing. The figures in the table above predate that sharing and are an upper bound; the numbers to size against are the process itself plus whatever the concurrent requests allocate while they run.
...
**The tool surface changes responses more than memory.** All three build the same canonical action catalog, and since the server is shared per configuration the surface says little about what one more credential costs: at rest that is 17 KiB on `dynamic`, 73 KiB on `meta` and 8 KiB on `individual`, so the cheapest and the dearest are not the ones the old "factor of two, with `dynamic` the most" rule named. Under load the ordering is `individual` first by some way. What the surface really decides is the size of a `tools/list`: 12 KB, 584 KB and 3.1 MB for `dynamic`, `meta` and `individual`. Pick the surface for token cost and response time, and pick the instance size for concurrency.
