...
Size the host from the HTTP server's resident set, not from the binary. Measured on the v{stats.version} build (darwin/arm64, idle immediately after startup):
