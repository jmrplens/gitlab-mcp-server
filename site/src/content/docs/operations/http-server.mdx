...
| `--gitlab-url`             | _(optional)_                 | GitLab instance URL. Omit to require `GITLAB-URL` per request. **Repeatable** (or comma-separated) to publish several instances: the first is the default, all are listed in the RFC 9728 `authorization_servers` field, and `GITLAB-URL` then selects among them                           |
...
:::note
What `GITLAB-URL` means depends on how many instances the deployment publishes. **None**: the client chooses, falling back to `https://gitlab.com`. **One**: it is authoritative, the header is ignored and logged in `ignored_options`. **Several**: the header selects among the published instances, and a value naming anything else is **refused** — `403` in OAuth mode, `400` in legacy — never silently answered with someone else's instance. In OAuth mode every published instance must be `https`, because the bearer token is forwarded to whichever one the request selected.
:::
...
When the server starts without `--gitlab-url`, a client selects the GitLab instance to target with the `GITLAB-URL` header. It is optional: a request without it falls back to `https://gitlab.com`.
