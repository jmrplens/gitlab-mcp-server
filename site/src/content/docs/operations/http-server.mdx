...
`--stateless` switches the streamable HTTP transport to the sessionless model introduced by [SEP-2567](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2567) (MCP protocol `2026-07-28`):
...
- Server-initiated requests are disabled — MCP elicitation flows fall back to their non-interactive alternatives (e.g. the `confirm` parameter for destructive actions).
