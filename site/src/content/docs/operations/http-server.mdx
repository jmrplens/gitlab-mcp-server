...
- The per-token server pool still applies, so repeated requests reuse a cached server instance.
...
:::note
MCP session lifecycle (the `Mcp-Session-Id` handshake, idle expiry via `--session-timeout`, and DELETE termination) applies to **legacy stateful mode** (`--stateless=false`) only. In the default stateless mode there are no MCP sessions — the server pool below still applies, since pool entries are keyed by token, not by session.
:::
