...
- **Size for concurrent load, not for the number of tokens.** The process idles near 40 MiB holding no catalog, and builds one per configuration it is asked to serve, shared by every credential of that configuration. A credential of its own adds a GitLab client and a little bookkeeping; what grows with the number of people using it at once is the requests they have in flight.
...
**Memory tracks configurations and concurrent work, not the number of tokens.** The pool holds one entry per distinct token and GitLab URL, but an entry is a GitLab client and its bookkeeping, not a catalog: the MCP server and its registered surface are built once per configuration and shared by every credential whose configuration matches. Measured at rest with no connection open, going from one credential to twenty grows the live heap by 0.14 MiB on `dynamic`, 0.15 MiB on `meta` and 0.16 MiB on `individual`, against 8, 15 and 27 MiB before the sharing. That is the tenancy on its own; a credential holding a connection also costs the buffers behind it, which the [resource benchmark](/gitlab-mcp-server/operations/resource-benchmark/) measures and reports as a larger per-credential slope. The figures in the table above predate the sharing and are an upper bound; the numbers to size against are the benchmark's, plus whatever the concurrent requests allocate while they run.

**`--max-http-clients` bounds pooled credentials; `--pool-idle-timeout` reclaims them.** The first defaults to 100 entries, the second (default `1h`) decides how long an unused one is kept. Neither is the memory dial it used to be, since an entry no longer carries a catalog, but both still bound how many live GitLab clients and watchers the process holds. `--session-timeout` is a different thing again, despite sounding like it: it bounds an idle MCP session, it applies to `--stateless=false` only (under the default stateless transport a session ends with the response to its own POST), and ending a session does not release the pooled entry behind it.

**The tool surface changes responses more than memory.** All three build the same canonical action catalog, and since the server is shared per configuration the surface says nothing about what one more credential costs at rest, with no connection open: 7.7 KiB on `dynamic`, 8.3 KiB on `meta` and 8.5 KiB on `individual`, which is the same figure three times over, because a credential's residue is its GitLab client and its bookkeeping and not its tools. A credential holding a connection costs more than that, by the buffers behind it, which is what the [resource benchmark](/gitlab-mcp-server/operations/resource-benchmark/) measures. The old "factor of two, with `dynamic` the most" rule described a server per credential and no longer describes anything. Under load the ordering is `individual` first by some way, and that is the requests, not the tenancy. What the surface really decides is the size of a `tools/list`: 12 KB, 584 KB and 3.1 MB for `dynamic`, `meta` and `individual`. Pick the surface for token cost and response time, and pick the instance size for concurrency.
