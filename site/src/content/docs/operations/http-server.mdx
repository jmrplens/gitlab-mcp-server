...
**Memory tracks configurations and concurrent work, not the number of tokens.** The pool holds one entry per distinct token and GitLab URL, but an entry is a GitLab client and its bookkeeping, not a catalog: the MCP server and its registered surface are built once per configuration and shared by every credential whose configuration matches. Measured at rest, going from one credential to twenty grows the live heap by 0.14 MiB on `dynamic`, 0.15 MiB on `meta` and 0.16 MiB on `individual`, against 8, 15 and 27 MiB before the sharing. The figures in the table above predate that sharing and are an upper bound; the numbers to size against are the process itself plus whatever the concurrent requests allocate while they run.
...
**The tool surface changes responses more than memory.** All three build the same canonical action catalog, and since the server is shared per configuration the surface says nothing about what one more credential costs at rest: 7.7 KiB on `dynamic`, 8.3 KiB on `meta` and 8.5 KiB on `individual`, which is the same figure three times over, because a credential's residue is its GitLab client and its bookkeeping and not its tools. The old "factor of two, with `dynamic` the most" rule described a server per credential and no longer describes anything. Under load the ordering is `individual` first by some way, and that is the requests, not the tenancy. What the surface really decides is the size of a `tools/list`: 12 KB, 584 KB and 3.1 MB for `dynamic`, `meta` and `individual`. Pick the surface for token cost and response time, and pick the instance size for concurrency.
