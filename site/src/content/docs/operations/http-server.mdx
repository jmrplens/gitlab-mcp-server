...
**Memory tracks configurations and concurrent work, not the number of tokens.** The pool holds one entry per distinct token and GitLab URL, but an entry is a GitLab client and its bookkeeping, not a catalog: the MCP server and its registered surface are built once per configuration and shared by every credential whose configuration matches. Measured at rest, a twentieth credential adds well under a mebibyte on every tool surface. The figures in the table above predate that sharing and are an upper bound; the numbers to size against are the process itself plus whatever the concurrent requests allocate while they run.
