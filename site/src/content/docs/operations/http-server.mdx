...
flyctl scale memory 2048   # Resize memory (the fly.toml in this repo declares 10 GB)
```
...
The shipped `fly.toml` runs in **legacy auth mode** (PAT per request). OAuth mode is also supported but requires a stable public URL known at startup so the OAuth discovery endpoints (`/.well-known/oauth-protected-resource`) advertise the right metadata. To enable OAuth, set `--auth-mode=oauth` and `--gitlab-url=<your-default>` in the `cmd` array, then redeploy. See [`docs/guides/oauth-app-setup.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/guides/oauth-app-setup.md) for the GitLab OAuth Application setup.
...
**HTTP mode needs at least 512 MB.** A 256 MB instance is not enough: the server idles at roughly 181 MB before any client attaches, and each pooled session adds its own catalog and client on top. That is not a theoretical limit — a 256 MB Fly.io VM running v2.4.0 was OOM-killed during startup. Headroom above that floor is driven by concurrency: the pool holds up to `--max-http-clients` entries, each with its own MCP server instance, so peak memory tracks distinct token+URL pairs rather than request volume. The reference deployment in this repository declares `memory = "10gb"` on `shared-cpu-8x`, which is generous for that reason; size your own from your expected concurrency.
