...
:::tip
If you only need read operations, use the `read_api` scope: the server detects it at startup and serves only the read actions, on stdio as in HTTP mode. `GITLAB_READ_ONLY=true` keeps a token that could write read-only as well, for defense-in-depth.
:::
...
- Each token and GitLab URL pair gets its own isolated MCP server instance
...
- Pooled entries are bounded by `--max-http-clients` (default: 100), which caps token+URL entries rather than sessions or concurrent requests
