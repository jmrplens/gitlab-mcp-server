...
- Rejections are cheap and bounded by the two budgets above, and a token GitLab has already refused is refused from memory for five minutes rather than asked about again. Neither an outage nor a `429` from GitLab is ever cached, since those say nothing about the credential
