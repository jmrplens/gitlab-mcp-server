...
```bash
# Signature: who pushed this index
...
:::note
Image provenance attestations start with the first release after 3.0.0. On 3.0.0 itself the `gh attestation verify` commands above answer that no attestation was found: that release was signed but not attested. The signature command works on 3.0.0.
:::
