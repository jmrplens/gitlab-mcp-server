...
faq:
  - q: "What is the difference between read-only mode and safe mode?"
    a: "**Read-only mode** (`GITLAB_MCP_READ_ONLY=true`) does not register any mutating tools, so only list, get, and search operations are available — a hard guarantee at the server level. **Safe mode** (`GITLAB_MCP_SAFE_MODE=true`) still registers mutating tools but intercepts them and returns a structured JSON preview of the tool name, parameters, and annotations instead of executing. If both are set, read-only takes precedence and mutating tools are fully disabled."
  ...
  - q: "What token scopes should I use?"
    a: "Use the **minimum scopes** for your workflow: `read_api` for read-only operations, `api` for full create/update/delete operations, and `read_repository` or `write_repository` for repository file access or modification. If you only need read operations, use `read_api`: on startup the server detects your token's scopes, serves only the read actions to a token that cannot write, and disables tools that require scopes the token lacks. `GITLAB_MCP_READ_ONLY=true` keeps a token that could write read-only as well, for defense-in-depth."
...
:::tip
If you only need read operations, use the `read_api` scope: the server detects it at startup and serves only the read actions, on stdio as in HTTP mode. `GITLAB_MCP_READ_ONLY=true` keeps a token that could write read-only as well, for defense-in-depth.
:::
...
- If `GITLAB_MCP_READ_ONLY=true` is also set, it takes precedence (mutating tools are fully disabled)
...
:::caution[Use Cosign 3.x for the container image]
Signatures are produced by Cosign 3.x, which attaches them as OCI 1.1 referrers. Cosign 2.x looks for the older `sha256-<digest>.sig` tag instead and reports **`no signatures found`** on a correctly signed image, with nothing to say it is looking in the wrong place. That message means your client is too old, not that the image is unsigned. Check with `cosign version` and upgrade if it reports 2.x. Blob verification of `checksums.txt` (step 3) is unaffected.
:::
...
The image also carries an SBOM attestation, one per platform manifest rather than one over the index, because an index has no contents of its own. `gh attestation verify` resolves a tag to the index and finds no SBOM there, so verify it by the digest of the manifest you run:

```bash
# SBOM: what is inside the image, one document per platform
...
:::note
Image provenance and SBOM attestations start with the first release after 3.0.0. On 3.0.0 itself the `gh attestation verify` commands above answer that no attestation was found: that release was signed but not attested. The signature command works on 3.0.0.
:::
...
For the complete technical reference, see [`docs/concepts/security.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/concepts/security.md) in the repository.
:::
