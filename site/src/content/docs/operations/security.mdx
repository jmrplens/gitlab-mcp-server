    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "How does GitLab MCP Server protect my GitLab token in stdio mode?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "In stdio mode the GitLab token never leaves the local server process. It is loaded from the environment and used exclusively for GitLab API calls, never sent to the MCP client and never included in tool outputs. The server runs as a local process communicating over stdin/stdout, so no network ports are opened, and it only needs a token with the scopes required for the operations you intend to use."
            }
          },
          {
            "@type": "Question",
            "name": "What token scopes should I use with GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Use the minimum scopes for your workflow: read_api for read-only operations, api for full create/update/delete operations, and read_repository or write_repository for repository file access or modification. If you only need read operations, use read_api and also enable GITLAB_READ_ONLY=true for defense-in-depth. On startup the server detects the token scopes and disables tools requiring scopes the token lacks."
            }
          },
          {
            "@type": "Question",
            "name": "What is the difference between read-only mode and safe mode?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Read-only mode (GITLAB_READ_ONLY=true) does not register any mutating tools, so only list, get, and search operations are available — a hard guarantee at the server level. Safe mode (GITLAB_SAFE_MODE=true) still registers mutating tools but intercepts them and returns a structured JSON preview of the tool name, parameters, and annotations instead of executing. If both are set, read-only takes precedence and mutating tools are fully disabled."
            }
          },
          {
            "@type": "Question",
            "name": "How do I verify the integrity of a downloaded GitLab MCP Server binary?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Every GitHub Release ships checksums.txt (SHA-256 hashes) and checksums.txt.sigstore.json (a keyless Cosign/Sigstore signature bundle using GitHub OIDC). Auto-update verifies binaries against checksums.txt automatically. For manual installs, run cosign verify-blob with the bundle, pinning the certificate identity to this repository and the OIDC issuer to GitHub, then validate the binary hash against the signed checksum. If verification fails, do not run the binary."
            }
          }
        ]
      }
...
Use the **minimum scopes** for your workflow: `read_api` for read-only operations, `api` for full create/update/delete operations, and `read_repository` or `write_repository` for repository file access or modification. If you only need read operations, use `read_api` and also enable `GITLAB_READ_ONLY=true` for defense-in-depth. On startup the server detects your token's scopes and disables tools that require scopes the token lacks.

### What is the difference between read-only mode and safe mode?

**Read-only mode** (`GITLAB_READ_ONLY=true`) does not register any mutating tools, so only list, get, and search operations are available — a hard guarantee at the server level. **Safe mode** (`GITLAB_SAFE_MODE=true`) still registers mutating tools but intercepts them and returns a structured JSON preview of the tool name, parameters, and annotations instead of executing. If both are set, read-only takes precedence and mutating tools are fully disabled.
