...
- The scope demanded is the least privilege the deployment can work with: `read_api` under `--read-only` or `--safe-mode`, `api` otherwise, advertised in the metadata's `scopes_supported`. An `api` token satisfies both
