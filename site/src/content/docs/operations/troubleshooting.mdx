...
<Tabs>
  <TabItem label="VS Code / GitHub Copilot">
    ...
    | Waiting for `initialize` with Docker logs showing HTTP mode | Add `--http=false` after the image name for stdio mode, or configure the client as HTTP with `http://host:8080/mcp` |
  </TabItem>
