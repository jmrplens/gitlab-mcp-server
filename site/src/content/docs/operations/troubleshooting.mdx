...
| Tool not found in `tools/list`                              | Tool surface mismatch                     | Individual mode uses `gitlab_create_issue`, meta mode uses `gitlab_issue` with `action: create`, and dynamic mode exposes `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool` |
