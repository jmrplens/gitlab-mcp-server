...
| Waiting for `initialize` with Docker logs showing HTTP mode | Add `-i` to the `docker run` args so the container gets a pipe to speak on and infers stdio (JSON-RPC over stdin/stdout, no port needed). Alternatively, run the container in HTTP mode with `-p 8080:8080` and configure the MCP client as HTTP pointing to `http://host:8080/mcp` — this only works if the server is running and the port is reachable. |
  </TabItem>
