...
faq:
  - q: "Why does my MCP client show hundreds of tools?"
    a: "The client is using the **individual tool surface**, which registers one tool per GitLab operation. Set `TOOL_SURFACE=meta` to consolidate them into {{meta.base}} domain-level meta-tools, or use the default **dynamic** surface, which exposes only `gitlab_find_action` and `gitlab_execute_action`. Tool names also differ by surface: individual mode uses `gitlab_create_issue`, meta mode uses `gitlab_issue` with `action: create`, and dynamic mode uses find and execute."
  ...
  - q: "Why are Enterprise tools missing?"
    a: "The **Enterprise/Premium catalog is disabled**. In stdio mode, set `GITLAB_TIER=premium` or `GITLAB_TIER=ultimate`. In HTTP mode, set `--tier=premium` or `--tier=ultimate` to force the catalog, or let CE/EE auto-detection enable it when GitLab reports its edition. The legacy `GITLAB_ENTERPRISE=true` and `--enterprise` flags still work but are deprecated. Enterprise tools also require a Premium or Ultimate license on the connected instance."
