...
| Proxy works for `curl` but not for the server    | Env vars not exported into the server process | Pass them in `.env`, Docker `environment:`, or your platform's secrets                |
...
| `400`/`403` saying the deployment does not serve that GitLab instance     | `--gitlab-url` names more than one instance, so it publishes an allow-list         | Send a `GITLAB-URL` the server publishes (the error names them all), or omit the header to get the default. OAuth mode answers `403` before the token is used anywhere; legacy mode answers `400` |
