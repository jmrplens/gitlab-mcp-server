...
That target runs the `common` and `ee` packages of the suite under `test/e2e/gitlab` against the real server binary over stdio. There is no Enterprise build tag: every test file carries `e2e` alone, and the package decides the runtime, so `ee` refuses an unlicensed instance before it writes anything while `common` runs its Free actions on the licensed catalog too. The unlicensed half is `make test-e2e-ce`, which `make test-e2e-docker` is an older name for.
...
For manual Enterprise setup, replace the compose commands with the EE image, hand the activation code to the container, and set the edition flag during provisioning:
...
Every scenario runs on the dynamic, meta and individual surfaces as subtests, so one surface is driven on its own by naming the subtest rather than a test family:

```bash
go test -v -tags e2e -p 1 -timeout 2700s \
  -run '/dynamic$' \
  ./test/e2e/gitlab/common/
```
