...
That target runs only tests selected from `test/e2e/suite/*_ee_test.go`, so CE/common coverage stays in `make test-e2e-docker` and Enterprise fixtures can be tuned independently. Set `E2E_DOCKER_ENTERPRISE_RUN` to override the generated `go test -run` selector for a focused retry.
