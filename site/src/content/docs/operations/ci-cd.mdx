    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "Can I use gitlab-mcp-server in CI/CD without an LLM?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Yes. The deterministic mode sends JSON-RPC 2.0 messages directly to the server over stdio, with no LLM or external API involved. Each interaction performs an initialize handshake, sends a notifications/initialized message, then issues one or more tools/call requests, and you parse the result with jq. This mode is fully deterministic and is the right choice for scripted operations such as listing issues, posting comments, or creating releases."
            }
          },
          {
            "@type": "Question",
            "name": "Which GitLab token type should I use in CI/CD pipelines?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Use a Project Access Token scoped to a single project and store it as a masked CI/CD variable, never committed to the repository. Choose the api scope for full access or read_api for read-only workflows, set a maximum 90-day expiration, and rotate before expiry. For workflows that span several projects, use a Group Access Token instead."
            }
          },
          {
            "@type": "Question",
            "name": "How do I block a CI script until a GitLab pipeline finishes?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Call the gitlab_pipeline meta-tool with the wait action. It polls the pipeline until it reaches a terminal state (success, failed, or canceled), which lets a CI script block until a triggered pipeline completes. The gitlab_job meta-tool offers an equivalent wait action for individual jobs."
            }
          },
          {
            "@type": "Question",
            "name": "Should I use stdio or HTTP transport for gitlab-mcp-server in CI?",
            "acceptedAnswer": {
              "@type": "Answer",
...
Call the `gitlab_pipeline` meta-tool with the `wait` action. It polls the pipeline until it reaches a terminal state (success, failed, or canceled), which lets a CI script block until a triggered pipeline completes. The `gitlab_job` meta-tool offers an equivalent `wait` action for individual jobs.
