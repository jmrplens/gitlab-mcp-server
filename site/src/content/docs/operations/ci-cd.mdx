...
The server communicates via the [MCP protocol](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio) over stdin/stdout using JSON-RPC 2.0. Each interaction requires an `initialize` handshake, an `initialized` notification, then one or more `tools/call` requests.
