...
faq:
  ...
  - q: "How do I block a CI script until a GitLab pipeline finishes?"
    a: "Call the `gitlab_pipeline` meta-tool with the `wait` action. It polls the pipeline until it reaches a terminal state (success, failed, or canceled), which lets a CI script block until a triggered pipeline completes. The `gitlab_job` meta-tool offers an equivalent `wait` action for individual jobs."
