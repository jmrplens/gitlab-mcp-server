...
faq:
  ...
  - q: "What license and configuration does Orbit require?"
    a: "Orbit requires GitLab Ultimate on a GitLab.com connection. Enable it with `GITLAB_TIER=ultimate` in stdio mode or `--tier=ultimate` in HTTP mode. The legacy `GITLAB_ENTERPRISE=true` and `--enterprise` flags still work as a fallback but are deprecated. When these conditions are met, Orbit appears as the `gitlab_orbit` meta-tool (six actions) or six `gitlab_orbit_*` individual tools."
