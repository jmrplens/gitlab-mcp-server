...
Dynamic mode moves that catalog behind a hidden registry. The model discovers only what it needs for the current task.

```mermaid
flowchart LR
    A[tools/list] --> B[3 public dynamic tools]
    B --> C[Search hidden registry]
...
| How does the model choose?       | Pick a domain tool and action                       | Search the hidden action registry, then describe the action |
