...
Large MCP servers can spend a lot of context on tool discovery before the user asks anything. GitLab MCP Server can expose up to {stats.tools.gitlab_com} individual operations on GitLab.com Enterprise/Premium with Orbit, and the optional meta-tool catalog advertises {stats.meta.base} to {stats.meta.gitlab_com} domain tools.
