...
Yes. Dynamic mode reuses the same safety model as meta-tools. Destructive actions still require explicit confirmation unless your deployment has intentionally disabled prompts with `GITLAB_MCP_YOLO_MODE` or `AUTOPILOT`.
...
For safer deployments, use `GITLAB_MCP_READ_ONLY=true` to remove mutating actions or `GITLAB_MCP_SAFE_MODE=true` to preview mutations without applying them.
...
This page is the user-facing overview. For the authoritative field-level contract and implementation notes, see [`docs/concepts/dynamic-tools.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/concepts/dynamic-tools.md) in the repository.
:::
