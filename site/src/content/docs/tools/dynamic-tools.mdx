...
Large MCP servers can spend a lot of context on tool discovery before the user asks anything. GitLab MCP Server can expose up to 1034 individual operations on GitLab.com Enterprise/Premium with Orbit, and the optional meta-tool catalog advertises 33 to 50 domain tools.
