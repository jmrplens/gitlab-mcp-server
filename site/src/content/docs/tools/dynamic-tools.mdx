...
Search accepts `explain: true` when the assistant needs deterministic scoring reasons. The default response stays compact. No-match searches return a small suggestions list, and curated workflows may return `related_actions`, such as `repository.compare` before `analyze.release_notes`.
