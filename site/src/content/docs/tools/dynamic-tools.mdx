...
Large MCP servers can spend a lot of context on tool discovery before the user asks anything. GitLab MCP Server can expose up to 1019 individual operations, and the optional meta-tool catalog advertises 33 to 50 domain tools.
...
| Where are schemas found?         | Tool schema or `gitlab://schema/meta/...` resources | `gitlab_find_action`                            |
