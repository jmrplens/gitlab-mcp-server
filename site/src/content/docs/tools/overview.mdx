...
In meta-tool mode (`TOOL_SURFACE=meta`), related operations are consolidated into **domain-level meta-tools**. Each meta-tool accepts an `action` parameter that routes to the appropriate handler. This reduces the tool count to **33 base meta-tools**, **47 on self-managed Enterprise/Premium**, or **48 on GitLab.com Enterprise/Premium** when Orbit is available, dramatically improving LLM token efficiency while preserving full functionality.
...
When the Enterprise/Premium catalog is enabled, the server registers 14 additional self-managed meta-tools for GitLab Premium and Ultimate features:
