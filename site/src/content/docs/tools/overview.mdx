...
In individual mode, the server registers one tool per GitLab operation: **877 tools** for CE, **1039 tools** for self-managed Enterprise/Premium, or **1045 tools** on GitLab.com Enterprise/Premium when Orbit is available. This provides maximum granularity but consumes significant LLM context tokens for tool discovery.
