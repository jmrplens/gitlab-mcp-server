...
In individual mode, the server registers one tool per GitLab operation: **867 tools** for CE, **1030 tools** for self-managed Enterprise/Premium, or **1036 tools** on GitLab.com Enterprise/Premium when Orbit is available. This provides maximum granularity but consumes significant LLM context tokens for tool discovery.
