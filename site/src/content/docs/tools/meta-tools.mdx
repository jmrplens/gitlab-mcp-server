...
For the lowest-token progressive-discovery mode, see [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/). Dynamic mode reuses the same meta-tool route catalog behind `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`.

## Why meta-tools?

LLMs have limited context windows. When an MCP server registers hundreds of individual tools (up to 1011 on GitLab.com Enterprise/Premium), the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.
...
Some AI clients impose tool count limits (e.g., JetBrains AI Assistant limits MCP servers to 100 tools). Meta-tool mode (32 base tools, 47 self-managed enterprise tools, or 48 GitLab.com Enterprise tools with Orbit) works within these constraints. If you disable meta-tools (`META_TOOLS=false`), clients with such limits will only see a subset of the complete individual tool set.
:::
...
| -------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `META_TOOLS`         | `true`   | Tool catalog selector: `true` for meta-tools, `false` for individual tools, `dynamic` for the low-token dynamic surface, `dynamic-3` for the explicit current dynamic surface, or experimental `dynamic-2`. |
...
See [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/) for the three-tool search, describe, execute surface.

`dynamic` is the recommended low-token dynamic surface for normal use. `dynamic-3` explicitly selects that same current three-tool surface for A/B reports and pinned configurations. `dynamic-2` is an experimental find/execute comparison surface and may change. `TOOL_SURFACE` can select any of these values explicitly and overrides `META_TOOLS` when both are set.

Meta-tools and dynamic mode consume the same canonical action catalog. Meta-tools expose that catalog as domain dispatchers with an `action` parameter; dynamic mode exposes it through search, describe, and execute tools. Both modes therefore share the same schemas, destructive-action protections, read-only filtering, safe-mode previews, token-scope filtering, and result formatting.
