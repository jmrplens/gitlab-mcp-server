...
LLMs have limited context windows. When an MCP server registers hundreds of individual tools (up to {stats.tools.gitlab_com} on GitLab.com Enterprise/Premium with Orbit), the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.
