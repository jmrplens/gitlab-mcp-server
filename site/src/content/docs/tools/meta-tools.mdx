...
LLMs have limited context windows. When an MCP server registers hundreds of individual tools (up to 1034 on GitLab.com Enterprise/Premium with Orbit), the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.

| Mode              | Tool Count        | Token Overhead | Functionality           |
| ----------------- | ----------------- | -------------- | ----------------------- |
| Individual        | 866 / 1028 / 1034 | Very high      | Full                    |
...
The `gitlab_server` update helper can appear separately for server maintenance actions and is not included in the 32/48/49 GitLab action catalog counts.

:::note[Client compatibility]
Some AI clients impose tool count limits (e.g., JetBrains AI Assistant limits MCP servers to 100 tools). Meta-tool mode (32 base tools, 48 self-managed enterprise tools, or 49 GitLab.com Enterprise tools with Orbit) works within these constraints. If you select individual tools with `TOOL_SURFACE=individual`, clients with such limits will only see a subset of the complete individual tool set.
:::
