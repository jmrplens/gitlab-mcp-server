...
Some AI clients impose tool count limits (e.g., JetBrains AI Assistant limits MCP servers to 100 tools). Meta-tool mode (32 base tools, 47 self-managed enterprise tools, or 48 GitLab.com Enterprise tools with Orbit) works within these constraints. If you disable meta-tools (`META_TOOLS=false`), clients with such limits will only see a subset of the individual tool catalog.
:::
