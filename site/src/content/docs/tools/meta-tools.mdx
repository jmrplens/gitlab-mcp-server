...
faq:
  ...
  - q: "How do I enable Enterprise meta-tools?"
    a: "The Enterprise/Premium catalog raises the meta-tool count from {{meta.base}} to {{meta.self_managed_enterprise}} on a self-managed instance, adding tools for GitLab Premium and Ultimate features. In stdio mode, set `GITLAB_TIER=premium` or `GITLAB_TIER=ultimate`; in HTTP mode, use `--tier=premium` (or `--tier=ultimate`) to force the catalog, or omit it to allow CE/EE auto-detection per token+URL entry. The legacy `GITLAB_ENTERPRISE=true` environment variable is still honored in stdio mode as a fallback when the tier is unset, but is deprecated; HTTP mode reads `--tier` only. Enabling the tier also adds enterprise-only routes (such as iterations, project mirrors, and SSH certificates) to existing base meta-tools."
...
The Enterprise/Premium catalog enables {stats.meta.self_managed_enterprise - stats.meta.base} additional meta-tools that expose GitLab Premium and Ultimate features. In stdio mode, set `GITLAB_TIER=premium` or `GITLAB_TIER=ultimate`; in HTTP mode, use `--tier=premium` (or `--tier=ultimate`) to force the catalog, or omit it to allow CE/EE auto-detection per token+URL entry. The legacy boolean `GITLAB_ENTERPRISE=true` is still honored in stdio mode as a fallback when `GITLAB_TIER` is unset, but is deprecated. HTTP mode has no equivalent: it reads `--tier` and nothing else. Tier-gating also prunes per-field schema entries via `pruneSchemaFieldsByTier` (see `internal/tools/action_catalog.go`). Additionally, enterprise-only action routes are added to existing base meta-tools:
...
| ------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GITLAB_MCP_TOOL_SURFACE`       | `meta`          | Canonical selector for this mode: set `meta` to use meta-tools. Set `individual` only when you intentionally want one MCP tool per GitLab operation.                               |
| `GITLAB_MCP_META_TOOLS`         | legacy          | Deprecated compatibility selector: `true` maps to `meta`, and `false` maps to `individual` when `GITLAB_MCP_TOOL_SURFACE` is absent.                                               |
...
| `--tier`                        | _(auto-detect)_ | HTTP-mode edition flag: `free`/`ce`, `premium`, or `ultimate`. Replaces the deprecated `GITLAB_ENTERPRISE` toggle, which HTTP mode does not read.                                  |
