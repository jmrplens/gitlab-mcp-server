...
LLMs have limited context windows. When an MCP server registers hundreds of individual tools (up to 1019 on GitLab.com Enterprise/Premium), the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.

| Mode              | Tool Count        | Token Overhead | Functionality           |
| ----------------- | ----------------- | -------------- | ----------------------- |
| Individual        | 863 / 1014 / 1019 | Very high      | Full                    |
...
The per-action schema response describes only `params`. The final tool call still uses the meta-tool envelope with both `action` and `params`. These resources remain available for meta-tools when `CAPABILITY_SURFACE=minimal` is enabled, while optional resources, prompts, and workflow guides are omitted. Dynamic deployments can still use `gitlab_find_action` for inline schemas; meta-tool deployments can keep `META_PARAM_SCHEMA=opaque` and read these resources instead of inlining schemas in `tools/list`. Current audit metrics show `compact` is 6.5x larger than `opaque`, and `full` is 11.9x larger.
...
| `CAPABILITY_SURFACE` | `full`   | Resource and prompt catalog selector: `full` or `minimal`. Minimal keeps `gitlab://workspace/roots`, omits optional resources, guides, and prompts, and keeps meta-schema resources for meta-tools. |
| `META_PARAM_SCHEMA`  | `opaque` | Controls how much per-action `params` schema is inlined in `tools/list`: `opaque`, `compact`, or `full`. Meta-tool schema resources are available with `CAPABILITY_SURFACE=full` or `minimal`.      |
