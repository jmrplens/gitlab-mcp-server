...
faq:
  ...
  - q: "How do I find the exact parameters for an action?"
    a: "By default `META_PARAM_SCHEMA=opaque` keeps each meta-tool schema small, so the exact `params` shape is discovered through the tool manifest resources. Read `gitlab://tools` to list visible tools and executable entries for the active surface, then read `gitlab://tools/{id}` — for example `gitlab://tools/gitlab_merge_request.create` — to get the accepted call shape and JSON Schema for one action. These manifest resources stay available even when `CAPABILITY_SURFACE=minimal`. Dynamic deployments can instead use `gitlab_find_action` for inline schemas."
...
- **`TOOL_SURFACE=meta` groups operations into {stats.meta.base} domain tools** that route on an `action` parameter, instead of one tool per operation.
