...
LLMs have limited context windows. When an MCP server registers 1000 individual tools, the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.

| Mode              | Tool Count | Token Overhead | Functionality           |
| ----------------- | ---------- | -------------- | ----------------------- |
| Individual        | 1000       | Very high      | Full                    |
| Meta (base)       | 28         | Low            | Full                    |
| Meta (enterprise) | 43         | Low            | Full + Premium/Ultimate |
...
Some AI clients impose tool count limits (e.g., JetBrains AI Assistant limits MCP servers to 100 tools). Meta-tool mode (28 tools) works within these constraints. If you disable meta-tools (`META_TOOLS=false`), clients with such limits will only see a subset of the 1000 individual tools.
:::
