...
| `GITLAB_MCP_TIER`               | _(auto-detect)_ | Edition selector: `free`/`ce`, `premium`, or `ultimate`. When unset, the tier is detected from `GET /license`, then from the token's namespace plans (fallback `free`).            |
| `--tier`                        | _(auto-detect)_ | HTTP-mode edition flag: `free`/`ce`, `premium`, or `ultimate`. When it is not passed, `GITLAB_MCP_TIER` from the environment is used instead.                                      |
