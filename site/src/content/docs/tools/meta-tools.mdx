...
LLMs have limited context windows. When an MCP server registers 1006 individual tools, the tool descriptions alone consume a large portion of the available tokens, leaving less room for the actual conversation.

| Mode              | Tool Count | Token Overhead | Functionality           |
| ----------------- | ---------- | -------------- | ----------------------- |
| Individual        | 1006       | Very high      | Full                    |
| Meta (base)       | 32         | Low            | Full                    |
| Meta (enterprise) | 47         | Low            | Full + Premium/Ultimate |

Meta-tools reduce the tool count by **97%** while preserving 100% of the functionality. Every individual tool operation is available as an action within one of the domain meta-tools.

:::note[Client compatibility]
Some AI clients impose tool count limits (e.g., JetBrains AI Assistant limits MCP servers to 100 tools). Meta-tool mode (32 tools) works within these constraints. If you disable meta-tools (`META_TOOLS=false`), clients with such limits will only see a subset of the 1006 individual tools.
:::
...
By default, `META_PARAM_SCHEMA=opaque` keeps the tool schema small: clients see the valid `action` enum, while `params` remains an action-specific object. To discover the exact shape for a specific action, call the model-controlled schema actions or read the schema resources:

| `gitlab_server` action | Use                                                                                                  |
| ---------------------- | ---------------------------------------------------------------------------------------------------- |
| `schema_index`         | Lists visible meta-tools/actions with schema URIs, action counts, and destructive flags              |
| `schema_get`           | Returns the JSON Schema for the `params` object of one concrete action, or one-tool index if omitted |

Example schema tool call:
...
The schema resources mirror the same visible route set:
...
The per-action schema response describes only `params`. The final tool call still uses the meta-tool envelope with both `action` and `params`. Schema actions and resources are always available, even if you set `META_PARAM_SCHEMA=compact` or `META_PARAM_SCHEMA=full` to inline more schema detail in `tools/list`.
...
| `META_PARAM_SCHEMA` | `opaque` | Controls how much per-action `params` schema is inlined in `tools/list`: `opaque`, `compact`, or `full`. `gitlab_server` schema actions and schema resources remain available in every mode. |
