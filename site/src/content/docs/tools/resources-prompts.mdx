...
:::note
MCP lists fixed resources and URI templates separately. In full mode, clients that only inspect `resources/list` see 9 fixed URIs: current user, groups, workspace roots, the meta-schema index, and 5 workflow guides. The remaining 37 entries are URI templates returned by `resources/templates/list` and are read by substituting project, group, issue, MR, or other IDs at runtime.
:::
...
For example, `gitlab://schema/meta/gitlab_merge_request/create` returns the parameter schema for the `create` action of `gitlab_merge_request`. These schema resources are available for meta-tools with `CAPABILITY_SURFACE=full` or `minimal`, regardless of `META_PARAM_SCHEMA` mode. Dynamic clients using `CAPABILITY_SURFACE=minimal` should use `gitlab_find_action` for inline schemas instead.
...
| -------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CAPABILITY_SURFACE` | `full`  | `full` registers resources, meta-schema resources, workflow guides, and prompts; `minimal` keeps workspace roots and, for meta-tools, meta-schema resources |
