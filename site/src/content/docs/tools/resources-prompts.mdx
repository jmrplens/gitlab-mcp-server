    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "What is the difference between MCP resources, prompts, and tools?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "GitLab MCP Server exposes three MCP primitives. Tools perform actions, such as creating an issue or merging a request. Resources provide read-only context data — for example a project's metadata or members — that a client can request at any time without invoking a tool. Prompts are reusable templates that collect GitLab data and return structured context to guide a workflow, such as reviewing a merge request. Resources and prompts add context; tools change state."
            }
          },
          {
            "@type": "Question",
            "name": "What is the difference between a static resource and a resource template?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "A static resource is a fixed URI that always points to the same thing, such as gitlab://user/current for the authenticated user. A resource template contains placeholders like {project_id} or {group_id} that the client fills in at request time to read a specific project, group, issue, or merge request. MCP lists fixed resources through resources/list and templates through resources/templates/list."
            }
          },
          {
            "@type": "Question",
            "name": "How do I read an MCP resource from GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "A client reads a resource with the MCP resources/read method, passing the resource URI in the params. For a static resource the URI is used as-is, for example gitlab://user/current. For a template, the client substitutes the actual identifier or URL-encoded path first, for example gitlab://project/my-group%2Fmy-project/issues. The server returns the resource content as structured JSON."
            }
          },
          {
            "@type": "Question",
            "name": "Can I disable resources and prompts in GitLab MCP Server?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "Yes. The CAPABILITY_SURFACE setting controls which capabilities are registered. The default value full registers resources, workflow guides, prompts, and the surface-aware gitlab://tools manifest. Setting CAPABILITY_SURFACE to minimal keeps only the gitlab://tools manifest, which is useful when a client does not consume resources or prompts and you want a smaller capability surface."
            }
          }
        ]
      }
...
A **static resource** is a fixed URI that always points to the same thing, such as `gitlab://user/current`. A **resource template** contains placeholders like `{project_id}` or `{group_id}` that the client fills in at request time to read a specific project, group, issue, or merge request. MCP lists fixed resources through `resources/list` and templates through `resources/templates/list`, so a client that inspects only `resources/list` sees the {stats.resources} resources as 8 fixed URIs plus 37 templates.
