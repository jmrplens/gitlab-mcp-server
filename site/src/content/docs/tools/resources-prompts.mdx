...
For example, `gitlab://schema/meta/gitlab_merge_request/create` returns the parameter schema for the `create` action of `gitlab_merge_request`. These schema resources are available with `CAPABILITY_SURFACE=full` regardless of `META_PARAM_SCHEMA` mode. They are omitted with `CAPABILITY_SURFACE=minimal`; Dynamic clients should use `gitlab_describe_tools` or `gitlab_find_action` for inline schemas in that mode.
...
| -------------------- | ------- | ------------------------------------------------------------------------------------------------- |
| `CAPABILITY_SURFACE` | `full`  | `full` registers resources, meta-schema resources, workflow guides, and prompts; `minimal` keeps only workspace roots |
