...
:::note
MCP lists fixed resources and URI templates separately. In full mode, clients that only inspect `resources/list` see 9 fixed URIs: current user, groups, workspace roots, the tool manifest, and 5 workflow guides. The remaining 37 entries are URI templates returned by `resources/templates/list` and are read by substituting project, group, issue, MR, or other IDs at runtime.
:::
...
| -------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `CAPABILITY_SURFACE` | `full`  | `full` registers resources, workflow guides, prompts, and `gitlab://tools`; `minimal` keeps workspace roots plus `gitlab://tools` |
