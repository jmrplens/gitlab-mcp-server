...
With `GITLAB_MCP_TOOL_SURFACE=meta`, the server exposes a baseline of **{stats.meta.base} domain-level meta-tools** instead of the individual catalog. The Premium tier adds 6 meta-tools for 40 total, Ultimate 11 more for {stats.meta.self_managed_enterprise} total on a self-managed instance, and GitLab.com on Premium or Ultimate adds `gitlab_orbit` (GitLab.com's Knowledge Graph feature) for {stats.meta.gitlab_com} total. Each meta-tool groups related operations:
