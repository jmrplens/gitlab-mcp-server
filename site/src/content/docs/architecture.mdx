...
- **Meta-tools** (`TOOL_SURFACE=meta`) group related actions behind domain tools such as `gitlab_issue`.
- **Individual tools** (`TOOL_SURFACE=individual`) project one visible MCP tool per action for compatibility and testing.
...
graph LR
    ...
    catalog --> individual["Individual tools<br/>gitlab_issue_list, gitlab_project_create, ...<br/>+ 6 gitlab_orbit_* on GitLab.com Enterprise"]
...
By default (`TOOL_SURFACE` unset, or `TOOL_SURFACE=dynamic` to make it explicit), the server exposes only `gitlab_find_action` and `gitlab_execute_action`. The same canonical action catalog remains available and is shared with meta-tools, so dynamic mode changes discovery rather than GitLab behavior.
...
With `TOOL_SURFACE=individual`, all individual tools are exposed (e.g., `gitlab_issue_list`, `gitlab_issue_create`): {stats.tools.ultimate_self_managed} on self-managed Enterprise/Premium, or {stats.tools.gitlab_com} on GitLab.com Enterprise/Premium with Orbit. This may be useful for testing but is not recommended for production.
...
- [`gitlab-org/api/client-go`](https://pkg.go.dev/gitlab.com/gitlab-org/api/client-go/v2) — GitLab's own Go API client, used for every REST call; domains it does not wrap are issued as raw GraphQL
