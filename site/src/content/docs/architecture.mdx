...
With `META_TOOLS=true` (default), the server exposes **32 domain-level meta-tools** (47 on self-managed Enterprise/Premium, 48 on GitLab.com Enterprise/Premium with Orbit) instead of the individual catalog. Each meta-tool groups related operations:
