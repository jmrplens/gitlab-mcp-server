...
sequenceDiagram
    ...
    AI->>MCP: tools/call: gitlab_execute_action {action: "merge_request.list", params: {project_id: "my-project", state: "opened"}}
...
Dynamic mode is the default low-token surface and is documented in [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/). Meta-tools remain available with `GITLAB_MCP_TOOL_SURFACE=meta`.

### Meta-tool mode

With `GITLAB_MCP_TOOL_SURFACE=meta`, the server exposes a baseline of **{stats.meta.base} meta-tools** instead of the individual catalog: 29 catalog-backed tools (28 GitLab domain dispatchers plus `gitlab_server`), `gitlab_discover_project`, and the four interactive creation flows. The Premium tier adds 6 meta-tools for 40 total, Ultimate 11 more for {stats.meta.self_managed_enterprise} total on a self-managed instance, and GitLab.com adds `gitlab_orbit` (GitLab.com's Knowledge Graph feature) on Premium and Ultimate alike, for {stats.meta.gitlab_com} total on GitLab.com Ultimate. Each meta-tool groups related operations:
...
With `GITLAB_MCP_TOOL_SURFACE=individual`, all individual tools are exposed (e.g., `gitlab_issue_list`, `gitlab_issue_create`): {stats.tools.ultimate_self_managed} on self-managed Ultimate, or {stats.tools.gitlab_com} on GitLab.com Ultimate with Orbit. This may be useful for testing but is not recommended for production.
...
A result about **one** object is a card: a heading and a list of `- **Label**: value` rows, as above. A table is for a collection of objects sharing columns, which is what a list response returns. The shape is not decided per formatter: one writer produces every card, and a hand-written row fails the [`check-md-escaping`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/markdown-card.md) gate.
...
The architecture above is the result of a handful of recorded decisions. Each Architectural Decision Record states the problem, the alternatives that were rejected, and the consequences accepted in exchange — useful when you want to know _why_ the server is shaped this way rather than just how.

| Decision                                                                                                                                                        | What it settled                                                                         | Trade-off accepted                                                                                                                  |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| [ADR-0004](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/adr-0004-modular-tools-subpackages.md) — modular sub-packages           | One Go package per GitLab domain under `internal/tools/`, rather than one large package | More packages to navigate, in exchange for isolated tests and no import cycles                                                      |
| [ADR-0005](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/adr-0005-meta-tool-consolidation.md) — meta-tool consolidation          | Grouping operations into domain dispatchers that route on an `action` parameter         | An extra indirection in the call shape, in exchange for a tool list clients can actually load                                       |
| [ADR-0011](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/adr-0011-low-token-dynamic-toolset.md) — dynamic toolset                | Making find/execute the default surface instead of listing every tool                   | One discovery call per task, in exchange for a {footprint.individual.ultimate.reduction_factor_vs_dynamic}× smaller startup context |
| [ADR-0014](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/adr-0014-catalog-first-runtime-architecture.md) — catalog-first runtime | Projecting all three surfaces from one canonical action catalog                         | A build-time projection step, in exchange for identical behaviour and safety across surfaces                                        |
| [ADR-0007](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/adr-0007-rich-error-semantics.md) — rich error semantics                | Returning actionable, classified errors instead of raw API failures                     | More error-handling code per tool, in exchange for errors a model can act on                                                        |

The [full ADR index](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/development/adr/README.md) covers the remaining decisions, including GraphQL migration strategy and the polled resource-subscriptions design (ADR-0015).
...
- [`modelcontextprotocol/go-sdk`](https://pkg.go.dev/github.com/modelcontextprotocol/go-sdk/mcp) — the official Go MCP SDK that implements tool, resource and prompt registration and both transports
- [`gitlab-org/api/client-go`](https://pkg.go.dev/gitlab.com/gitlab-org/api/client-go/v3) — GitLab's own Go API client, used for every REST call; domains it does not wrap are issued as raw GraphQL
...
For the complete technical reference, see [`docs/concepts/architecture.md`](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/concepts/architecture.md) in the repository.
:::
