...
With `TOOL_SURFACE=meta`, the server exposes a baseline of **32 domain-level meta-tools** instead of the individual catalog. Self-managed Enterprise/Premium environments add 16 Enterprise/Premium tools for 49 total, and GitLab.com Enterprise/Premium with Orbit (GitLab.com's Knowledge Graph feature) adds one more tool for 49 total. Each meta-tool groups related operations:
...
With `TOOL_SURFACE=individual`, all individual tools are exposed (e.g., `gitlab_list_issues`, `gitlab_create_issue`): 1028 on self-managed Enterprise/Premium, or 1034 on GitLab.com Enterprise/Premium with Orbit. This may be useful for testing but is not recommended for production.
