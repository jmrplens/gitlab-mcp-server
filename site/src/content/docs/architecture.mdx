...
With `META_TOOLS=true` (default), the server exposes **32 domain-level meta-tools** (47 with enterprise) instead of 1006+ individual tools. Each meta-tool groups related operations:
...
With `META_TOOLS=false`, all 1006+ individual tools are exposed (e.g., `gitlab_list_issues`, `gitlab_create_issue`). This may be useful for testing but is not recommended for production.
...
44 read-only MCP resources that provide contextual data:
