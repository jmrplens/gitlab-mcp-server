...
- **Dynamic tools** search, describe, and execute the same catalog entries with a much smaller visible tool list.
...
Meta-tools remain the default today. Dynamic mode is the low-token alternative and is documented in [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/).
