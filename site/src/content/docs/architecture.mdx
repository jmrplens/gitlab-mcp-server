...
With `META_TOOLS=true` (default), the server exposes **32 base domain-level meta-tools**, expanding to 47 on self-managed Enterprise/Premium and 48 on GitLab.com Enterprise/Premium with Orbit, instead of the individual catalog. Each meta-tool groups related operations:
