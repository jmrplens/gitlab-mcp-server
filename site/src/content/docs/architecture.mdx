...
With `TOOL_SURFACE=dynamic`, the server exposes only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`. The complete action catalog remains available through a hidden registry built from the same meta-tool route maps, so dynamic mode changes discovery rather than GitLab behavior.

```mermaid
flowchart TD
    A[Meta-tool route maps] --> B[Hidden dynamic registry]
...
Meta-tools remain the default today. Dynamic mode is the low-token candidate for a future default and is documented in [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/).
