...
With `GITLAB_MCP_TOOL_SURFACE=meta`, the server exposes a baseline of **{stats.meta.base} domain-level meta-tools** instead of the individual catalog. The Premium tier adds 6 meta-tools for 38 total, Ultimate 11 more for {stats.meta.self_managed_enterprise} total on a self-managed instance, and GitLab.com on Premium or Ultimate adds `gitlab_orbit` (GitLab.com's Knowledge Graph feature) for {stats.meta.gitlab_com} total. Each meta-tool groups related operations:
...
With `GITLAB_MCP_TOOL_SURFACE=individual`, all individual tools are exposed (e.g., `gitlab_issue_list`, `gitlab_issue_create`): {stats.tools.ultimate_self_managed} on self-managed Enterprise/Premium, or {stats.tools.gitlab_com} on GitLab.com Enterprise/Premium with Orbit. This may be useful for testing but is not recommended for production.
