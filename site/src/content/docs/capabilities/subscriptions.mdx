...
Watchers use the subscriber's own GitLab token, capped at **10 watchers per credential** (one pool entry per token+URL in HTTP mode) and at **512 per process**. The worst case — ten watchers at the 5s floor — is 120 requests/minute; ten demoted watches cost one. A 429 from GitLab pauses every watcher with exponential back-off (30s doubling to 5 minutes).
