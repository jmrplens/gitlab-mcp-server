...
Subscriptions are advertised on `CAPABILITY_SURFACE=full` (the default). The `minimal` surface does not register the GitLab data resources a subscription applies to, so it does not advertise the capability either.

## What can be watched?

26 single-object resource kinds — objects with a lifecycle worth following:
