...
Because multi-round-trip prompts travel inside the tool result, elicitation keeps working over the default [stateless HTTP transport](/operations/http-server/#stateless-mode) for clients on protocol `2026-07-28`. Legacy-protocol clients need a server-initiated channel, so over stateless HTTP they get the non-interactive fallbacks instead (wizard error hints, and `confirm` for destructive actions).
