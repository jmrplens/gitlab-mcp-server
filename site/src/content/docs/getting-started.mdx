...
This repository ships an [Agent Plugins](https://agent-plugins.org/) 1.0 manifest (root `plugin.json` and MCP config `mcp.json`) — plus the legacy Open Plugins manifest (`.plugin/plugin.json`) for older hosts — so the server can be installed in a single step on any conformant host (Cursor, Claude Code, VS Code, OpenCode):
