...
By default, the server runs in **dynamic find/execute mode** with `gitlab_find_action` and `gitlab_execute_tool`. Set `TOOL_SURFACE=meta` to use **33 domain-level meta-tools** (49 on self-managed Enterprise/Premium, 50 on GitLab.com Enterprise/Premium with Orbit) that cover the same functionality through consolidated domain dispatchers.
...
For the smallest stable catalog, set `TOOL_SURFACE=dynamic`. The server exposes only `gitlab_find_action` and `gitlab_execute_tool`, backed by the same canonical GitLab action catalog.

Pair dynamic mode with `CAPABILITY_SURFACE=minimal` to keep only workspace roots and omit optional resources, meta-schema resources, workflow guides, and prompts when you need the smallest initial MCP context. Dynamic keeps inline schemas through `gitlab_find_action`.
:::
