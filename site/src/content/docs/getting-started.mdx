...
This repository ships an [Open Plugins](https://open-plugins.com/) v1.0.0 manifest (`.plugin/plugin.json`) and referenced MCP config (`mcp.json`) so the server can be installed in a single step on any conformant host (Cursor, Claude Code, OpenCode):
