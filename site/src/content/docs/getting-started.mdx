...
For the smallest visible stable catalog, set `TOOL_SURFACE=dynamic`. The server exposes only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`, backed by the same hidden GitLab action registry. `TOOL_SURFACE=dynamic-2` is available as an experimental find/execute A/B candidate.
