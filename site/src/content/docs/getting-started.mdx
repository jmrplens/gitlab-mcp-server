...
`TOOL_SURFACE` is the preferred current selector and takes precedence over `META_TOOLS` when both are set. Use `TOOL_SURFACE=meta` instead of `META_TOOLS=true`, `TOOL_SURFACE=individual` instead of `META_TOOLS=false`, and `TOOL_SURFACE=dynamic`, `dynamic-2`, or `dynamic-3` instead of the same legacy `META_TOOLS` values. `META_TOOLS` remains supported for backward compatibility but should be treated as deprecated in favor of the explicit selector.
...
For the smallest stable catalog, set `TOOL_SURFACE=dynamic`. The server exposes only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`, backed by the same canonical GitLab action catalog. `TOOL_SURFACE=dynamic-2` is available as an experimental find/execute option.
