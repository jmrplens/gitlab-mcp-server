...
Published as [`gitlab-mcp-server`](https://pypi.org/project/gitlab-mcp-server/) using the same model as `uv` and `ruff`: each platform wheel carries the native binary, the installer places it on the scripts path as the command itself, and no Python runs when the server does.
