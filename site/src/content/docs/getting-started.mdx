...
`TOOL_SURFACE` is the canonical selector and takes precedence over `META_TOOLS` when both are set. Use `TOOL_SURFACE=meta` for the default consolidated catalog, `TOOL_SURFACE=individual` for individual tools, and `TOOL_SURFACE=dynamic`, `dynamic-2`, or `dynamic-3` for dynamic surfaces. `META_TOOLS` remains supported for one compatibility window but should be treated as deprecated in favor of the explicit selector.
...
Both GitLab CE (free) and EE (Premium/Ultimate) are fully supported. For EE-exclusive features like DORA metrics, epics, and vulnerability management, enable the Enterprise/Premium catalog with `GITLAB_ENTERPRISE=true` in stdio mode or `--enterprise`/auto-detection in HTTP mode. See [Configuration](/gitlab-mcp-server/configuration/) for all available options.
...
By default, the server runs in **meta-tool mode** — instead of registering hundreds of individual tools (up to 1019 on GitLab.com Enterprise/Premium), it consolidates them into **33 domain-level meta-tools** (49 on self-managed Enterprise/Premium, 50 on GitLab.com Enterprise/Premium with Orbit) that cover 100% of the functionality.
...
For the smallest stable catalog, set `TOOL_SURFACE=dynamic`. The server exposes only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`, backed by the same canonical GitLab action catalog. `TOOL_SURFACE=dynamic-3` explicitly selects that same three-tool surface, and `TOOL_SURFACE=dynamic-2` is available as an experimental find/execute variant.

Pair dynamic mode with `CAPABILITY_SURFACE=minimal` to keep only workspace roots and omit optional resources, meta-schema resources, workflow guides, and prompts when you need the smallest initial MCP context. Dynamic keeps inline schemas through `gitlab_describe_tools`.
:::
