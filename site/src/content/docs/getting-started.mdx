...
description: Install and configure GitLab MCP Server in under five minutes — via Docker, native binary, or setup wizard — for VS Code, Claude, Cursor, and other MCP clients.
chips:
  - text: Zero-install with Docker
  - text: Any MCP client
  - text: Setup wizard included
...
- **Three ways to install**: a Docker image, a single native binary, or the interactive setup wizard.
...
:::tip[Prefer a guided flow?]
The built-in [setup wizard](#setup-wizard) collects your token and configures your client for you — no JSON to edit. Run `gitlab-mcp-server --setup` after installing the binary.
:::
...
:::note
The setup wizard configures stdio clients only. It offers Web, TUI, and CLI interfaces. See [Setup Wizard](/gitlab-mcp-server/setup-wizard/) for modes, supported clients, advanced options, and reconfiguration behavior.
:::
