...
GitLab MCP Server works with both Community Edition (CE) and Enterprise Edition (EE). The Enterprise/Premium catalog unlocks 14 additional enterprise-only meta-tools: use `GITLAB_ENTERPRISE=true` in stdio mode, or `--enterprise`/HTTP edition auto-detection in HTTP mode.
...
| ----------- | :-----------------------------------------------------------------------: |
| Tools       | ✅ (up to 1022 individual / 33 base, 49 self-managed, 50 GitLab.com meta) |
| Resources   |                                  ✅ (46)                                  |
| Prompts     |                                  ✅ (37)                                  |
...
The 1022 individual figure is the expanded set of distinct tool instances. The 33 base, 49 self-managed, and 50 GitLab.com counts are meta-tool catalog sizes whose actions expand to that larger individual tool surface.
