...
| `GITLAB_MCP_RATE_LIMIT_RPS`            | `0`     | Per-server rate limit, in req/s, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `prompts/get` (`0` disables it). Stdio only; HTTP mode defaults to `10` via `--rate-limit-rps`                                                                                                                                  |
...
| `--rate-limit-rps`         | `10`            | Per-server rate limit, in req/s, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `prompts/get` (`0` disables it); on by default because an HTTP deployment is shared     |
