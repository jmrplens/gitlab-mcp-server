...
GitLab MCP Server is configured through environment variables for stdio mode and CLI flags for HTTP mode. This page covers the options most users need; see the repository [environment reference](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/env-reference.md) and [CLI reference](https://github.com/jmrplens/gitlab-mcp-server/blob/main/docs/cli-reference.md) for the exhaustive tables.
...
| `CAPABILITY_SURFACE`     | `full`               | Resource and prompt catalog selector: `full` keeps the complete catalog; `minimal` keeps `gitlab://workspace/roots`, disables optional resources, prompts, and workflow guides, and keeps meta-schema resources only for `TOOL_SURFACE=meta` |
...
:::tip
**Dynamic mode** (`TOOL_SURFACE=dynamic`) is the default and lowest-token catalog. It exposes `gitlab_find_action` and `gitlab_execute_tool`, while the same GitLab actions remain available through the canonical action catalog.
...
For deployments using the dynamic tool surface where resources and prompts dominate initial context, set `CAPABILITY_SURFACE=minimal`. It keeps `gitlab://workspace/roots` for project discovery and omits optional resources, prompts, workflow guides, and dynamic meta-schema resources. This does not remove action schemas from dynamic discovery: use `gitlab_find_action` to retrieve exact schemas inline before calling `gitlab_execute_tool`. In `TOOL_SURFACE=meta`, minimal keeps meta-schema resources so callers can keep `META_PARAM_SCHEMA=opaque` without losing exact parameter schemas. The default remains `full`; no schemas-only capability mode is exposed today.
:::
...
| `--capability-surface`   | `full`                       | Resource and prompt catalog selector: `full` or `minimal`; minimal omits optional resources, workflow guides, and prompts, but keeps meta-schema resources for `--tool-surface=meta` |
