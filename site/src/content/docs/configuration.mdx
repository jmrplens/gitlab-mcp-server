...
| `META_TOOLS`             | `true`               | Tool catalog selector: `true` for meta-tools, `false` for individual tools, `dynamic` for the low-token search/describe/execute surface, `dynamic-3` for the explicit current dynamic surface, or experimental `dynamic-2` |
...
Use `TOOL_SURFACE=dynamic` for the current lowest-token catalog: only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool` are visible, while the same GitLab actions remain available through the canonical action catalog. Meta-tools remain the default today, but dynamic mode is the current candidate for a future low-token default. Use `TOOL_SURFACE=dynamic-2` only for the parked find/execute A/B candidate.

See [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/) for the search, describe, execute workflow.

For dynamic deployments where resources and prompts dominate initial context, set `CAPABILITY_SURFACE=minimal`. It keeps `gitlab://workspace/roots` for project discovery and omits optional resources, meta-schema resources, workflow guides, and prompts. The default remains `full`.
:::
