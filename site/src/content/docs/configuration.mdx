...
Use `TOOL_SURFACE=dynamic` for the current lowest-token catalog: only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool` are visible, while the same GitLab actions remain available through a hidden registry. Meta-tools remain the default today, but dynamic mode is the current candidate for a future low-token default. Use `TOOL_SURFACE=dynamic-2` only for the parked find/execute A/B candidate.
