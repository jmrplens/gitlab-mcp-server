...
| `META_TOOLS`             | `true`               | Tool catalog selector: `true` for meta-tools, `false` for individual tools, or `dynamic` for the low-token search/describe/execute surface                                        |
| `TOOL_SURFACE`           | —                    | Explicit tool catalog selector: `meta`, `individual`, or `dynamic`. When set, it overrides `META_TOOLS`                                                                           |
...
Use `TOOL_SURFACE=dynamic` for the lowest-token catalog: only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool` are visible, while the same GitLab actions remain available through a hidden registry.
...
| `--tool-surface`         | —                            | Explicit tool catalog selector: `meta`, `individual`, or `dynamic`. Overrides `--meta-tools`               |
