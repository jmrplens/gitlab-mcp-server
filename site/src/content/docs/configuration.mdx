...
| `CAPABILITY_SURFACE`     | `full`               | Resource and prompt catalog selector: `full` keeps the complete catalog; `minimal` keeps only `gitlab://workspace/roots` and disables optional resources, prompts, workflow guides, and meta-schema resources |
...
:::tip
**Meta-tools mode** (`TOOL_SURFACE=meta`) is strongly recommended. It groups tools by domain (e.g., `gitlab_issue` handles list, get, create, update, delete) which reduces token usage and improves AI tool selection accuracy.
...
See [Dynamic toolset](/gitlab-mcp-server/tools/dynamic-tools/) for the search, describe, execute workflow.

For deployments using the dynamic tool surface where resources and prompts dominate initial context, set `CAPABILITY_SURFACE=minimal`. It keeps `gitlab://workspace/roots` for project discovery and omits optional resources, prompts, workflow guides, and meta-schema resources. This does not remove action schemas from dynamic discovery: use `gitlab_describe_tools` to retrieve exact schemas inline before calling `gitlab_execute_tool`. The default remains `full`; no schemas-only capability mode is exposed today.
:::
...
| `--capability-surface`   | `full`                       | Resource and prompt catalog selector: `full` or `minimal`; minimal omits optional resources, meta-schema, and prompts |
