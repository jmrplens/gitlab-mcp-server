...
For deployments using the dynamic tool surface where resources and prompts dominate initial context, set `CAPABILITY_SURFACE=minimal`. It keeps `gitlab://tools` for action discovery, and omits optional resources, prompts, and workflow guides. This does not remove action schemas from dynamic discovery: use `gitlab_find_action` to retrieve exact schemas inline before calling `gitlab_execute_action`, or read `gitlab://tools/{id}` for a manifest entry. The default remains `full`; no schemas-only capability mode is exposed today.
:::
