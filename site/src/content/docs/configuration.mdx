...
For deployments using the dynamic tool surface where resources and prompts dominate initial context, set `CAPABILITY_SURFACE=minimal`. It keeps `gitlab://workspace/roots` for project discovery and omits optional resources, prompts, workflow guides, and meta-schema resources. This does not remove action schemas from dynamic discovery: use `gitlab_find_action` to retrieve exact schemas inline before calling `gitlab_execute_tool`. The default remains `full`; no schemas-only capability mode is exposed today.
:::
