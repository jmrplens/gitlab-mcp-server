...
| `META_PARAM_SCHEMA`      | `opaque`             | Meta-tool input-schema strategy: `opaque` (default), `compact` (~5x), or `full` (~10x). Per-action schema always discoverable via `gitlab_server` `schema_get` and the `gitlab://schema/meta/{tool}/{action}` resource |
