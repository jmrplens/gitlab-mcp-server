...
| `--public-url`           | _(empty)_                    | Externally reachable origin (`scheme://host[:port][/path]`). Required with `--auth-mode=oauth`; its origin is also trusted for cross-origin browser requests                         |
| `--trusted-origins`      | _(empty)_                    | Comma-separated origins allowed to make cross-origin browser requests; `*` accepts any origin. Empty rejects all                                                                     |
