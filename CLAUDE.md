...
9. **`server.json`'s `packages` array says what a client should install, and almost nothing about it is schema-checkable.** Six package entries are declared: the `.mcpb` bundle, the `ghcr.io` image, the Docker Hub image, the npm launcher, the pypi distribution, and the nuget tool. Three traps, each of which passed `make check-server-json` while being wrong:
   ...
   - **A cosign signature is not build provenance.** The signature over the image index has an empty in-toto predicate: it says the release workflow pushed this index, not which commit or run produced it. `actions/attest-build-provenance` answers the second question and runs three times per registry with `push-to-registry: true`, over the index and each of the two platform manifests it lists, so every attestation is a referrer on its subject as well as a record in GitHub's attestation store, which a scanner reading a registry cannot see. The platform manifests are attested because verifiers disagree about which digest the image is: `gh attestation verify oci://` resolves a tag to the index, while a scanner that pulls the image for its own platform resolves the index to one manifest and reads that digest's referrers (verifymcp.io's scan of 3.0.0 reports the linux/amd64 manifest as its subject, and found the cosign signature there only because `cosign sign --recursive` signs every manifest the index lists). It changes the subjects' referrers and never the subjects, so the digest `server.json` pins is unaffected and no re-stamp is needed. `cosign-release` is pinned on all nine `cosign-installer` steps for a related reason: the cosign major decides how a signature is attached, and a 2.x client reports "no signatures found" on an image a 3.x client verifies. **None of this runs in the job that builds the image.** `sign-attest` holds the two cosign signatures and all ten attestations, and `docker` keeps `packages: write` and loses `id-token`: the build is not byte-reproducible (BuildKit's own `provenance: true` attestation carries timestamps whatever the build args say), so a re-run of the build job pushes a new index under the same `3.0.0` and `latest` tags and orphans the one already signed, which `server.json` is by then pinning. With the split, a failed attestation costs a re-run of three minutes of signing against the published image and no tag moves. The GitHub release being immutable does not cover this: what moves is a registry tag. The SBOMs are still generated in `docker` and cross as an artifact, because a rehearsal has no pushed image to scan and does have the two smoke images in that job's daemon.
   - **The image's SBOM is published twice, on purpose.** `sbom: true` on the build writes an SPDX document into the index as an attestation manifest, one per platform, and a client reading the registry's referrers never sees it: those `unknown/unknown` manifests are part of the index rather than referrers of anything. `actions/attest` with `sbom-path` publishes the same information as a referrer, so both readers are served and neither document replaces the other; it is the action itself rather than the deprecated `actions/attest-sbom` wrapper, pinned to the version the provenance wrapper delegates to, so all ten attestation steps run one credential lookup. It runs **per platform and never over the index**, because an SBOM describes contents and an index has none of its own: it lists manifests, and the two platform images differ, so one document over the index would be a document about nothing. The consequence is that `gh attestation verify oci://…:<tag> --predicate-type https://spdx.dev/Document/v2.3` finds nothing, by design, since gh resolves a tag to the index: the SBOM is verified by the platform manifest's digest, which the security page shows. syft is fetched by hand in the docker job for the same reason the release job fetches it that way, its bytes pinned by SHA256 and its provenance checked with cosign, since `anchore/sbom-action` downloads an install script from a branch head and this job holds the OIDC identity the attestation store trusts. A generated document listing no packages fails the step rather than attesting an empty claim. A rehearsal runs the install, the scan and that guard against the two smoke images the daemon already holds, since it pushes nothing to scan; only the attest steps stay skipped.
