...
│   ├── audit_md_cards/          # Judges the shape of every Markdown response this server can produce, by rendering rather than by reading: it builds a sample of each registered output type, asks the registry for its Markdown exactly as a tool call does, and judges the finished string, so a card assembled across conditionals, loops and three packages is judged as what comes out of them. Four rules — a pipe row no delimiter row turns into a table, a card field inside a table, any other block holding both shapes, and a field written "**Label**: value" with no list marker — and `-rules` stages which of them a run judges, the way `-contexts` stages its sibling. Two samples a type, zero and populated, so the conditional lines are written; a formatter that panics on one is reported unrenderable rather than failing, which is the same split `audit_md_escaping` gives a value it cannot follow. `-census` prints the card/table count by package, which is the number issue 697 opened with, kept as something a run regenerates (make check-md-cards, make audit-md-cards)
...
A formatter also writes one shape a block, and the rule deciding which is that **a table row is an object, not a field**: a table carries many objects, one row an object, under column headings that name the fields they hold, and a one-object card carries one object, one field a line. `| Field | Value |` is that card written in the other notation with the information taken out of its header, which is why 27 packages answered in one shape and 76 in the other, and four in both ([issue 697](https://github.com/jmrplens/gitlab-mcp-server/issues/697)). A card is written through `toolutil.WriteMdField`, `WriteMdFieldIf`, `WriteMdFieldInt`, `WriteMdFieldBool`, `WriteMdFieldTime`, `WriteMdFieldCode` and `WriteMdFieldLink`, each of which escapes or renders its own value, so the choice of shape is not made again in every formatter and the escaping of a list-item value is settled once. `WriteMdFieldRendered` takes a value a renderer in `toolutil` already produced and escapes nothing, and is the one card helper declared a sink in `cmd/audit_md_escaping`, so a raw value reaching it is reported against the package that wrote the call and a `//gitlab:allow-unescaped` directive still answers it. None of them formats through `fmt`: the line is assembled out of writes, so the escaping audit reads no template inside `toolutil` and reports nothing there. `make check-md-cards` (`cmd/audit_md_cards`) is the gate, and it renders rather than reads — a gate over the source would have to model the conditionals, loops and helpers a card is built from across three packages, and an AST prototype over this tree produced 25 false positives for that reason alone. It gates three rules today; the fourth, a field written `**Label**: value` with no list marker, is 24 formatters and is reported by `make audit-md-cards` until that sweep lands.
