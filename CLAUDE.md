...
└── VERSION                      # Semantic version (2.6.3)
```
...
| `CLIENT_COMPAT`          | No       | Per-client response compatibility (`auto` default): Codex sessions get float `priority` in annotations rounded to 0/1; `off` disables. See `internal/clientcompat` and `docs/guides/client-compatibility.md` |
