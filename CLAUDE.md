...
| `GITLAB_MCP_ACTION_TIMEOUT`         | No       | Cancel an action still running after this long, in both transports (`65m` default, `0` disables, max 24h). Applied where every action passes, `toolutil.WrapAction`, so it ends a handler whose client is still waiting; the transports already cancel a call whose client went away. Above the longest wait any action offers (a pipeline wait caps itself at 3600 s). Flag `--action-timeout` in HTTP mode |
| `GITLAB_MCP_DRAIN_DELAY`            | No       | HTTP mode: after `SIGTERM`, keep the listener open and answer `/health` with `503 draining` (`Cache-Control: no-store`) for this long before closing it, so a balancer that polls `/health` removes the instance before the close (`0` default, max 5m). Set it to at least one probe interval. `/health` also carries `build` (closest release plus short commit, `.dirty` when the tree had changes; `cmd/server/health.go`) and `config_digest` (twelve hex over tool surface, capability surface, meta parameter schema, tier, read-only, safe mode and excluded tools) so the instances behind one balancer can be compared. Flag `--drain-delay` |
...
| `--rate-limit-rps` | `10` | Per-server rate limit, in req/s, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get`) (`0` disables it). On by default in HTTP mode; the `GITLAB_MCP_RATE_LIMIT_RPS` env var used by stdio still defaults to `0` |
...
Four invariants of HTTP mode that are easy to break by touching the wrong layer:
