...
**Rehearse before tagging.** `gh workflow run release.yml -f tag=v<previous>` runs the whole release chain against an existing tag with the same jobs and secrets and skips only the irreversible steps (image push and signatures, the GitHub release and attestations, npm and PyPI uploads, the tap push, the registry publish, the commit to main, winget). Everything else is real: the E2E gate, GoReleaser's build, the publishers' download and cosign verification of what that tag shipped, the package validation, and `verify-published` reading the registries. A dispatch is always a rehearsal; there is no input that makes it publish. The publisher jobs were added after 2.7.5 and had never run before this existed.
