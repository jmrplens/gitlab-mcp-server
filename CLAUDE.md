...
9. **`server.json`'s `packages` array says what a client should install, and almost nothing about it is schema-checkable.** Six package entries are declared: the `.mcpb` bundle, the `ghcr.io` image, the Docker Hub image, the npm launcher, the pypi distribution, and the nuget tool. Three traps, each of which passed `make check-server-json` while being wrong:
   ...
   - **The image's SBOM is published twice, on purpose.** `sbom: true` on the build writes an SPDX document into the index as an attestation manifest, one per platform, and a client reading the registry's referrers never sees it: those `unknown/unknown` manifests are part of the index rather than referrers of anything. `actions/attest-sbom` publishes the same information as a referrer, so both readers are served and neither document replaces the other. It runs **per platform and never over the index**, because an SBOM describes contents and an index has none of its own: it lists manifests, and the two platform images differ, so one document over the index would be a document about nothing. syft is fetched by hand in the docker job for the same reason the release job fetches it that way, its bytes pinned by SHA256 and its provenance checked with cosign, since `anchore/sbom-action` downloads an install script from a branch head and this job holds the OIDC identity the attestation store trusts. A generated document listing no packages fails the step rather than attesting an empty claim.
