...
8. **`server.json`'s `packages` array says what a client should install, and almost nothing about it is schema-checkable.** Two package entries are declared: the `.mcpb` bundle and the `ghcr.io` image. Three traps, each of which passed `make check-server-json` while being wrong:
   ...
   The npm launcher is **not** declared as a package yet. The registry validates npm ownership through an `mcpName` field in the published `package.json`, and the version on the registry (2.7.5) predates it. The field is now committed in `npm/gitlab-mcp-server/package.json`, so the next release publishes a package that can be claimed; the `server.json` entry can only be added after that version is live on npm, or the publish step fails.
