...
└── VERSION                      # Semantic version (2.6.7)
```
...
- **What is subscribable** — a whitelist of 26 single-object kinds in `kind.go`, parsed segment by segment. Collections are excluded on purpose. Two drift guards round-trip the real resource registry, so a new template must be classified and a renamed one cannot leave a stale decision behind.
