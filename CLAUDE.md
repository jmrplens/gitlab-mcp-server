...
6. **npm publishes automatically in the release workflow** (unlike LobeHub — npm authenticates in CI with a token). The distribution is a launcher package (`@jmrplens/gitlab-mcp-server`) plus six per-platform packages that each carry a release binary — the esbuild/biome model, so `npx` works, `--ignore-scripts` works, and nothing is downloaded at install time. The release job's `Publish to npm` step runs right after GoReleaser, reading the same `dist/` binaries the mcpb step uses, and **skips with a warning when `NPM_TOKEN` is unset** — so it never blocks a release before the secret exists, the same shape as the Homebrew and Docker Hub steps. `scripts/build-npm.mjs` assembles the packages from `dist/` (Node's `win32`/`x64` names mapped to Go's `windows`/`amd64` in one table there); `scripts/publish-npm.sh` publishes the platform packages first, then the launcher — that order matters, or an install racing the publish resolves optional dependencies not on the registry yet — and it **skips any version already published** so re-running a release job does not die on npm's 409. The launcher's version and its optionalDependency pins move together via `build-npm.mjs --sync-only`, wired into `scripts/update-server-json-sha.sh` so the committed `npm/gitlab-mcp-server/package.json` stays current with the release stamp. The launcher forces `AUTO_UPDATE=false` (npm owns the binary, like the Homebrew formula and the mcpb manifest). For an out-of-band publish, `make publish-npm NPM_BINARIES=dist` from a machine with an npm token (rehearse with `make publish-npm-dry`). The unscoped name `gitlab-mcp-server` was already taken on npm, hence the scope.
