...
- **Does GitLab say it sends what we publish.** The one check here whose oracle is GitLab itself. It joins the inventory to `docs/development/gitlab-api-shapes.json` (`cmd/gen_api_shapes`, extracted from the OpenAPI document GitLab generates out of its own Grape entities) and reports every `*Output` field under `internal/tools` that no endpoint its package was recorded calling declares in a response: 629 fields across 131 packages today, among them the 18 `mrapprovals` fields recorded independently in [issue 580](https://github.com/jmrplens/gitlab-mcp-server/issues/580). It **reports and never gates**, because three deliberate concessions each lose findings so that none is invented: the inventory names a package, so a package's endpoints are unioned before the comparison (exact for one endpoint, weaker as the package grows); an operation the document gives no response schema for, 553 of the 1847, silences its package rather than condemning it; and a nested output type is skipped, since GitLab lists the properties of the object an endpoint returns and not of the objects inside it. The join is published beside the findings (`shapes.join`: 894 of 1479 REST rows exact, 504 after accepting one untemplated fixture segment, 81 unmatched) because a package whose rows all miss would otherwise have its whole output condemned by a lookup failure, and the untemplated segments are published with it, which measures this repository's own recording rather than the server.
