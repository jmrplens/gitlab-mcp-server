...
Three checks, two of which gate (`make audit-1to1-paths`, run by `make analyze` and by CI's generated-artifacts job, with no network and no suite run):

- **Has the path ever been observed.** An action whose owning package issued no request has never had its request seen by anything. Held at package grain, because nothing on the wire names an action. A package silent for a reason is held to a declaration with a category and a reason in `cmd/audit_1to1/internal/paths/declarations.go`, and a declaration that no longer describes the tree is itself a finding. One entry today: `internal/tools/adminspecs` declares specs whose handlers live in other packages, so its 92 actions' requests are recorded under the packages that make them.
- **Does the document validate.** Every raw GraphQL document in the source against the pinned schema, through `cmd/internal/graphqldocs`, shared with `make check-graphql-documents`.
- **Does the endpoint exist.** `-check-endpoints` compares every recorded REST endpoint with GitLab's own API documentation (`make audit-1to1-paths-endpoints`). A **candidate list, never a gate**: the oracle is prose, so a deprecated-but-working alias (`/services/` for the integrations endpoints), an endpoint documented under `doc/user` rather than `doc/api` (the generic package registry, Terraform state), and one whose only mention is a sentence (`/usage_data/track_events`) all look exactly like a defect. The page listing comes from the repository tree rather than `api_resources.md`, because 101 of the 253 pages under `doc/api` are not linked from that index or from `rest/_index.md`, and against the index alone the comparison reports roughly five times as many candidates as it does against all 247 readable pages (75 of 1381 recorded endpoints today); the report counts the pages it could not read, since a hole in the corpus produces candidates that are only about the hole.

What none of it can do is check that GitLab answers the way our output struct expects. The layers stack: the pinned schema and the inventory catch a request that cannot work, a real instance catches a response we misread, and the other five rules keep catching the surface we failed to expose.
