...
| Go packages               | 265 in the module (`go list ./...`); the README's 267 counts directories holding tracked `.go` files, so it adds the two `testdata` stand-in programs `go list` leaves out. 179 under `internal/tools/...` (the root package plus 178 domain sub-packages) |
