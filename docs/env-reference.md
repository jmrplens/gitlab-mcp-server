...
| `GITLAB_ENTERPRISE` | `false` | Enable Enterprise/Premium tools for GitLab Premium/Ultimate features in stdio mode. In HTTP mode, the matching `--enterprise` flag is only a fallback because CE/EE is auto-detected per token+URL pool entry when GitLab reports edition in `/api/v4/version` |
...
| `GITLAB_ENTERPRISE` | `--enterprise` | In HTTP mode, fallback only; CE/EE is auto-detected per token+URL pool entry when available |
