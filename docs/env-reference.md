...
| `CAPABILITY_SURFACE`   | `full`     | Resource and prompt catalog selector: `full` keeps the complete catalog; `minimal` keeps `gitlab://workspace/roots` plus the surface-aware `gitlab://tools` manifest and disables optional GitLab data resources, workflow guides, and prompts. Dynamic describe/find still returns action schemas inline with `minimal`                                                                                                                                                                    |
