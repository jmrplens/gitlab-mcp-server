...
| ----------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--max-http-clients`                      | `100`       | Pooled credentials, evicted least-recently-used first; upper bound 10000                                                    | Set it above your expected simultaneous population. Eviction is no longer cheap: it ends that credential's subscriptions and closes its listen streams. Size pressure prefers an entry that is not serving one and takes a busy entry only when every entry is busy, and a busy entry holds one of the two process-wide ceilings (512 listen streams, 512 watchers), so a value above their sum of 1024 makes that fallback unreachable at rest on either transport; 513 suffices on the default stateless one, where a watcher needs a stream. See [the lever](http-server-mode.md#the-lever-and-what-it-costs) |
...
| `--rate-limit-rps` / `--rate-limit-burst` | `10` / `40` | Calls a second per credential that reach GitLab, plus `tools/list` on a bucket refilled a tenth as fast with the same burst | Per credential **per process**. Several instances multiply it unless affinity pins each caller to one. See [The limiter multiplies](#the-limiter-multiplies). Size the burst for the clients that reconnect together on one credential: each lists once as it connects, and a refused listing is a JSON-RPC error the client, not a model, has to survive                                                                                                                                                                                                                                                        |
| `--action-timeout`                        | `65m`       | How long one action may run                                                                                                 | Above the longest wait any action offers. Lower it only if you would rather fail a pipeline wait than hold a goroutine; raise it if callers move files too large to upload or download in that time, since it ends a transfer too                                                                                                                                                                                                                                                                                                                                                                                |
...
| `subscription refused: this process is at its watcher ceiling`  | Logs          | A `WARN` naming the 512 watchers already running. Nothing to raise, since the ceiling is not configurable: it says this instance is saturated for subscriptions and callers are being refused                       |
