...
| ----------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--max-http-clients`                      | `100`       | Pooled credentials, evicted least-recently-used first; upper bound 10000                                                    | Set it above your expected simultaneous population. Eviction is no longer cheap: it ends that credential's subscriptions and closes its listen streams. Size pressure prefers an entry that is not serving one and takes a busy entry only when every entry is busy, so on the default stateless transport a value above the process-wide ceiling of 512 listen streams makes that fallback unreachable at rest; under `--stateless=false` no value does yet, because watchers are bounded per credential and by nothing per process. See [the lever](http-server-mode.md#the-lever-and-what-it-costs) |
