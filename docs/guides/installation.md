...
- **Two values configure it.** `GITLAB_TOKEN` is the only required setting: a Personal Access Token (`glpat-...`) with the `api` scope. A `read_api` token also works; pair it with `GITLAB_READ_ONLY=true` so the surface itself is read-only, since over stdio the writes are otherwise listed and refused by GitLab one by one (HTTP mode narrows the surface on its own). `GITLAB_URL` defaults to `https://gitlab.com`, so set it only for a self-managed instance. Everything else is optional and listed in the [configuration reference](../reference/configuration.md).
...
The settings map to environment variables: GitLab URL (`GITLAB_URL`, default `https://gitlab.com`) and Personal Access Token (`GITLAB_TOKEN`, `api` scope) are required; Tool surface (`GITLAB_MCP_TOOL_SURFACE`, default `dynamic`), GitLab tier (`GITLAB_TIER`, empty for auto-detect), Read-only mode (`GITLAB_READ_ONLY`), Safe mode (`GITLAB_SAFE_MODE`), Skip TLS verification (`GITLAB_SKIP_TLS_VERIFY`) and Log level (`GITLAB_MCP_LOG_LEVEL`, written to Claude Desktop's MCP log files) are optional. Claude Desktop stores the token in the operating system keychain, not in a plain-text file.
