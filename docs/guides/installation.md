...
The container reads the same variables as the binary. The Agent Plugins config (root `mcp.json`) forwards this set with `-e`, and `server.json` declares the same variables, minus `GITLAB_MCP_CLIENT_COMPAT`, as `environmentVariables`: `GITLAB_URL`, `GITLAB_TOKEN`, `GITLAB_MCP_SKIP_TLS_VERIFY`, `GITLAB_MCP_TOOL_SURFACE`, `GITLAB_MCP_CAPABILITY_SURFACE`, `GITLAB_MCP_META_PARAM_SCHEMA`, `GITLAB_MCP_TIER`, `GITLAB_MCP_READ_ONLY`, `GITLAB_MCP_SAFE_MODE`, `GITLAB_MCP_EMBEDDED_RESOURCES`, `GITLAB_MCP_EXCLUDE_TOOLS`, `GITLAB_MCP_IGNORE_SCOPES`, `GITLAB_MCP_UPLOAD_MAX_FILE_SIZE`, `GITLAB_MCP_ALLOWED_IMPORT_DIRS`, `GITLAB_MCP_RATE_LIMIT_RPS`, `GITLAB_MCP_RATE_LIMIT_BURST`, `GITLAB_MCP_CLIENT_COMPAT` and `GITLAB_MCP_LOG_LEVEL`. In stdio mode each one you use needs its own `-e NAME` in `args` and its value in the client's `env` block. In HTTP mode the flags in the command line take their place; see the [configuration reference](../reference/configuration.md).

For a self-signed certificate, mount the CA into the container and set `SSL_CERT_FILE=/path/to/ca-bundle.crt` rather than reaching for `GITLAB_MCP_SKIP_TLS_VERIFY=true`, which disables verification outright and which OAuth mode refuses for any non-loopback instance.
...
The settings map to environment variables: GitLab URL (`GITLAB_URL`, default `https://gitlab.com`) and Personal Access Token (`GITLAB_TOKEN`, `api` scope) are required; Tool surface (`GITLAB_MCP_TOOL_SURFACE`, default `dynamic`), GitLab tier (`GITLAB_MCP_TIER`, empty for auto-detect), Read-only mode (`GITLAB_MCP_READ_ONLY`), Safe mode (`GITLAB_MCP_SAFE_MODE`), Skip TLS verification (`GITLAB_MCP_SKIP_TLS_VERIFY`) and Log level (`GITLAB_MCP_LOG_LEVEL`, written to Claude Desktop's MCP log files) are optional. Claude Desktop encrypts the token before saving it, with a key protected by the operating system: the Keychain on macOS, DPAPI under your Windows login on Windows, and the desktop keyring (such as GNOME Keyring or KWallet) on Linux. A Linux desktop without a keyring leaves the token without that protection.
