...
> 📖 **User documentation**: See the [CI/CD Usage](https://jmrp.io/docs/gitlab-mcp-server/operations/ci-cd/) on the documentation site for a user-friendly version.
>
...
```bash
curl -sSL "https://github.com/jmrplens/gitlab-mcp-server/releases/download/v2.1.0/gitlab-mcp-server-linux-amd64" \
  -o gitlab-mcp-server
chmod +x gitlab-mcp-server
```
...
2. Create a token with the required scope: `api` for full read/write access to all available tools, or `read_api` for read-only operations (list, get, search tools).
...
- [Configuration](../reference/configuration.md) — Environment variables and server setup
...
- [Environment Variables](../reference/env.md) — Complete variable reference
- [Security](../concepts/security.md) — Token management and security model
