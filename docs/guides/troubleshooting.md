...
| `x509: certificate has expired`                 | Expired TLS certificate                    | Renew the certificate on the GitLab server. `GITLAB_SKIP_TLS_VERIFY=true` is a temporary workaround in legacy mode only                                                                                                                                                  |
...
| Updates never arrive under `npx`                                    | The launcher sets `AUTO_UPDATE=false` — npm owns the binary                                                                                                                                                                                                                             | Expected. Upgrade with npm, or set `AUTO_UPDATE` explicitly to override the launcher                                                                                                                                                                  |
