...
| Symptom                                                     | Cause                                          | Solution                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| MCP client shows hundreds of individual tools instead of 32 | Individual surface selected                    | Set `TOOL_SURFACE=meta` to use 32 base meta-tools, 48 self-managed enterprise meta-tools, or 49 GitLab.com Enterprise meta-tools instead of the individual catalog                                                                                                                                                                                                                                                                         |
...
| `403 Forbidden` with `error="insufficient_scope"`                     | The token is genuine but lacks the scope this deployment requires       | Reauthorize the application with the scope named in the challenge's `scope` parameter. A read-only deployment asks for `read_api`; any other asks for `api`, and an `api` token satisfies both |
...
| Same invalid token keeps returning `401` after it was fixed in GitLab | The rejection is cached for five minutes to keep replays off GitLab     | Wait for the entry to expire, or restart the server. Only definitive rejections are cached; throttling and outages never are                                                                   |
