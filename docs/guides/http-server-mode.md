...
| Rate limits and HTTP pool policy    | `--rate-limit-rps`, `--rate-limit-burst`, `--max-http-clients`, `--session-timeout`, `--revalidate-interval` and `--pool-idle-timeout`, each falling back to `GITLAB_MCP_RATE_LIMIT_RPS`, `GITLAB_MCP_RATE_LIMIT_BURST`, `GITLAB_MCP_MAX_HTTP_CLIENTS`, `SESSION_TIMEOUT`, `SESSION_REVALIDATE_INTERVAL` and `GITLAB_MCP_POOL_IDLE_TIMEOUT`; `--http-idle-timeout`, `--trusted-proxy-header`, `--trusted-proxies`, `--stateless` and `--json-response` are CLI-only                                                                                                                                                                              | No                                          | Ignored and logged in `ignored_options` when sent as config-like headers such as `RATE-LIMIT-RPS`                                                                                                                                    |
