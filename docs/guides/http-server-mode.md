...
| `config_digest`  | Twelve hex characters over the settings that decide what a client sees: tool surface, capability surface, meta parameter schema, tier, read-only, safe mode and the excluded tools. Instances that must serve the same catalog must report the same digest; it is a digest of settings, none of which is recoverable from it |
