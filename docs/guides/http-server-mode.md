...
| `--rate-limit-rps`              | `10`           | Per-server rate limit, in requests per second, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get` (`0` disables it). On by default in HTTP mode; falls back to `GITLAB_MCP_RATE_LIMIT_RPS`                                                                                                      |
