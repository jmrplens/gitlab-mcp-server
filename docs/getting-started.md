...
By default, the server registers **40 meta-tools** (59 with `GITLAB_ENTERPRISE=true`) — domain-grouped dispatchers that reduce token overhead. Each meta-tool handles multiple actions via an `action` parameter.
