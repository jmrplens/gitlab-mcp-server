...
| `GITLAB_MCP_RATE_LIMIT_RPS`       | No       | Per-credential rate limit, in requests per second, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get`); `0` disables it (default `0`) |
