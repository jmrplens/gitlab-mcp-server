...
Both modes authenticate with a **Personal Access Token** (PAT) or **Project Access Token**. The server supports the full 1004-tool surface when using a token with `api` scope.
...
2. Create a token with the required scope:
   - `api` — full read/write access to all 1004 tools
