...
The [dynamic toolset](../dynamic-tools.md) reuses this same meta-tool route catalog behind a hidden registry. In dynamic mode, clients see only `gitlab_search_tools`, `gitlab_describe_tools`, and `gitlab_execute_tool`, then discover and execute the same canonical domain actions progressively.
