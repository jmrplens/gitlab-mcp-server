...
> **Individual tools**: 23
> **Meta-tools**: `gitlab_environment` (default `TOOL_SURFACE=meta` catalog). Protected environment actions use `protected_*` prefix, freeze period actions use `freeze_*` prefix, deployment actions use `deployment_*` prefix.
...
With the default `TOOL_SURFACE=meta` catalog, the 23 individual tools below are consolidated into one meta-tool (`gitlab_environment`) that dispatches by `action` parameter.
