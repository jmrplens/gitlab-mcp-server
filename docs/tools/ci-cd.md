...
> **Individual tools**: 59
> **Meta-tools**: `gitlab_pipeline`, `gitlab_job`, `gitlab_ci_variable`, `gitlab_pipeline_trigger`, `gitlab_instance_variable` (when `META_TOOLS=true`, default). Pipeline schedule actions are accessed via `gitlab_pipeline` with `schedule_*` action prefix.
