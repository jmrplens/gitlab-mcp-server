...
| `--tier`                 | _(detected)_                 | Force the licensing tier: `free`, `ce`, `premium`, or `ultimate`. When set, used verbatim with no license check. When omitted, HTTP mode detects the tier per token+URL pool entry from the instance license (fallback `free`) |
