...
| `META_TOOLS` | `true` | Tool catalog selector: `true` for domain meta-tools, `false` for individual tools, `dynamic` for the low-token search/describe/execute surface, or experimental `dynamic-2`/`dynamic-3` comparison surfaces |
...
| **Dynamic toolset** | `TOOL_SURFACE=dynamic` or `META_TOOLS=dynamic` | `gitlab_search_tools`, `gitlab_describe_tools`, `gitlab_execute_tool` | Low-token clients that can search, describe, then execute actions |
