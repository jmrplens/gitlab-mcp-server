...
Meta-tools group related GitLab operations under a single MCP tool with an `action` parameter. Instead of 1005 individual tools, **28 domain meta-tools** (43 with `GITLAB_ENTERPRISE=true`) provide the same functionality while reducing token overhead for LLMs.
...
| Individual tools (`false`) | 1005       | Clients that benefit from granular tool discovery                |
