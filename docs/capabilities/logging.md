...
This means **every single tool call** (up to 1045 individual tools on GitLab.com Enterprise/Premium) generates a structured log entry with the tool name, execution duration, and success or failure status — with zero manual instrumentation per tool.
