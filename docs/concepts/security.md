...
### When the arguments decide, and not the action

One bit per action cannot express an action that destroys something for some arguments and nothing for the rest. Such an action is **not** classified destructive, because the bit would then be wrong for the calls that make up most of its traffic, and a confirmation raised by every ordinary edit is one a caller learns to pass through, which costs more than it buys on the calls that need it. It is guarded in its own handler instead, on the same precedence as the catalog bit: `GITLAB_MCP_YOLO_MODE` and an explicit `confirm=true` proceed, an eliciting client is asked, and a client that cannot prompt is refused with an error naming what would be lost and how to confirm. The reserved `confirm` key is read from the raw tool call by `toolutil.ExplicitConfirmFromRequest`, which is what lets it work identically on all three surfaces, and the action declares a `confirm` field so its input schema advertises the key.

Two actions are guarded that way today, and they are the whole of the list:

- `issue.work_item_update` (`gitlab_update_work_item`): an explicit empty `assignee_ids` or `crm_contact_ids` replaces the current list with nothing. Omitting the field leaves it untouched, so the destructive call and the ordinary one differ only in an argument.
- `project.pull_mirror_configure` (`gitlab_project_pull_mirror_configure`) — `mirror_overwrites_diverged_branches` makes every sync replace this project's diverged branches with the source's, and [GitLab documents that](https://docs.gitlab.com/user/project/repository/mirror/pull/) as "the loss of local changes"; the replaced commits are unreachable afterwards and no call here brings them back. The guard asks when the configuration this call leaves behind both pulls and overwrites, and when it is this call that arms or re-aims it, by setting the flag, by enabling the mirror, or by pointing it at another URL. Repointing counts because the overwrite may already be stored: aiming an overwriting mirror at a repository named in this call is the same outcome reached in two steps.

Pull-mirror configuration is deliberately **not** destructive as a whole, which is the decision recorded in [issue 674](https://github.com/jmrplens/gitlab-mcp-server/issues/674). Without the overwrite flag GitLab "stops updating the branch", which it states is there to prevent data loss, so an ordinary mirror adds branches and commits and removes nothing; `enabled=false` is the opposite call; and unlike a push mirror nothing of the project leaves the instance, since the credentials in the call are the caller's to supply rather than the project's to leak. Classifying the action would also put a confirmation in front of turning a hostile mirror **off**, which is the guard working backwards.
...
- **Host validation** — A request whose `Host` names a host nobody declared is refused with `403`, on every route, to mitigate DNS rebinding. Declared means: the host `--http-addr` binds when it names one, the loopback family, and **the host `--public-url` advertises**, which is what makes a reverse proxy that preserves the client's `Host` work — every proxy configuration in [Remote Deployment](../guides/remote-deployment.md#behind-a-reverse-proxy) preserves it. A request from an address in `--trusted-proxies` may carry whatever host that hop forwards, since the operator has vouched for it, and a request carrying no `Host` at all is served, because a balancer health check sends none and no browser omits it. Binding to all interfaces (`0.0.0.0` or `::`) declares no host of its own: such a listener refuses an undeclared `Host` only on a connection accepted over loopback, which is what a rebinding attack has to reach, and answers whatever a proxy forwards on any other address. A unix socket listener applies none of it: rebinding needs a name that resolves to the listener, and no name resolves to a file, while the `Host` a socket client sends is whatever its HTTP library needed to build the request. The check lives in `cmd/server/host_guard.go`, in one place: the SDK's own localhost protection is switched off, since it cannot see either flag and refused both exceptions
...
- **Outbound destination guard** — the instance a caller names in `GITLAB-URL` under `--allow-any-gitlab-url` may not be a private, loopback or CGNAT address unless `--allow-private-instances` is passed, and may never be a cloud metadata address. See [Outbound Destinations](#outbound-destinations)
...
- **Least-privilege scope** — admission asks only for `read_api`, the least any action needs, and the write check is applied per action instead. A `read_api` token is therefore accepted by a deployment that writes, and is served the read-only tool surface; the only credential refused at the door is one carrying no GitLab API scope at all. The metadata's `scopes_supported` advertises one scope, `api`, or `read_api` on a read-only deployment (`--read-only` or `--safe-mode`): a client asks GitLab for every scope listed, and GitLab refuses a request naming a scope the OAuth application does not have, so listing both broke every application checked for only one. A client that wants a credential which cannot mutate anything names `read_api` itself. An `api` token satisfies the minimum everywhere, since `api` is a superset
...
Four layers sit in front of that, in this order:

1. **Per-address failure budget**: ten authentication failures in one minute block the address for the rest of the window, answered `429` with `Retry-After`. Checked before anything that could reach GitLab, so a blocked caller costs nothing at all. Failures at this layer and at the pool share one budget, so exhausting one does not earn a fresh allowance at the other. The count and the window are `--auth-failure-limit` and `--auth-failure-window`; a limit of `0` turns this budget off rather than blocking on the first failure.

   What the block refuses is an authentication, not an address. A credential this deployment is **already serving** is admitted from a blocked address too: in legacy mode one the credential pool holds an entry for, in OAuth mode one the verified-identity cache holds. Both answers come from a map, so the budget still bounds exactly what it exists to bound, the distinct credentials one address can have verified upstream, and a request it admits reaches GitLab no more than any other pool hit does. The distinction matters because an address is not a client: a corporate NAT, a campus, a mobile carrier and a reverse proxy running without `--trusted-proxy-header` all present one address for many people, and charging the address alone let one of them relaying invented tokens answer `429` to all the others for the rest of the window. Producing a credential the deployment already serves means holding one GitLab accepts, which is not the state a token sprayer is in.
2. **Distinct-credential budget**: fifty *different* credentials refused for one address inside ten minutes block it, for a minute, then ten, then an hour, reset by an hour of silence. It asks a different question from the budget above, and the difference is what makes it safe to escalate on: ten failures in a minute is a stuck client retrying one bad token as much as it is an attack, while fifty distinct invalid tokens is only an attack, because a person has one token and a fleet behind a NAT has one each. A legitimate neighbor therefore never reaches it however badly its client misbehaves.

   What it protects is not really this server, which refuses those requests cheaply either way. Every distinct credential that reaches verification is a request to GitLab from the deployment's own address, and GitLab rate-limits failed authentication per source address, so a sprayer is spending the deployment's standing upstream and the throttle GitLab eventually applies lands on everyone using it. The distinct count is the earliest signal of that, earlier than the failure count.

   Credentials are held as truncated SHA-256 digests and never in the clear, both tables are bounded, and the admitted-credential exemption above applies here too. `--auth-distinct-token-limit` and `--auth-distinct-token-window` set it, `0` turns it off, and the escalation ladder is derived from `--auth-failure-window` rather than configured separately.
3. **Rejected-token cache**: a token GitLab has already refused is refused from memory for five minutes without asking again. Keyed by SHA-256 digest, never the raw credential, and bounded to 4096 entries because the keys come from the caller.
4. **Verified-identity cache**: the existing positive cache, so an accepted token is verified once per TTL rather than once per request.
...
| Brute force            | Per-address failure budget (10/minute) plus a rejected-token cache, so repeated attempts never reach GitLab; a credential the deployment already serves is exempt from the block and answered from memory             |
...
Everything above is about what reaches this server. This is about where this server connects **to**, which is the other half of a request-forgery question and applies to both transports.

Two destinations are not chosen by the operator. One is an instance a caller named in the `GITLAB-URL` header under `--allow-any-gitlab-url`: the caller is nominally the operator on that single-user local deployment, but the thing choosing the header is a model that has read issue bodies and CI logs, so it is a network scanner driven by untrusted text. The other fires on an ordinary pinned deployment: `credentialSafeRedirect` follows a cross-host 302 because six shipped read-only actions depend on it (GitLab answers artifact, trace and package reads with a redirect to object storage), and the hop's destination is chosen by whatever answered rather than by anyone here. The body comes back to the caller in both cases.

The guard runs as `net.Dialer.ControlContext`, so it fires **after DNS resolution and once per candidate address**: a name that resolves to something other than what it claims is judged by what it resolved to, with no window between the check and the connection. One hook covers the first request and every redirect hop, since every transport the server builds dials through a dialer carrying it. Behind an outbound proxy the dialer sees only the proxy, so the destination is judged per request in `destinationTransport` instead, as far as its URL spells it (see the last paragraph of this section).

| Tier | What it refuses                                                                                      | Where it applies                                                                                                        | Opt-out                                                                                 |
| ---- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| A    | The cloud metadata addresses: `169.254.169.254`, `169.254.170.2`, `fd00:ec2::254`, `100.100.100.200` | Every hop this server dials, and behind a proxy every hop whose URL spells the address, for every client and deployment | None. Nothing legitimate serves a GitLab API or a presigned object-storage URL from one |
| B    | Loopback, RFC 1918, CGNAT `100.64.0.0/10`, link-local, unique-local and unspecified addresses        | A `GITLAB-URL` header under `--allow-any-gitlab-url`, and a redirect hop that left the instance's own host              | `--allow-private-instances` / `GITLAB_MCP_ALLOW_PRIVATE_INSTANCES=true`                 |

**An address the operator named is exempt, whatever it resolves to.** `--gitlab-url` and `GITLAB_URL` are the operator's own configuration, so a GitLab on `localhost`, on `10.x`, on `192.168.x` or behind a VPN on CGNAT works with nothing set. There is no exception list, and there is deliberately no way to make the guard stricter: the point is to constrain destinations the operator did not choose, and DNS rebinding is a threat only when an attacker controls the name. See [ADR-0022](../development/adr/adr-0022-operator-named-destinations-are-exempt.md) before narrowing it.

One case inside tier B is allowed without the flag: a redirect to a private address when the instance the operator configured **itself** resolves private, which is a self-managed GitLab with its object store on the same network. Tier A still applies there. An instance a caller named never gets this: its first hop is already refused a private address, so the only way it could look private afterwards is a name answering differently the second time it is asked, which is DNS rebinding by whoever holds the name. A caller-named instance that really is private needs `--allow-private-instances`.

**A reused connection is judged by the pool it sits in.** A request served from an idle keep-alive connection is not dialed, so the hook never sees it, and Go keys those connections on scheme, host and port with no notion of the policy they were opened under. Requests are therefore split across two connection pools (shared by every client that verifies certificates; a client that skips verification has a pair of its own) by what tier B answers for them, using the same predicate the dialer applies. A request tier B would refuse a private address, which is a caller-named instance, a redirect hop away from one, and a redirect hop away from a public instance, is only ever handed a connection that was itself dialed under that refusal, and so leads to a public address. Every request to the operator's own instance stays on the other pool, so ordinary traffic reuses connections exactly as before. Until [issue 937](https://github.com/jmrplens/gitlab-mcp-server/issues/937) there was one pool, and a refused request could be served the connection a permitted one had opened to the same host.

**Behind an outbound proxy, the guard judges the proxy dial with tier A alone, and the destination only as far as it is spelled.** With `HTTP_PROXY` or `HTTPS_PROXY` set, the connection this server opens is to the proxy, which is the operator's own configuration, so a proxy on `localhost` or on a private network is not refused for its own address, and one on a cloud metadata address always is. (A proxy host net/http rewrites to its IDNA form is the exception: it is judged as the destination, as every proxy used to be.) What the proxy is asked to reach is never dialed here. A destination the URL writes as an address, such as a redirect to `http://169.254.169.254/` or a `GITLAB-URL` header naming `http://10.0.0.1`, is judged before anything is sent, under the same two tiers as a direct connection; a destination written as a name is resolved by the proxy, so what it reaches, a cloud metadata endpoint included, is the proxy's decision and nothing here can see it. An instance a caller named under `--allow-any-gitlab-url` gets the same answer: the caller chose the instance, not the proxy. Until [issue 942](https://github.com/jmrplens/gitlab-mcp-server/issues/942) a proxy was judged as though it were the destination of every request, so behind one on a private address every redirect off a public instance, an artifact download to object storage among them, was refused, while behind one on a public address, and behind a private one for a hop off an instance that is itself private, even a redirect to the metadata address went through.
