...
One case inside tier B is allowed without the flag: a redirect to a private address when the configured instance **itself** resolves private, which is a self-managed GitLab with its object store on the same network. Tier A still applies there.
