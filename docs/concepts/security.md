...
What this deliberately does not cover: a proxy named by `HTTPS_PROXY` is what gets dialed, so the guard judges the proxy and the destination behind it is that proxy's decision.
