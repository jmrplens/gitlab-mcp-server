...
2. **Distinct-credential budget**: fifty *different* credentials refused for one address inside ten minutes block it, for a minute, then ten, then an hour, reset by an hour of silence. It asks a different question from the budget above, and the difference is what makes it safe to escalate on: ten failures in a minute is a stuck client retrying one bad token as much as it is an attack, while fifty distinct invalid tokens is only an attack, because a person has one token and a fleet behind a NAT has one each. A legitimate neighbour therefore never reaches it however badly its client misbehaves.
