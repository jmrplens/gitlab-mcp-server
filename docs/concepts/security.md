...
- `work_item.update` (`gitlab_update_work_item`) — an explicit empty `assignee_ids` or `crm_contact_ids` replaces the current list with nothing. Omitting the field leaves it untouched, so the destructive call and the ordinary one differ only in an argument.
