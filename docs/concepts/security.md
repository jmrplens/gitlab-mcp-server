...
- **Host validation** — When listening on a specific local host, requests with unexpected `Host` headers are rejected to mitigate DNS rebinding attacks. Binding to all interfaces (`0.0.0.0` or `::`) leaves Host validation to the reverse proxy deployment. A unix socket listener performs none: rebinding needs a name that resolves to the listener, and no name resolves to a file, while the `Host` a socket client sends is whatever its HTTP library needed to build the request
