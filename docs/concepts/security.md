...
The guard runs as `net.Dialer.ControlContext`, so it fires **after DNS resolution and once per candidate address**: a name that resolves to something other than what it claims is judged by what it resolved to, with no window between the check and the connection. One hook covers the first request and every redirect hop, since every transport the server builds dials through a dialer carrying it.

| Tier | What it refuses                                                                                      | Where it applies                                                                                           | Opt-out                                                                                 |
| ---- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| A    | The cloud metadata addresses: `169.254.169.254`, `169.254.170.2`, `fd00:ec2::254`, `100.100.100.200` | Every hop, every client, every deployment                                                                  | None. Nothing legitimate serves a GitLab API or a presigned object-storage URL from one |
