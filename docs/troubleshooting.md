...
| Tool not found in `tools/list` | Tool not registered, or tool surface mismatch | Check the active tool surface. `TOOL_SURFACE` is the preferred mode selector and overrides legacy `META_TOOLS` when both are set. Use `TOOL_SURFACE=individual` or `META_TOOLS=false` for one tool per operation, `TOOL_SURFACE=meta` or `META_TOOLS=true` for consolidated domain meta-tools, and `TOOL_SURFACE=dynamic` for the supported low-token search/describe/execute surface. See [Configuration](configuration.md#tool-modes) and [Dynamic Toolset](dynamic-tools.md) for mode selection details |
