...
| `GITLAB_MCP_EMBEDDED_RESOURCES`   | `true`               | Embed the canonical `gitlab://` MCP resource URI as an `EmbeddedResource` content block in the get results that carry one (today `issue.get`). Set `false` to disable for clients that don't tolerate duplicate content blocks                                                                                                                                                                                       |
