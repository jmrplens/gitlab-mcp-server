...
| `GITLAB_MCP_EXCLUDE_TOOLS`           | *(empty)*            | Comma-separated tool names, group names or action IDs, excluded from the tool surface and from the resources, subscriptions, prompts and argument completions that return the same objects, so the removal holds on every request path. The same spellings reach the standalone utilities on every surface (`gitlab_interactive`, `interactive.issue_create`, `discover_project.resolve`), and an entry that names nothing is logged as a startup warning rather than refused                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `--exclude-tools`          | *(empty)*      | Comma-separated tool names, group names or action IDs, excluded from the tool surface and from the resources, subscriptions, prompts and argument completions that return the same objects, so the removal holds on every request path. The same spellings reach the standalone utilities on every surface, and an entry that names nothing is logged as a startup warning                                                                                                                                                                                                                      |
