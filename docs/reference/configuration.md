...
Started by hand with neither `GITLAB_TOKEN` nor `GITLAB_URL` set, and with a
terminal attached, the server prints what it is and what it needs, then waits
for Enter. Double-clicking the binary on Windows reaches the same screen, and
...
| `GITLAB_MCP_ACTION_TIMEOUT`   | `1h`      | Cancel an action still running after this long; `0` disables it (upper bound 24h). Both transports; HTTP mode also has `--action-timeout`                                                                                           |
| `GITLAB_MCP_RATE_LIMIT_RPS`   | `0`       | Per-server rate limit, in requests/second, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `prompts/get` (`0` disables it). Stdio only; HTTP mode defaults to `10` via `--rate-limit-rps` |
...
| `--gitlab-url`             | *(optional)*   | GitLab instance URL. Omit it to require each client to send `GITLAB-URL` per request. Repeatable, and a single comma-separated value spells the same list: the first instance is the default, and `GITLAB-URL` then selects among the published ones                                                                         |
...
| `--resource-documentation` | *(empty)*      | https URL published as RFC 9728 `resource_documentation`; point it at a page describing your own OAuth application (its client ID and registered redirect URIs). Empty publishes this project's OAuth setup guide                                                                                                            |
...
| `--action-timeout`         | `1h`           | Cancel an action still running after this long; `0` disables it (upper bound 24h). Falls back to `GITLAB_MCP_ACTION_TIMEOUT`                                                                                                                                                                                                 |
...
| `--rate-limit-rps`         | `10`           | Per-server rate limit, in req/s, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `prompts/get` (`0` disables it). On by default because an HTTP deployment is shared: one looping client's calls are charged to its egress address                                                 |
...
No `GITLAB_TOKEN` is needed at startup — each client provides its own token per-request via `PRIVATE-TOKEN` header or `Authorization: Bearer`. The `GITLAB-URL` header follows how many instances the server published: with none it selects the instance per request; with exactly one `--gitlab-url` that instance is authoritative and the header is ignored and logged; with several (`--gitlab-url` repeated, or one comma-separated value) the header selects among them, and a value naming an unpublished instance is refused — `403` in OAuth mode, `400` in legacy mode — rather than silently served the default.
