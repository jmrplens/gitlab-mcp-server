...
| `CLIENT_COMPAT`          | `auto`               | Per-client response compatibility: OpenAI Codex sessions get the float `priority` in annotations rounded to 0 or 1 (their bundled parser rejects non-integer values); everything else is delivered unchanged. Set `off` to disable. See [Client Compatibility](../guides/client-compatibility.md)                                                                      |
...
| **Individual tools**          | `TOOL_SURFACE=individual` | 893 CE / 1061 self-managed enterprise / 1067 GitLab.com Enterprise | Clients that need granular tool selection                                     |
