...
| `GITLAB_MCP_SAFE_MODE`               | `false`              | Safe mode: intercepts mutating operations and returns a JSON preview instead of executing. Interception is per action, so read operations keep working on every surface and the preview names the canonical action (e.g. `issue.create`). If `GITLAB_MCP_READ_ONLY=true`, it takes precedence                                                                                                                                                         |
