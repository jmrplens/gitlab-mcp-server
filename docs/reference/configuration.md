...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------- |
| `meta`       | Applies to every visible domain meta-tool. `opaque` shows `{action, params}`; `compact` and `full` inline per-action `oneOf` schemas | `gitlab://tools` and `gitlab://tools/{id}` in full or minimal | Not applicable                                                   | `full` is 18.3x larger than `opaque`; `compact` is 8.7x larger | Keep `opaque`; use `gitlab://tools` for exact params |
