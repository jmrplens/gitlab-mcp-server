...
The switches that already began with `GITLAB_` were renamed as well, so that every variable of this server reads alike: `GITLAB_TIER`, `GITLAB_READ_ONLY`, `GITLAB_SAFE_MODE`, `GITLAB_IGNORE_SCOPES` and `GITLAB_SKIP_TLS_VERIFY` are now `GITLAB_MCP_TIER`, `GITLAB_MCP_READ_ONLY`, `GITLAB_MCP_SAFE_MODE`, `GITLAB_MCP_IGNORE_SCOPES` and `GITLAB_MCP_SKIP_TLS_VERIFY`, and `YOLO_MODE` is `GITLAB_MCP_YOLO_MODE`. **The old spellings stopped working in 3.1.0.** They were to have gone in 3.0.0 and were held back one release, because 2.7.5 carries a self-updater and 3.0.0 does not: a 2.7.5 deployment updates itself into 3.0.0 with nobody reading a release note, so removing them there would have broken those deployments in silence. 3.1.0 is the first version nobody is carried into. `GITLAB_ENTERPRISE`, deprecated before this in favour of the tier, stays as it is.
...
| `GITLAB_MCP_SKIP_TLS_VERIFY`         | `false`              | Skip TLS certificate verification for self-signed certs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `GITLAB_MCP_TIER`                    | *(detected)*         | Licensing tier: `free`/`ce`, `premium`, or `ultimate`. When set, used verbatim with no license check. When unset, detected in two steps: `GET /license` (the instance plan, readable only by an administrator on self-managed and by nobody on GitLab.com), then the plan of the namespaces the token administers (`GET /namespaces`), which is the real plan on GitLab.com and always `default` on self-managed. That listing is paged, stopping at the first `ultimate` and after 10 pages of 100 either way, so an account administering more than 1,000 namespaces whose only paid one sits past that bound resolves lower than it holds. Where neither step answers, the tier is `free` and an enterprise instance logs a warning naming this setting. In HTTP mode use `--tier`; when omitted the tier is detected per token+URL pool entry. Enterprise/Premium tools are gated when the resolved tier is Premium or Ultimate |
| `GITLAB_MCP_READ_ONLY`               | `false`              | Read-only mode: removes every mutating operation at startup while read operations keep working. Filtering is per action, so a meta-tool or `gitlab_execute_action` that serves both reads and writes keeps serving its reads                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `GITLAB_MCP_SAFE_MODE`               | `false`              | Safe mode: intercepts mutating operations and returns a preview card instead of executing. Interception is per action, so read operations keep working on every surface and the preview names the canonical action (e.g. `issue.create`) and echoes the arguments. If `GITLAB_MCP_READ_ONLY=true`, it takes precedence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `GITLAB_MCP_EXCLUDE_TOOLS`           | *(empty)*            | Comma-separated tool names, group names or action IDs, excluded from the tool surface and from the resources, subscriptions, prompts and argument completions that return the same objects, so the removal holds on every request path. The same spellings reach the standalone utilities on every surface (`gitlab_interactive`, `interactive.issue_create`, `discover_project.resolve`), and an entry that names nothing is logged as a warning rather than refused. The warning is written when the catalog for a configuration is first built: at startup on stdio, on the first request for that configuration in HTTP mode. It also names the dynamic surface's own `gitlab_find_action` and `gitlab_execute_action`, although that surface removes them by name                                                                                                                                                              |
| `GITLAB_MCP_IGNORE_SCOPES`           | `false`              | Skip PAT scope detection and register all tools regardless of token permissions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `GITLAB_MCP_PPROF_ADDR`              | *(empty)*            | Serve Go's profiling handlers on this loopback address (`127.0.0.1:6060`), on a listener of their own; a host that is not loopback is refused at startup. Empty serves nothing. The `--pprof-addr` flag sets the same variable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `GITLAB_MCP_ALLOW_PRIVATE_INSTANCES` | `false`              | Permit a private, loopback or CGNAT address as a destination the operator did not choose: an instance a caller named in `GITLAB-URL` under `--allow-any-gitlab-url`, or a redirect hop that left the configured instance. An address `GITLAB_URL` named is never checked, and cloud metadata addresses are always refused. The `--allow-private-instances` flag sets the same variable. See [Security](../concepts/security.md#outbound-destinations)                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `GITLAB_MCP_YOLO_MODE`        | `false`   | Skip destructive action confirmation prompts                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `AUTOPILOT`                   | `false`   | Same as `GITLAB_MCP_YOLO_MODE` — skip confirmation prompts                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `GITLAB_MCP_RATE_LIMIT_RPS`   | `0`       | Per-credential rate limit, in requests/second, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get` (`0` disables it), plus `tools/list` on a bucket of its own refilled a tenth as fast and holding the same burst, charged because it spends the processor every tenant shares rather than because it reaches GitLab. Both transports: the default is `0` in stdio and `10` in HTTP mode, where `--rate-limit-rps` overrides it |
...
| **Meta-tools**                | `GITLAB_MCP_TOOL_SURFACE=meta`       | 34 base / 51 self-managed Ultimate / 52 GitLab.com Ultimate                    | Clients that prefer consolidated domain dispatchers with `action` parameters  |
| **Individual tools**          | `GITLAB_MCP_TOOL_SURFACE=individual` | 866 Free/CE / 1085 self-managed Ultimate / 1091 GitLab.com Ultimate with Orbit | Clients that need granular tool selection                                     |

Use the default dynamic surface for normal low-token deployments. Set `GITLAB_MCP_TOOL_SURFACE=meta` only when a client or workflow prefers domain meta-tools.
...
| Tool surface | Visible tool schema impact                                                                                                           | Tool manifest availability                                    | Dynamic describe behavior                                        | Schema-size impact                                                                                                                                                                                                                                       | Recommended use                                      |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `meta`       | Applies to every visible domain meta-tool. `opaque` shows `{action, params}`; `compact` and `full` inline per-action `oneOf` schemas | `gitlab://tools` and `gitlab://tools/{id}` in full or minimal | Not applicable                                                   | Served meta-tool input schemas, summed as serialized bytes: `full` is 18.3x the `opaque` total, `compact` 8.7x. Not an aggregate startup-token measurement — resources and prompts are counted in neither. `go run ./cmd/audit_tokens --compare-schemas` | Keep `opaque`; use `gitlab://tools` for exact params |
...
| `--allow-any-gitlab-url`   | `false`        | Start with no instance published and let `GITLAB-URL` name any host. For a single-user local deployment where the operator is the caller; it warns at startup, has no environment variable on purpose, and is refused unless `--http-addr` binds a loopback address or a unix socket                                                                                                                                                                                                                                                                                                            |
...
| `--rate-limit-rps`         | `10`           | Per-credential rate limit, in req/s, on every call that reaches GitLab: `tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get` (`0` disables it), plus `tools/list` on a bucket of its own refilled a tenth as fast and holding the same burst, charged because it spends the processor every tenant shares rather than because it reaches GitLab. The bucket lives on the pool entry, so each token and URL pair is limited on its own. On by default because an HTTP deployment is shared: one looping client's calls are charged to its egress address |
| `--rate-limit-burst`       | `40`           | Token-bucket burst size when `--rate-limit-rps` > 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `--exclude-tools`          | *(empty)*      | Comma-separated tool names, group names or action IDs, excluded from the tool surface and from the resources, subscriptions, prompts and argument completions that return the same objects, so the removal holds on every request path. The same spellings reach the standalone utilities on every surface, and an entry that names nothing is logged as a warning when the first request for a configuration builds its catalog. The warning also names `gitlab_find_action` and `gitlab_execute_action`, although the dynamic surface removes them by name                                    |
