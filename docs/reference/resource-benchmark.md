...
build, both named in the generated text under that heading; each chart repeats
along its bottom edge the part of it a figure lifted out of this page cannot
be read without, which is how many logical processors measured, on which
operating system and kernel, and when. A reader who wants their own numbers
can run the same target rather than trusting these.
...
<picture>
  ...
  <img alt="Lines on a log scale of credentials showing each surface's peak resident memory per step and the count each series stopped at." src="benchmarks/series-memory.light.svg">
</picture>
...
#### http, meta surface: 4 in flight per credential, 10 s per step
...
#### http, individual surface: 2 in flight per credential, 10 s per step
