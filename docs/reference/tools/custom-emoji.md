...
> **Domain**: Custom Emoji
> **Individual tools**: 3
> **Meta-tool**: `gitlab_custom_emoji` (`GITLAB_MCP_TOOL_SURFACE=meta` catalog)
...
On the default dynamic surface, these operations are the `custom_emoji.*` entries of the canonical action catalog: find them with `gitlab_find_action` and run them with `gitlab_execute_action` by `domain.action` ID. With `GITLAB_MCP_TOOL_SURFACE=individual`, each is the tool named in the tables below.

With `GITLAB_MCP_TOOL_SURFACE=meta`, all 3 individual tools below are consolidated into a single `gitlab_custom_emoji` meta-tool that dispatches by `action` parameter.
