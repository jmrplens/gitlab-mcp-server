...
> **Domain**: Runners
> **Individual tools**: 34
> **Meta-tool**: `gitlab_runner` (`GITLAB_MCP_TOOL_SURFACE=meta` catalog, 34 actions)
...
On the default dynamic surface, these operations are the `runner.*` entries of the canonical action catalog: find them with `gitlab_find_action` and run them with `gitlab_execute_action` by `domain.action` ID. With `GITLAB_MCP_TOOL_SURFACE=individual`, each is the tool named in the tables below.

With `GITLAB_MCP_TOOL_SURFACE=meta`, the 34 individual tools below are consolidated into a single `gitlab_runner` meta-tool that dispatches by `action` parameter.
