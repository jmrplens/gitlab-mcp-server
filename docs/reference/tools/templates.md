...
> **Domain**: Templates
> **Individual tools**: 12
> **Meta-tool**: `gitlab_template` (`GITLAB_MCP_TOOL_SURFACE=meta` catalog — also includes CI lint actions from the `cilint` sub-package)
...
On the default dynamic surface, these operations are the `ci_catalog.*`, `pipeline.*`, `repository.*`, `template.*` entries of the canonical action catalog: find them with `gitlab_find_action` and run them with `gitlab_execute_action` by `domain.action` ID. With `GITLAB_MCP_TOOL_SURFACE=individual`, each is the tool named in the tables below.

With `GITLAB_MCP_TOOL_SURFACE=meta`, all 12 template tools are consolidated into a single `gitlab_template` meta-tool. The meta-tool also includes CI lint actions (`lint`, `lint_project`) from the `cilint` sub-package for convenience.
