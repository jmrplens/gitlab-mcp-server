...
> **Domain**: Users & Todos
> **Individual tools**: 64
> **Meta-tools**: `gitlab_user` (`GITLAB_MCP_TOOL_SURFACE=meta` catalog)
...
On the default dynamic surface, these operations are the `user.*` entries of the canonical action catalog: find them with `gitlab_find_action` and run them with `gitlab_execute_action` by `domain.action` ID. With `GITLAB_MCP_TOOL_SURFACE=individual`, each is the tool named in the tables below.

With `GITLAB_MCP_TOOL_SURFACE=meta`, the individual tools below are consolidated into domain-specific meta-tools that dispatch by `action` parameter.
