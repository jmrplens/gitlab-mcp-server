...
| `-stateless`            | bool     | `false`                    | Sessionless streamable HTTP (SEP-2567 / protocol 2026-07-28): no `Mcp-Session-Id` tracking, every POST is self-contained, GET/DELETE return `405`. See [HTTP Server Mode — Stateless Mode](../guides/http-server-mode.md#stateless-mode) |
...
# Stateless streamable HTTP (SEP-2567) with plain JSON responses
gitlab-mcp-server --http --gitlab-url=https://gitlab.com --stateless --json-response
