...
| `-action-timeout`         | duration | `65m`                      | Cancel an action still running after this long; `0` disables it (upper bound: 24h). Both transports: stdio reads `GITLAB_MCP_ACTION_TIMEOUT`. Above the longest wait any action offers (a pipeline wait caps itself at 3600 s), so it ends a handler nobody else bounds rather than a legitimate call                                                                                                                                                                                                                                                                                                                   |
