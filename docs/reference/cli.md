...
| `-tool-search`               | string | _(empty)_ | Search registered tools by name or description, then exit. Honors `--tool-surface` and `--tier`, so the search covers the surface a deployment would register                                                                                                                                                            |
...
| `-yolo-mode`                 | string | _(empty)_ | `true` skips the confirmation prompt on destructive actions. Sets `YOLO_MODE`, which takes precedence over `AUTOPILOT`                                                                                                                                                                                                   |
...
| `-action-timeout`         | duration | `65m`                      | Cancel an action still running after this long; `0` disables it (upper bound: 24h). Both transports read `GITLAB_MCP_ACTION_TIMEOUT` when the flag is not passed. Above the longest wait any action offers (a pipeline wait caps itself at 3600 s), so it ends a handler nobody else bounds rather than a legitimate call                                                                                                                                                                                                                                                                                               |
...
| `-rate-limit-rps`         | float    | `10`                       | Per-server rate limit, in requests/second, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get`). `0` disables it. On by default in HTTP mode: the deployment is shared, so one looping client's calls are charged to its egress address. Stdio defaults to `0`. See [Security — Rate Limiting Model](../concepts/security.md#rate-limiting-model)                                                                                                                                                                                           |
