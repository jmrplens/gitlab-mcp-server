...
| `-gitlab-url`             | string   | _(optional)_               | GitLab instance URL. Omit it to require each client to send `GITLAB-URL` per request. **Repeatable** (or comma-separated): publishing several instances makes the first the default, lists them all in the RFC 9728 `authorization_servers` field, and makes `GITLAB-URL` a choice **among them** — a value naming anything else is refused, not ignored                                                |
...
| `-rate-limit-rps`         | float    | `10`                       | Per-server `tools/call` rate limit in requests/second. `0` disables it. On by default in HTTP mode: the deployment is shared, so one looping client's calls are charged to its egress address. Stdio defaults to `0`. See [Security — Rate Limiting Model](../concepts/security.md#rate-limiting-model)                                                                                                 |
