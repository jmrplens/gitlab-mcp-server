...
| `-exclude-tools`              | string   | _(empty)_      | Comma-separated tool names, group names or canonical action IDs, excluded from the tool surface and from the resources, subscriptions, prompts and argument completions that return the same objects, so the removal holds on every request path. The same spellings reach the standalone utilities on every surface (`gitlab_interactive`, `interactive.issue_create`, `discover_project.resolve`), and an entry that names nothing is logged as a warning rather than refused, when the first request for a configuration builds its catalog. The warning also names `gitlab_find_action` and `gitlab_execute_action`, although the dynamic surface removes them by name                               |
