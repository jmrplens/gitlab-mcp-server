...
| `-rate-limit-rps`         | float    | `10`                       | Per-credential rate limit, in requests/second, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `subscriptions/listen`, `prompts/get`). `0` disables it. On by default in HTTP mode: the deployment is shared, so one looping client's calls are charged to its egress address. Stdio defaults to `0`. See [Security — Rate Limiting Model](../concepts/security.md#rate-limiting-model)                                                                                                                                                                                       |
