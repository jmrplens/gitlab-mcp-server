...
| `TRUSTED_ORIGINS`             | *(empty)* | Comma-separated absolute origins (`scheme://host[:port]`) allowed to make cross-origin browser requests. `*` accepts any origin and disables the protection; empty rejects all |
