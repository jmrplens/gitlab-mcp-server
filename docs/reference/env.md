...
| `GITLAB_MCP_META_PARAM_SCHEMA`  | `opaque`     | Meta-tool input-schema strategy: `opaque` (compact `{action, params:any}` envelope, default), `compact` (oneOf with property names + types only, 6.5x opaque size) or `full` (oneOf with full per-action JSON Schemas, 11.9x opaque size). Applies to visible meta-tool schemas in `meta`; has no practical effect on `dynamic` or `individual` tool schemas. Per-action call shapes and JSON Schemas are discoverable through `gitlab://tools` and `gitlab://tools/{id}` for every surface                                                                             |
...
| `GITLAB_READ_ONLY`              | `false`      | Read-only mode: mutating operations are removed **per action**, not per tool, so reads keep working on every surface — `gitlab_issue` survives with its read actions and `gitlab_execute_action` stays available (annotated read-only) for the reads it can route. On the individual surface, where one tool is one action, this is simply the removal of the mutating tools (`true`/`false`)                                                                                                                                                                           |
| `GITLAB_SAFE_MODE`              | `false`      | Safe mode: intercepts mutating operations **per action** and returns a structured JSON preview naming that action instead of executing, so `issue.create` previews while `issue.list` executes. If `GITLAB_READ_ONLY=true`, it takes precedence (`true`/`false`)                                                                                                                                                                                                                                                                                                        |
...
| `GITLAB_MCP_CLIENT_COMPAT`      | `auto`       | Per-client response compatibility. On `auto`, a session identifying itself as OpenAI Codex gets the float `priority` in content and resource annotations rounded to 0 or 1 — the Codex builds bundled with ChatGPT.app reject a fractional value and mark the call as an unexpected response type. Every other field, and every other client, is delivered unchanged. Set `off` to disable the rewriting. Read from the process environment in both stdio and HTTP modes — it has no CLI flag equivalent. See [Client Compatibility](../guides/client-compatibility.md) |
...
| Variable                          | Default | Description                                                                                                                  |
| --------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `GITLAB_MCP_UPLOAD_MAX_FILE_SIZE` | `2GB`   | Maximum file size for upload tools. Supports human-friendly suffixes: `KB`, `MB`, `GB` (case-insensitive). Upper bound: 1 TB |
...
| Variable                       | Default   | Description                                                                                                                                                                                                                                                                |
| ------------------------------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GITLAB_MCP_MAX_HTTP_CLIENTS`  | `100`     | Maximum concurrent client sessions in the server pool. Upper bound: 10,000                                                                                                                                                                                                 |
| `SESSION_TIMEOUT`              | `30m`     | Idle MCP session timeout. Upper bound: 24h                                                                                                                                                                                                                                 |
| `GITLAB_MCP_ACTION_TIMEOUT`    | `1h`      | Cancel an action still running after this long, in both transports; `0` disables it. Upper bound: 24h. Above the longest wait any action offers (a pipeline wait caps itself at 3600 s), so it ends a handler nobody else bounds, not a legitimate call                    |
| `GITLAB_MCP_POOL_IDLE_TIMEOUT` | `1h`      | Reclaim a pooled per-token-and-URL server entry after this long unused; `0` keeps entries until the pool size bound evicts them. Upper bound: 24h                                                                                                                          |
...
| `GITLAB_MCP_RATE_LIMIT_RPS`    | `0`       | Per-server rate limit, in requests/second, on every call that reaches GitLab (`tools/call`, `resources/read`, `resources/subscribe`, `prompts/get`). `0` disables the limiter (default). See [Security — Rate Limiting Model](../concepts/security.md#rate-limiting-model) |
...
The rows marked *(none)* below have no environment counterpart at all — the transport flags in particular. A deployment that configures everything else through the environment must still pass those on the command line.
...
| *(none)*                        | `--resource-documentation` | CLI-only; https URL published as RFC 9728 `resource_documentation`. Empty publishes this project's OAuth setup guide                                                                                                                                                                                                                                                                                                                                             |
