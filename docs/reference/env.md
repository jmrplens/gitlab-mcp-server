...
| `CAPABILITY_SURFACE`   | `full`       | Resource and prompt catalog selector: `full` keeps the complete catalog; `minimal` keeps the surface-aware `gitlab://tools` manifest and disables optional GitLab data resources, workflow guides, and prompts. Dynamic describe/find still returns action schemas inline with `minimal`                                                                                                                                                                                                    |
...
| `POOL_IDLE_TIMEOUT`           | `1h`     | Reclaim a pooled per-token server after this long unused; `0` keeps entries until the pool size bound evicts them. Upper bound: 24h                                          |
