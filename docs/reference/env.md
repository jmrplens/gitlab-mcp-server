...
| `TRUSTED_ORIGINS`             | *(empty)* | Comma-separated absolute origins (`scheme://host[:port]`) allowed to make cross-origin browser requests. `*` accepts any origin and disables the protection; empty adds none, though a configured `PUBLIC_URL` origin is trusted regardless |
...
| ----------------------------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GITLAB_URL`                  | `--gitlab-url`             | Optional in stdio mode; defaults to `https://gitlab.com`. Optional in HTTP mode unless `--auth-mode=oauth` is used, which requires at least one instance. When set in HTTP mode it fixes the GitLab instance; when omitted, clients must send `GITLAB-URL` per request. A comma-separated value (or a repeated `--gitlab-url`) publishes several instances: the first is the default and `GITLAB-URL` then selects among them, refusing anything else |
