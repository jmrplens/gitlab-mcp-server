...
| G6 orphans      | Every exported constant of the register's value and code files is read by a declared site and by something outside the register; every declared Alias is read by code other than an alias initializer, directly or through a chain of aliases, so an enforcing site gone back to a literal fails; every rule function is named by a row                                                                            |
...
| G8 refusal      | A refusal's stable text begins a string its code folds, and its literal carries the row's status, code, `Retry-After` source and challenge; a JSON-RPC refusal carries the row's code, read from the case naming its sentinel where its holder switches on one, and a tool-error result is flagged as an error; every gate literal of a function holding a declared gate refusal is carried exactly by one of them |
