...
It loads `./cmd/server` and `./internal/...` through `cmd/internal/goprogram`, without test files, and leaves out the test-support packages the server never links (`internal/testutil/...`, `internal/graphqlschema`, `internal/freshness`). Fourteen rules:
...
| G6 orphans      | Every exported constant of the register's value and code files is read by a declared site and by something outside the register; every rule function is named by a row                                                               |
| G7 charges      | Every refusal the authentication failure table's functions return matches one row, charged exactly when the charge helper is called before it in its own block; only the charge helpers spend a budget                               |
| G8 refusal      | A refusal's stable text begins a string its code folds, and its literal carries the row's status, code, `Retry-After` source and challenge; a JSON-RPC refusal carries the row's code and a tool-error result is flagged as an error |
| G9 reason       | A quoted reason is still in the doc comment of the declaration it names, or of the block around it                                                                                                                                   |
| G10 tripwire    | A limit constructor, a refusal literal that reads as a limit's and a package-level name that reads as a limit each sit in a declared site or are answered by the exemption table                                                     |
| G11 validate    | The register's own validators accept it                                                                                                                                                                                              |
| G12 leaf        | The register imports only packages the server already imports, and declares no package-level variable                                                                                                                                |
...
| G14 config      | Every variable a row names is on the configuration package's list of prefixed names and read by nothing else, unless its row carries the finding that records it                                                                     |
...
**What it cannot see.** A new limit that uses none of the listed constructors, refuses with a code outside the policy set or not at all, has no lexicon word in its name, and lives in a package no row names escapes the tripwire, and only review sees it. A variable read through a name that does not fold is not traced. A charge is bound to a return by position, so a charge moved into a helper the table does not name fails rather than passing, with a finding that names the position rather than the policy. A rule row's logic is declared by symbol: the rule fails when the symbol disappears, not when its logic changes.
