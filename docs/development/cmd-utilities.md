...
| `audit_tenancy`                | Source quality audits         | Holds the tenant policy register (`internal/tenancy`) to the code that enforces it: every row's sites, values, refusals, charges and reasons against the source, and anything shaped like a limit to a row or a reasoned exemption; `-compare-binaries` proves a change moved no code                                                                     | `make check-tenancy`, `make audit-tenancy`, `make tenancy-code-identity`                                                                                                |
...
| G8 refusal      | A refusal's stable text begins a string its code folds, and its literal carries the row's status, code, `Retry-After` source and challenge; a JSON-RPC refusal carries the row's code, read from the first case naming its sentinel where its holder switches on one, and a tool-error result is flagged as an error; every gate literal of a function holding a declared gate refusal is carried exactly by one of them                                                                                                                   |
...
| `check-tenancy`                          | `audit_tenancy -check`                                           | Every row of the tenant policy register is held to the code, nothing shaped like a limit sits outside a row or a reasoned exemption, and every exemption still answers something                   | Non-zero on any finding of G1 to G14 or stale exemption, or if the source cannot be loaded                                                                          |
