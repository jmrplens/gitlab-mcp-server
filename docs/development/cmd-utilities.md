...
| G8 refusal      | A refusal's stable text begins a string its code folds, and its literal carries the row's status, code, `Retry-After` source and challenge; a JSON-RPC refusal carries the row's code, read, where its holder switches on its sentinel, from the first case naming the sentinel in the last switch whose such case assigns the code, and a tool-error result is flagged as an error; every gate literal of a function holding a declared gate refusal is carried exactly by one of them                                                                                                                                                                                                                                                |
| G9 reason       | A quoted reason is still in the doc comment of the declaration it names, or of the block around it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| G10 tripwire    | A limit constructor or a literal of a limit's options type sits in a declared Enforce site; a refusal literal that reads as a limit's, and a 429 or 503 written with `http.Error` or `WriteHeader`, is the refusal a row declares in its function, by code or status; a package-level name that reads as a limit is a declared site; each is otherwise answered by the exemption table                                                                                                                                                                                                                                                                                                                                                 |
