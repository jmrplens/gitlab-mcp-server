...
| `audit_tenancy`                | Source quality audits         | Holds the tenant policy register (`internal/tenancy`) to the code that enforces it: every row's sites, values, refusals, charges and reasons against the source, and anything shaped like a limit, in a shape its tripwire reads, to a row or a reasoned exemption; `-compare-binaries` proves a change moved no code                                     | `make check-tenancy`, `make audit-tenancy`, `make tenancy-code-identity`                                                                                                |
...
Holds the tenant policy register, `internal/tenancy` ([ADR-0023](adr/adr-0023-tenant-policy-is-declared-once.md)), to the code that enforces it.

The register declares every decision this server makes about who a caller is and what it may hold, spend or be told: one row per requirement of the [tenant policy specification](tenant-policy-spec.md), with the symbols that decide, enforce, refuse and state it. Its own `Validate` holds the rows to the specification's invariants, and cannot see the code, since the register imports nothing. This command is what binds the two, so a row cannot keep describing code that moved, and a new limit cannot land without a row when the tripwire reads it: built with a listed constructor or options type outside every declared Enforce site or from a literal or a package value no row declares, refused through a refusal-shaped literal or a 429 or 503 status write in a function that declares no refusal of that code or status, or named like a limit at package level in a package that holds an Alias, Arg, Pin or Enforce site of a row that is not a request bound (see What it cannot see for the limits it does not read).

It loads `./cmd/server` and `./internal/...` through `cmd/internal/goprogram`, without test files, and leaves out the test-support packages the server never links (`internal/testutil/...`, `internal/graphqlschema`, `internal/freshness`). The load names `linux/amd64` whatever the host is, so the verdict does not depend on who runs it: a load that followed the host left out every file constrained away from it, and an exemption naming a declaration in `socket_mode_unix.go` failed the gate on Windows while it passed on Linux. A file constrained to another platform is not read. Fourteen rules:

| Rule            | Holds                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G1 resolve      | Every site of every row, key derivation and failure, and every exemption, names one declaration; no exemption is also a site                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| G2 alias        | An Alias site is a reference to the register constant it names, or to a declared alias of it, and a const keeps the constant's typedness; every element of an aliased literal is one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| G3 arg          | Inside an Arg site's function, the calls it names pass the register constant at its index, exactly as many times as it says                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| G4 pin          | A Pin site keeps its own literal, equal in value and type to the register constant (the second statements issue 958 records)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| G5 references   | An Enforce site that names what it reads refers to it, and a register function a row names is called from one of its Enforce sites                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| G6 orphans      | Every exported constant of the register's value and code files is read by a declared site and by something outside the register; every declared Alias is read by code other than an alias initializer, directly or through a chain of aliases, so an alias whose every reader has gone back to a literal fails; every rule function is named by a row. One reader going back to a literal while another still reads the alias passes; another rule fails it only where it writes the literal straight into a listed limit constructor or options literal (G10), is itself a declared Alias (G2), changed a call a declared Arg names at the index it names (G3), or is an Enforce site declared to read that constant (G5, none today) |
| G7 charges      | Every refusal the authentication failure table's functions return matches one row, charged exactly when the charge helper is called before it in its own block or in a block enclosing it; only the charge helpers spend a budget; each failure agrees with its row's gate refusal of the same status and prefix, which names a budget exactly when the failure is charged                                                                                                                                                                                                                                                                                                                                                             |
| G8 refusal      | A refusal's stable text begins a string its code folds, and its literal carries the row's status, code, `Retry-After` source and challenge; a JSON-RPC refusal whose code is a local variable has the row's code among the constants assigned to it, which, where its holder switches on its sentinel, are those assigned in one case body, the first case naming the sentinel in the last switch whose such case assigns the code; a tool-error result is flagged as an error; every gate literal of a function holding a declared gate refusal is carried exactly by one of them                                                                                                                                                     |
| G9 reason       | A quoted reason is still in the doc comment of the declaration it names, or of the block around it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| G10 tripwire    | A limit constructor or a literal of a limit's options type sits in a declared Enforce site; a refusal literal that reads as a limit's, and a 429 or 503 written with `http.Error` or `WriteHeader`, is the refusal a row declares in its function, by code or status; a package-level name that reads as a limit, in a package that holds an Alias, Arg, Pin or Enforce site of a row that is not a request bound, is a declared site; each is otherwise answered by the exemption table                                                                                                                                                                                                                                               |
| G11 validate    | The register's own validators accept it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| G12 leaf        | The register imports only packages the server already imports, and declares no package-level variable and no init function                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| G13 share words | A number on a key a caller can mint is not called fair, a quota or an entitlement, unless its row carries the finding that records it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| G14 config      | Every variable a row names is on the configuration package's list of prefixed names and read by nothing else (`os.Getenv`, `os.LookupEnv`, their `syscall` twins, a template `os.ExpandEnv` expands), unless its row carries the finding that records it                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

`cmd/audit_tenancy/declarations.go` holds the table a reviewer judges. `notADecision` is the exemption table: a declaration shaped like a limit that decides nothing about a caller, keyed `package:Name`, with a category and a reason. A declaration that answers nothing is a finding: an exemption nothing needed, and one naming an undefined category. Every rule applies to every row. While the values of issue 565 moved into the register, a `pending` list deferred G2, G3 and G6 for the rows whose values had not moved yet; the last value layer emptied it and the layer that closed the gate deleted it, so from the day a row is written an alias that is not a reference to its constant, an Arg call that does not pass it and an alias nothing but alias initializers read are findings. A layer that reads the row's value as a literal somewhere else, while its alias is still read, is a finding only where a rule reads that place: a literal written straight into a listed limit constructor or options literal (G10), a declared Alias (G2), the call a declared Arg names for that constant (G3), or an Enforce site declared to read it (G5).

**What it cannot see.** A new limit escapes the tripwire, and only review sees it, when it uses none of the listed constructors or options types; makes no refusal the literal rule holds to a row, because it refuses not at all, or through none of the three literal types and no 429 or 503 status write, or through such a literal or write in a function that already holds a declared refusal of its code or status, or by calling, from anywhere, a function whose refusal a row already declares (a new counter guarded by a mutex inside `toolutil.AttachRateLimitFunc` that returns the `-42900` the declared `rateLimitedError` builds is one: the rule reads the literals a function writes, never the refusals it obtains from a call); and is named in a way the name rule does not read: with no lexicon word, in a package that holds no Alias, Arg, Pin or Enforce site of a row that is not a request bound (a package a row names only through its Refuse, Reason, Charge or Derive sites, or that only request-bound rows name, is not read: `internal/completions` and `internal/tools/dynamic` are two), or as a value local to a function, which the name rule never reads (the ten seconds `revalidateAll` gives each probe is one: ADM-009 states it and no row value binds it). Any one of those three ways of naming is enough on its own. A layer that stops reading a register value in one place while another place still reads its alias passes G6, and fails only where another rule reads the place that stopped: where it writes the literal straight into a listed limit constructor's arguments or a listed options literal (G10, declared site or not), is itself a declared Alias (G2), changed a call a declared Arg names at the index it names (G3), or is an Enforce site declared to read that constant (G5, which none is today). Anywhere else it passes: `config.Load` going back to a literal burst while the `--rate-limit-burst` default and the HTTP overlay still read `config.DefaultRateLimitBurst` is that case, although `Load` is a declared Arg, since the Arg it is holds the rate's call to `parseFloatNonNegative` and not the burst's. Of those four rules, G2 judges every declared Alias and G3 every call a declared Arg names, so each holds every place it names; G5 fails an Enforce site declared to read the constant only once nothing in its body refers to the constant or a declared alias of it, so it holds the last such reference and not each one. G8 reads a code held in a local variable as a set of constants, with no control flow. Where no switch of the holder names the refusal's sentinel, the set is every constant the holder assigns the variable. Where one does, the set is every constant assigned in one case body, the first case naming the sentinel in the last switch whose such case assigns the code, and nothing assigned outside that case is read: a second assignment in the case joins the set rather than replacing the first, and an assignment after the switch, plain or under an `if` naming the sentinel, is not read at all, so a code changed in any of those three ways passes. A case whose condition adds more than the sentinel is still taken as the one that matches it, an earlier case that matches the sentinel's error without naming it (by its text or its type) is not seen, and neither is a return, a branch or a loop that keeps a later switch from running for the sentinel. A variable read through a name that does not fold is not traced. A charge is bound to a return by position, so a charge moved into a helper the table does not name fails rather than passing, with a finding that names the position rather than the policy. A rule row's logic is declared by symbol: the rule fails when the symbol disappears, not when its logic changes.

**Code identity.** `-compare-binaries A B` reads two ELF binaries and exits non-zero unless every allocated section but the line table, `.gopclntab`, is byte-identical. It is how a change that moves a value into the register proves it moved no code. `make tenancy-code-identity BASE=<ref>` exports the ref with `git archive`, builds `./cmd/server` there and in the working tree with `-trimpath -buildvcs=false` and no build id, and runs the comparison; `BASE_TREE=<dir>` names an exported tree instead, for a build host that holds no history.

What the change is compared against is the whole of the proof, and the value layers of issue 565 measured it. A change that inserts no line into the server's source and gives the register no new importer is compared against its parent as it is. A change that imports the register into a file fails against its parent without changing any code: the inserted lines resize the line table, which can move the sections the linker lays out after it (on Go 1.27.1 the type data among them) and every reference into them; and a package that imports the register for the first time changes where the linker places content-addressed read-only data, or which copy of a duplicated compiler-generated function it keeps. Such a change is compared against its parent with the same import lines added as blank imports (`_ "github.com/jmrplens/gitlab-mcp-server/v3/internal/tenancy"`) at the same positions, exported and passed as `BASE_TREE`. Against that base each value layer was byte-identical as a whole file, stripped and with DWARF, while every one of them failed against its parent. A change to the register itself is a third case, which the review layer of issue 565 measured: rows, text and code the server never reaches still change the package's object, the linker then lays out content-addressed data and duplicated closures differently, and the comparison against the parent fails in eight sections with every symbol present and the same size. Such a change is compared against its parent with `internal/tenancy` replaced by its own, exported and passed as `BASE_TREE`, which proves nothing outside the register moved; that the register's linked code did not change is its oracle tests' and value pins' to show. A rule promoted into the register is not proved by the binary: a layer calls it, and the compiler lays its callers out differently. Its proof is an oracle test and a fuzz target that hold it to a verbatim copy of the code it replaced, with its allocations pinned.

#### Usage

```bash
# Report
go run ./cmd/audit_tenancy/

# Report, listing what the exemption table answered
...
# Two binaries
go run ./cmd/audit_tenancy/ -compare-binaries base.bin head.bin
```
...
| Flag                | Type     | Default | Description                                                                  |
| ------------------- | -------- | ------- | ---------------------------------------------------------------------------- |
| `-dir`              | `string` | `.`     | Repository root the packages are loaded from                                 |
| `-check`            | `bool`   | `false` | Exit non-zero when the register and the code disagree                        |
| `-v`                | `bool`   | `false` | Also list what the exemption table answered                                  |
| `-compare-binaries` | `bool`   | `false` | Compare the two ELF binaries given as arguments instead of auditing the tree |

The gate takes no package patterns: it always loads the server and `internal/`, since a row may name any package there.

#### Output

Every finding with its rule, what it is about, its position where it has one, and what is wrong; with `-v`, every declaration the exemption table answered, with the part of G10 it answered, its category and its reason; then a summary counting the rows, the failures, the declared sites and the packages, what the behavioral rules read (refusal returns, refusals, reasons and settings, so a rule that reads nothing shows as zero), the findings and the exemptions used. Exits `1` under `-check` on any finding, and `1` whenever the source could not be loaded.

#### Make targets

- `make audit-tenancy`: the report, verbose.
- `make check-tenancy`: CI gate, also step 23 of `make analyze`.
- `make tenancy-code-identity BASE=<ref>`: the code-identity proof for a change that claims to move policy without changing it.
...
The go/packages front end for the gates that type-check this repository's own source. Ten packages load through it: [`audit_md_escaping`](#audit_md_escaping) walks the calls that interpolate a GitLab-authored value into Markdown, [`audit_readonly_graphql`](#audit_readonly_graphql) walks the calls a read-only action can reach, `cmd/internal/graphqldocs` folds every raw GraphQL document to the one string GitLab would receive, [`audit_graphql_shapes`](#audit_graphql_shapes) pairs each of those documents with the struct that decodes it, [`audit_action_ids`](#audit_action_ids) folds the action IDs the server publishes to a model, [`audit_catalog_first`](#audit_catalog_first) resolves the calls that aggregate each package's ActionSpecs, [`audit_dead_consts`](#audit_dead_consts) holds every unexported constant to something that reads it, [`audit_sdk_context`](#audit_sdk_context) holds every call into client-go to the caller's context, [`audit_tenancy`](#audit_tenancy) holds the tenant policy register to the code that enforces it, and [`audit_e2e_coverage`](#audit_e2e_coverage)'s static check reads the end-to-end test packages. It owns `LoadMode`, `Load(dir, patterns, overlay)`, `LoadWith` for a load that needs test variants or build tags, and the refusal of a package that did not type-check; the indexers, the detectors, the questions and the binaries stay with each gate.
...
The `overlay` parameter is not a convenience. It is how the tests of seven of the ten (`audit_action_ids`, `audit_dead_consts`, `audit_md_escaping`, `audit_readonly_graphql`, `audit_sdk_context`, `audit_tenancy` and `cmd/internal/graphqldocs`) supply a fixture package written in the test file itself, type-checked against the real packages it imports, so the classifiers are exercised on the shapes they have to handle rather than on a mock of them. Production passes `nil`, and so do the tests of the other three, which write their fixtures to disk: `audit_graphql_shapes` and `audit_catalog_first` a module in a temporary directory, `audit_e2e_coverage` the planted trees under its `testdata`.
...
| `check-tenancy`                          | `audit_tenancy -check`                                           | Every row of the tenant policy register is held to the code, nothing shaped like a limit, in a shape the tripwire reads, sits outside a row or a reasoned exemption, and every exemption still answers something | Non-zero on any finding of G1 to G14 or stale exemption, or if the source cannot be loaded                                                                          |
