...
`cmd/audit_tenancy/declarations.go` holds the two tables a reviewer judges. `notADecision` is the exemption table: a declaration shaped like a limit that decides nothing about a caller, keyed `package:Name`, with a category and a reason. `pending` lists the rows whose values have not moved into the register yet; G2, G3 and G6 are deferred for them and every other rule applies in full, and each value layer of issue 565 deletes the rows it moves. A declaration that answers nothing is a finding: an exemption nothing needed, one naming an undefined category, a pending entry that names no row, and a pending row whose deferred checks already pass.
...
**Code identity.** `-compare-binaries A B` reads two ELF binaries and exits non-zero unless every allocated section but the line table, `.gopclntab`, is byte-identical. It is how a layer that moves a value into the register proves it moved no code: the compiler's assembly for such a package gains a relocation to the new import's init task, while the linked binary's instructions, data, types and function metadata stay the same, which was measured before it was relied on. `make tenancy-code-identity BASE=<ref>` exports the ref with `git archive`, builds `./cmd/server` there and in the working tree with `-trimpath -buildvcs=false` and no build id, and runs the comparison; `BASE_TREE=<dir>` names an exported tree instead, for a build host that holds no history.
...
Every finding with its rule, what it is about, its position where it has one, and what is wrong; then the pending rows; with `-v`, every declaration the exemption table answered, with the part of G10 it answered, its category and its reason; then a summary counting the rows, the failures, the declared sites and the packages, what the behavioral rules read (refusal returns, refusals, reasons and settings, so a rule that reads nothing shows as zero), the findings, the pending rows and the exemptions used. Exits `1` under `-check` on any finding, and `1` whenever the source could not be loaded.
...
| `check-tenancy`                          | `audit_tenancy -check`                                           | Every row of the tenant policy register is held to the code, nothing shaped like a limit sits outside a row or a reasoned exemption, and every exemption and pending row still answers something   | Non-zero on any finding of G1 to G14, stale exemption or pending row, or if the source cannot be loaded                                                             |
