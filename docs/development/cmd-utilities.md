...
Why 180 days: GitLab ships monthly and narrows fields in place, so half a year is roughly six releases of drift — long enough not to ambush an unrelated change often, short enough that a narrowing is noticed within a release cycle or two. A record older than that is a gate that has quietly stopped asking, and the only honest way to say so is to fail. This paragraph and `MaxAge` are the whole of that decision: each of the three command sections says that its `--check` refuses a record past the window and links here, and none of them restates the number or the reason.
