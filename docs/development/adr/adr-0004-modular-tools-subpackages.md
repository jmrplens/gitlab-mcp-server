...
**Accepted** — refined by [ADR-0005](adr-0005-meta-tool-consolidation.md) (meta-tool consolidation). Runtime registration mechanics are superseded by the catalog-first architecture in [tool surfaces and action core](../tool-surfaces-and-action-core.md): domain sub-packages remain the ownership boundary, but root tool surfaces are projected from canonical `ActionSpec` metadata.
