...
| ---------------- | ------------------------------------------ | --------------------------------------------: | ---------------------------------------------------------------------- |
| Individual tools | `GITLAB_MCP_TOOL_SURFACE=individual`       |                 One tool per GitLab operation | Canonical action catalog projected by `RegisterIndividualCatalogTools` |
...
`CommonConfusions`) and a field's description, a `jsonschema` tag or a schema
map's `description` entry, all name an action by its canonical ID. Each is
...
tool names in its `See also:` clause alone, the one part `gitlab://tools`
rewrites into each surface's names when it serves that description on the
dynamic and meta surfaces, and the rest of it is read like a `Usage` line.
A standalone surface tool's keeps none: the guided flows and project discovery are registered on meta and
...
`internal/resources`, which are outside its load.
