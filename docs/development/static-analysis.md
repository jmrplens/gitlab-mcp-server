...
| `make check-api-shapes`             | The REST twin of the schema pin: fail when the committed OpenAPI record GitLab generates from its own Grape definitions is one this build cannot read, is truncated, carries no provenance, records a retrieval date nothing can read or that has not happened, or is older than 180 days                                                                                                                                                                                           |
