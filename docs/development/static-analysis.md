...
| Target                | Description                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `make analyze`        | Run `golangci-lint config verify`, `golangci-lint fmt --diff`, `golangci-lint run`, `govulncheck`, `markdownlint`, the test-goroutine abort gate, the subtest gate, the supply-chain policy gate, the Markdown escaping gate, the three GraphQL gates (pinned schema, documents, response shapes), the request-path gate, the meta-description gate and the pinned GitLab API record |
...
| `make check-graphql-shapes`         | Fail when a struct a GraphQL response is decoded into cannot hold what its document selects, declares a field the document never selects, or reads a scalar the audit has no serialization for; the request-side gates read the document and the variables, this one reads the answer                                                                                                                                                                                               |
| `make audit-graphql-shapes`         | Same check, listing every pairing judged and every selection nothing reads                                                                                                                                                                                                                                                                                                                                                                                                          |
| `make check-api-exposes`            | Fail when the committed record of the conditions GitLab sends each REST entity field under cannot be read, is too short to be GitLab's whole entity tree, lacks provenance, or is older than the shared staleness window                                                                                                                                                                                                                                                            |
| `make gen-api-exposes`              | Re-pin that record from GitLab's Ruby source at a ref; needs the network, so it is not a gate (`-source` reads a local checkout instead)                                                                                                                                                                                                                                                                                                                                            |
...
| `make check-api-shapes`             | The REST twin of the schema pin: fail when the committed OpenAPI record GitLab generates from its own Grape definitions is one this build cannot read, is truncated, carries no provenance, records a retrieval date nothing can read or that has not happened, or is past the shared staleness window                                                                                                                                                                              |
| `make gen-api-shapes`               | Re-read that record from `gitlab-org/gitlab`; needs the network, so it is not a gate                                                                                                                                                                                                                                                                                                                                                                                                |
